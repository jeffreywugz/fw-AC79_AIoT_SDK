...
extern void auto_shutdown_disable(void);
...
//------------------------------------------------------
int bt_is_connect(void)
{
    return (ble_work_state == BLE_ST_NOTIFY_IDICATE ? 1 : 0);
}


int bt_tx_data(void *priv, u8 *data, u16 len)
{
    return app_send_user_data_do(priv, data, len);
}
...
#if 1
#define LOCAL_NAME_LEN	32
    char name_p[LOCAL_NAME_LEN] = "JL_HID";
