...
#if (TCFG_BLE_DEMO_SELECT == DEF_BLE_DEMO_MAMBO)
...
static uint16_t att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    case ATT_CHARACTERISTIC_a620_01_CLIENT_CONFIGURATION_HANDLE:
    case ATT_CHARACTERISTIC_a621_01_CLIENT_CONFIGURATION_HANDLE:
    case ATT_CHARACTERISTIC_a625_01_CLIENT_CONFIGURATION_HANDLE:
        if (buffer) {
...
static int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    case ATT_CHARACTERISTIC_a625_01_CLIENT_CONFIGURATION_HANDLE:
        ...
        break;
    case ATT_CHARACTERISTIC_a622_01_VALUE_HANDLE:
        printf("\n-a622_rx(%d):", buffer_size);
        printf_buf(buffer, buffer_size);
        break;
    case ATT_CHARACTERISTIC_a623_01_VALUE_HANDLE:
        printf("\n-a623_rx(%d):", buffer_size);
        printf_buf(buffer, buffer_size);
        break;
    case ATT_CHARACTERISTIC_a624_01_VALUE_HANDLE:
        printf("\n-a624_rx(%d):", buffer_size);
        printf_buf(buffer, buffer_size);
        break;
...
static int make_set_adv_data(void)
{
    u8 offset = 0;
    u8 *buf = adv_data;
    u8 manufacturer_data[11];
    //Company ID（2 bytes） + 厂商ID（2 Bytes）+ 注册状态（1byte）+ MAC(6 bytes)
    le_controller_get_mac(manufacturer_data + 5);
    ...
    offset += make_eir_packet_val(&buf[offset], offset, HCI_EIR_DATATYPE_MORE_16BIT_SERVICE_UUIDS, 0xa602, 2);
    offset += make_eir_packet_data(&buf[offset], offset, HCI_EIR_DATATYPE_MANUFACTURER_SPECIFIC_DATA, manufacturer_data, 11);
...
#endif
    return app_send_user_data(ATT_CHARACTERISTIC_a621_01_VALUE_HANDLE, (const char *)data, len, ATT_OP_AUTO_READ_CCC);
}
