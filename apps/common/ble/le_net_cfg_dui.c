...
#include "le_net_cfg_dui.h"
#include "le_common.h"

#if (TCFG_BLE_DEMO_SELECT == DEF_BLE_DEMO_NET_CFG_DUI)
...
static const char user_description[] = "NETWORK CONFIG POINT";
...
static uint16_t att_read_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    case ATT_CHARACTERISTIC_2222_01_CLIENT_CONFIGURATION_HANDLE:
        ...
        att_value_len = 2;
        break;
    case ATT_CHARACTERISTIC_2222_01_USER_DESCRIPTION_HANDLE:
        att_value_len = strlen(user_description);
        if ((offset >= att_value_len) || (offset + buffer_size) > att_value_len) {
            break;
        }
        if (buffer) {
            memcpy(buffer, &user_description[offset], buffer_size);
            att_value_len = buffer_size;
        }
        break;
    default:
...
static int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    static char ssid_buf[118];
    static char user_buf[117];
    static u8 recv_ssid = 0;
    ...
    case ATT_CHARACTERISTIC_2222_01_VALUE_HANDLE:
        ...
        int send_len = 0;
        //握手信息
        if (buffer_size == 8 && !strncmp((char *)buffer, "AISPEECH", strlen("AISPEECH"))) {
            user_buf_offset = 0;
            user_data_size = 0;
            recv_ssid = 1;
            memset(user_buf, 0, sizeof(user_buf));
            memset(ssid_buf, 0, sizeof(ssid_buf));
            send_len = strlen("AISPEECH-ACK");
            send_len = ntohl(send_len);
            app_send_user_data(ATT_CHARACTERISTIC_2222_01_VALUE_HANDLE, (const char *)&send_len, sizeof(send_len), ATT_OP_AUTO_READ_CCC);
            app_send_user_data(ATT_CHARACTERISTIC_2222_01_VALUE_HANDLE, "AISPEECH-ACK", strlen("AISPEECH-ACK"), ATT_OP_AUTO_READ_CCC);
            break;
        }
        //结束包
        if (buffer_size == 8 && !strncmp((char *)buffer, "DONE-ACK", strlen("DONE-ACK"))) {
            printf("ble cfg net done!\n");
            extern int bt_net_config_set_ssid_pwd(const char *data);
            bt_net_config_set_ssid_pwd(ssid_buf);
            break;
        }
        //开始接收SSID信息
        if (0 == user_data_size && buffer_size == 4) {
            user_data_size = (buffer[2] << 8) + buffer[3];
        } else {
            if (buffer_size + user_buf_offset < sizeof(user_buf)) {
                memcpy(&user_buf[user_buf_offset], buffer, buffer_size);
            }
            user_buf_offset += buffer_size;
            if (user_buf_offset == user_data_size) {
                if (recv_ssid) {
                    send_len = strlen("00000");
                    send_len = ntohl(send_len);
                    app_send_user_data(ATT_CHARACTERISTIC_2222_01_VALUE_HANDLE, (const char *)&send_len, sizeof(send_len), ATT_OP_AUTO_READ_CCC);
                    app_send_user_data(ATT_CHARACTERISTIC_2222_01_VALUE_HANDLE, "00000", strlen("00000"), ATT_OP_AUTO_READ_CCC);
                    strcpy(ssid_buf, user_buf);
                } else {
                    extern void dui_set_net_cfg_uid(const char *uid);
                    dui_set_net_cfg_uid(user_buf);
                    app_send_user_data(ATT_CHARACTERISTIC_2222_01_VALUE_HANDLE, "DONE", strlen("DONE"), ATT_OP_AUTO_READ_CCC);
                }
                user_data_size = 0;
                user_buf_offset = 0;
                memset(user_buf, 0, sizeof(user_buf));
...
static int make_set_adv_data(void)
{
    ...
    le_controller_get_mac(tmp_ble_addr);
    ...
    offset += make_eir_packet_val(&buf[offset], offset, HCI_EIR_DATATYPE_MORE_16BIT_SERVICE_UUIDS, 0x1111, 2);
    offset += make_eir_packet_data(&buf[offset], offset, HCI_EIR_DATATYPE_MANUFACTURER_SPECIFIC_DATA, tmp_ble_addr, 6);
...
#endif
    return app_send_user_data(ATT_CHARACTERISTIC_2222_01_VALUE_HANDLE, (const char *)data, len, ATT_OP_AUTO_READ_CCC);
}
