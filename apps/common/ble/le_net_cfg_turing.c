...
#include "le_net_cfg_turing.h"
#include "le_common.h"

#if (TCFG_BLE_DEMO_SELECT == DEF_BLE_DEMO_NET_CFG_TURING)
...
static int app_send_user_data(u16 handle, const char *data, u16 len, u8 handle_type);
...
static const char rsp_cmd[][5] = {
    {0xA0, 0x01, 0x00, 0x00, 0x5F},
    {0xA0, 0x00, 0x01, 0x00, 0x5F},
    {0xA0, 0x00, 0x02, 0x00, 0x5E},
    {0xA0, 0x00, 0x03, 0x00, 0x5D},
};

static u8 ssid_buf_offset = 0, pwd_buf_offset = 0, openid_buf_offset = 0, complete = 0;
static char ssid_buf[32 * 3 + 1];
static char pwd_buf[64 * 3 + 1];
static char openid_buf[33];
static int repeat_timer;

static void repeat_send(void *priv)
{
    turing_ble_cfg_net_result_notify(-1, NULL, NULL, NULL);
}

static void clear_turing_net_cfg_info(void)
{
    complete = 0;
    ssid_buf_offset = 0;
    pwd_buf_offset = 0;
    ...
    memset(openid_buf, 0, sizeof(openid_buf));
}
...
static int att_write_callback(hci_con_handle_t connection_handle, uint16_t att_handle, uint16_t transaction_mode, uint16_t offset, uint8_t *buffer, uint16_t buffer_size)
{
    ...
    case ATT_CHARACTERISTIC_FF01_01_VALUE_HANDLE:
        ...
        if (buffer[0] == 0xB0 && buffer[1] == 0x01 && buffer[2] == 0x00 && buffer[3] == 0x00 && buffer[4] == 0x4f) {
            complete = 0;
#if !CONFIG_POWER_ON_ENABLE_BLE
            ble_module_enable(0);
#endif
        ...
        if (crc8 != (u8)(~buffer[buffer_size - 1]) + 1) {
            break;
        }
        if (buffer[1] == 0x01) {
            if (buffer[4] && ssid_buf_offset + buffer[4] < sizeof(ssid_buf) - 1) {
                memcpy(ssid_buf + ssid_buf_offset, &buffer[5], buffer[4]);
                ssid_buf_offset += buffer[4];
            }
            if (buffer[2] == 0 || buffer[2] == buffer[3]) {
                complete |= BIT(0);
            }
        }
        if (buffer[1] == 0x02) {
            if (buffer[4] && pwd_buf_offset + buffer[4] < sizeof(pwd_buf) - 1) {
                memcpy(pwd_buf + pwd_buf_offset, &buffer[5], buffer[4]);
                pwd_buf_offset += buffer[4];
            }
            if (buffer[2] == 0 || buffer[2] == buffer[3]) {
                complete |= BIT(1);
            }
        ...
        if (buffer[1] == 0x03) {
            if (buffer[4] && openid_buf_offset + buffer[4] < sizeof(openid_buf) - 1) {
                memcpy(openid_buf + openid_buf_offset, &buffer[5], buffer[4]);
                openid_buf_offset += buffer[4];
            }
            if (buffer[2] == 0 || buffer[2] == buffer[3]) {
                complete |= BIT(2);
            }
        }
        if (!repeat_timer && complete != 0x07) {
            repeat_timer = sys_timer_add_to_task("app_core", NULL, repeat_send, 3000);
        }
        if (complete == 0x07) {
            if (repeat_timer) {
                sys_timer_del(repeat_timer);
                repeat_timer = 0;
            }
            app_send_user_data(ATT_CHARACTERISTIC_FF02_01_VALUE_HANDLE, rsp_cmd[0], 5, ATT_OP_AUTO_READ_CCC);
            char cfg_buf[256];
            snprintf(cfg_buf, sizeof(cfg_buf), "{\"ssid\":\"%s\", \"pass\":\"%s\"}", ssid_buf, pwd_buf);
            extern int bt_net_config_set_ssid_pwd(const char *data);
            bt_net_config_set_ssid_pwd(cfg_buf);
        }
        break;
    default:
        break;
...
static int make_set_adv_data(void)
{
    ...
    offset += make_eir_packet_val(&buf[offset], offset, HCI_EIR_DATATYPE_MORE_16BIT_SERVICE_UUIDS, 0xFFFF, 2);
...
#endif
    return app_send_user_data(ATT_CHARACTERISTIC_FF02_01_VALUE_HANDLE, (const char *)data, len, ATT_OP_AUTO_READ_CCC);
}
...
void bt_ble_init(void)
{
    ...
    if (gap_device_name_len > BT_NAME_LEN_MAX - 1) {
        gap_device_name_len = BT_NAME_LEN_MAX - 1;
    }
...
void turing_ble_cfg_net_result_notify(int succeeded, const char *mac, const char *api_key, const char *deviceid)
{
    ...
    if (succeeded == 1 && mac && api_key && deviceid) {
        //发送mac数据
        buf[1] = 0x01;
        buf[2] = 0x01;
        buf[3] = 0x00;
        ...
        buf[12] = (u8)(~buf[12]) + 1;
        app_send_user_data(ATT_CHARACTERISTIC_FF02_01_VALUE_HANDLE, buf, 13, ATT_OP_AUTO_READ_CCC);
        ...
        buf[5] = 0x0D;
        buf[19] = 0;
        memcpy(&buf[6], api_key, 13);
        for (u32 i = 0; i < 19; ++i) {
            buf[19] += buf[i];
        }
        buf[19] = (u8)(~buf[19]) + 1;
        app_send_user_data(ATT_CHARACTERISTIC_FF02_01_VALUE_HANDLE, buf, 20, ATT_OP_AUTO_READ_CCC);
        ...
        buf[5] = 0x03;
        buf[9] = 0;
        memcpy(&buf[6], &deviceid[13], 3);
        for (u32 i = 0; i < 9; ++i) {
            buf[9] += buf[i];
        }
        buf[9] = (u8)(~buf[9]) + 1;
        app_send_user_data(ATT_CHARACTERISTIC_FF02_01_VALUE_HANDLE, buf, 10, ATT_OP_AUTO_READ_CCC);
    } else if (succeeded == -1) {
        //请求重发
        if (con_handle) {
            if ((complete & BIT(0)) == 0) {
                ssid_buf_offset = 0;
                memset(ssid_buf, 0, sizeof(ssid_buf));
                app_send_user_data(ATT_CHARACTERISTIC_FF02_01_VALUE_HANDLE, rsp_cmd[1], 7, ATT_OP_AUTO_READ_CCC);
            }
            if ((complete & BIT(1)) == 0) {
                pwd_buf_offset = 0;
                memset(pwd_buf, 0, sizeof(pwd_buf));
                app_send_user_data(ATT_CHARACTERISTIC_FF02_01_VALUE_HANDLE, rsp_cmd[2], 7, ATT_OP_AUTO_READ_CCC);
            }
            if ((complete & BIT(2)) == 0) {
                openid_buf_offset = 0;
                memset(openid_buf, 0, sizeof(openid_buf));
                app_send_user_data(ATT_CHARACTERISTIC_FF02_01_VALUE_HANDLE, rsp_cmd[3], 7, ATT_OP_AUTO_READ_CCC);
            }
        }
    } else {
        buf[6] = 0x50;
        if (con_handle) {
            app_send_user_data(ATT_CHARACTERISTIC_FF02_01_VALUE_HANDLE, buf, 7, ATT_OP_AUTO_READ_CCC);
        }
        complete = 0;
#if !CONFIG_POWER_ON_ENABLE_BLE
        if (os_mutex_valid(&mutex)) {
...
{
    *len = complete == 0x07 ? openid_buf_offset : 0;
    return openid_buf;
}
