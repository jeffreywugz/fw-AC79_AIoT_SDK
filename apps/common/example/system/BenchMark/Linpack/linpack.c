...

{
    ...
    printf("Enter array size[%d] 0-> [200]\n", arsize);
...

{
    ...
    n = arsize / 2;
    arsize2d = (long)arsize * (long)arsize;
    ops = ((FLOPS_CONSTANT(2.0) * n * n * n) / FLOPS_CONSTANT(3.0) + FLOPS_CONSTANT(2.0) * n * n);
    a = (REAL *)mempool;
    b = a + arsize2d;
    ipvt = (int *)&b[arsize];
    tdgesl = 0;
    ...
    if (totalt < FLOPS_CONSTANT(0.5) || tdgefa + tdgesl < FLOPS_CONSTANT(0.2)) {
        return (0.);
    }
...

{
    ...
    init = 1325;
    *norma = FLOPS_CONSTANT(0.0);
    for (j = 0; j < n; j++)
        for (i = 0; i < n; i++) {
            init = (int)((long)3125 * (long)init % 65536L);
            a[lda * j + i] = (init - FLOPS_CONSTANT(32768.0)) / FLOPS_CONSTANT(16384.0);
            *norma = (a[lda * j + i] > *norma) ? a[lda * j + i] : *norma;
        }
    for (i = 0; i < n; i++) {
        b[i] = FLOPS_CONSTANT(0.0);
    }
    for (j = 0; j < n; j++)
        for (i = 0; i < n; i++) {
            b[i] = b[i] + a[lda * j + i];
...
{
    extern unsigned int timer_get_sec(void);
    return timer_get_sec();
