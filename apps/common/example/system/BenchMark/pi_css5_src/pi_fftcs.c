...
#endif
    npow = 4;
    elap_time = (double) timer_get_sec() - (double) start_time ;

    do {
        clock_t start_loop_time = timer_get_sec();
        npow *= 2;
        /* ---- e = (a + b) / 2 ---- */
        ...
        loop_time =
            (double) timer_get_sec() - (double) start_loop_time ;
        elap_time += loop_time;
#ifndef QUIET_OUT
        printf("precision= %d: %0.2f sec\n", 4 * nprc * log10_radix,
               loop_time);
#endif
    } while (4 * nprc <= n);
    start_time = timer_get_sec();
