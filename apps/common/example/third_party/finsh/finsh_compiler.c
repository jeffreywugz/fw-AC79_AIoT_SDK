...
static int finsh_compile(struct finsh_node *node)
{
    if (node != NULL) {
        ...
        case FINSH_NODE_SYS_ADD:
            if (node->data_type == FINSH_DATA_TYPE_BYTE) {
                finsh_code_byte(FINSH_OP_ADD_BYTE);
            } else if (node->data_type == FINSH_DATA_TYPE_WORD) {
        ...
        case FINSH_NODE_SYS_MUL:
            if (node->data_type == FINSH_DATA_TYPE_BYTE) {
                finsh_code_byte(FINSH_OP_MUL_BYTE);
            } else if (node->data_type == FINSH_DATA_TYPE_WORD) {
                finsh_code_byte(FINSH_OP_MUL_WORD);
            } else if (node->data_type == FINSH_DATA_TYPE_DWORD) {
                finsh_code_byte(FINSH_OP_MUL_DWORD);
            }
            break;

        case FINSH_NODE_SYS_DIV:
            if (node->data_type == FINSH_DATA_TYPE_BYTE) {
                finsh_code_byte(FINSH_OP_DIV_BYTE);
            } else if (node->data_type == FINSH_DATA_TYPE_WORD) {
                finsh_code_byte(FINSH_OP_DIV_WORD);
            } else if (node->data_type == FINSH_DATA_TYPE_DWORD) {
                finsh_code_byte(FINSH_OP_DIV_DWORD);
            }
            break;

        case FINSH_NODE_SYS_MOD:
            if (node->data_type == FINSH_DATA_TYPE_BYTE) {
                finsh_code_byte(FINSH_OP_MOD_BYTE);
            } else if (node->data_type == FINSH_DATA_TYPE_WORD) {
                finsh_code_byte(FINSH_OP_MOD_WORD);
            } else if (node->data_type == FINSH_DATA_TYPE_DWORD) {
                finsh_code_byte(FINSH_OP_MOD_DWORD);
            }
            break;
        ...
        case FINSH_NODE_SYS_AND:
            if (node->data_type == FINSH_DATA_TYPE_BYTE) {
                finsh_code_byte(FINSH_OP_AND_BYTE);
            } else if (node->data_type == FINSH_DATA_TYPE_WORD) {
                finsh_code_byte(FINSH_OP_AND_WORD);
            } else if (node->data_type == FINSH_DATA_TYPE_DWORD) {
                finsh_code_byte(FINSH_OP_AND_DWORD);
            }
            break;

        case FINSH_NODE_SYS_OR:
            if (node->data_type == FINSH_DATA_TYPE_BYTE) {
                finsh_code_byte(FINSH_OP_OR_BYTE);
            } else if (node->data_type == FINSH_DATA_TYPE_WORD) {
                finsh_code_byte(FINSH_OP_OR_WORD);
            } else if (node->data_type == FINSH_DATA_TYPE_DWORD) {
                finsh_code_byte(FINSH_OP_OR_DWORD);
            }
            break;

        case FINSH_NODE_SYS_XOR:
            if (node->data_type == FINSH_DATA_TYPE_BYTE) {
                finsh_code_byte(FINSH_OP_XOR_BYTE);
            } else if (node->data_type == FINSH_DATA_TYPE_WORD) {
                finsh_code_byte(FINSH_OP_XOR_WORD);
            } else if (node->data_type == FINSH_DATA_TYPE_DWORD) {
                finsh_code_byte(FINSH_OP_XOR_DWORD);
            }
            break;

        case FINSH_NODE_SYS_BITWISE:
            if (node->data_type == FINSH_DATA_TYPE_BYTE) {
                finsh_code_byte(FINSH_OP_BITWISE_BYTE);
            } else if (node->data_type == FINSH_DATA_TYPE_WORD) {
                finsh_code_byte(FINSH_OP_BITWISE_WORD);
            } else if (node->data_type == FINSH_DATA_TYPE_DWORD) {
        ...
        case FINSH_NODE_SYS_SHL:
            if (node->data_type == FINSH_DATA_TYPE_BYTE) {
                finsh_code_byte(FINSH_OP_SHL_BYTE);
            } else if (node->data_type == FINSH_DATA_TYPE_WORD) {
                finsh_code_byte(FINSH_OP_SHL_WORD);
            } else if (node->data_type == FINSH_DATA_TYPE_DWORD) {
                finsh_code_byte(FINSH_OP_SHL_DWORD);
            }
            break;

        case FINSH_NODE_SYS_SHR:
            if (node->data_type == FINSH_DATA_TYPE_BYTE) {
                finsh_code_byte(FINSH_OP_SHR_BYTE);
            } else if (node->data_type == FINSH_DATA_TYPE_WORD) {
                finsh_code_byte(FINSH_OP_SHR_WORD);
            } else if (node->data_type == FINSH_DATA_TYPE_DWORD) {
                finsh_code_byte(FINSH_OP_SHR_DWORD);
            }
            break;
