...
#include "os/os_api.h"
#include "init.h"
__attribute__((weak)) const char *finsh_get_uart(void)
{
    return "uart1";
}
static void *uart_hdl;
static u8 uart_buf[128] __attribute__((aligned(32))); //用于串口接收缓存数据的循环buf
static int finsh_uart_init(void)
{
    uart_hdl = dev_open(finsh_get_uart(), NULL);
    if (!uart_hdl) {
        printf("finsh %s init fail !!!\n", finsh_get_uart());
        return -1;
    }
    ...
    /* parm = 1000; */
    /* dev_ioctl(mp_test_uart_dev_handle, UART_SET_RECV_TIMEOUT, (u32)parm); */
...
int finsh_system_init(void)
{
    ...
    os_task_create(finsh_thread_entry, NULL, FINSH_THREAD_PRIORITY, FINSH_THREAD_STACK_SIZE, 0, FINSH_THREAD_NAME);
