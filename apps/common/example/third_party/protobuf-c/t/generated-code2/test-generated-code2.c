...
test_alloc_fail(void)
{
    ...
    SETUP_TEST_ALLOC_BUFFER(packed, len);
    do {
        test_alloc_graceful_cleanup(packed, len, i++);
...
test_message_check(void)
{
    ...
    m.required_msg = &sm;
    m.required_bytes.data = (uint8_t *)str;
    m.required_bytes.len = 1;
    assert(0 == protobuf_c_message_check(&m.base));
    ...
    m.optional_bytes.data = NULL;
    m.optional_bytes.len = 1;
    assert(0 == protobuf_c_message_check(&m.base));

    /* test with correct optional bytes */
    m.optional_bytes.data = (uint8_t *)str;
    m.optional_bytes.len = 1;
    assert(1 == protobuf_c_message_check(&m.base));
