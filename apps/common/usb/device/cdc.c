...
static void cdc_endpoint_init(struct usb_device_t *usb_device, u32 itf)
{
    ...
    usb_g_ep_config(usb_id, CDC_DATA_EP_IN | USB_DIR_IN, USB_ENDPOINT_XFER_BULK,
                    0, cdc_hdl[usb_id]->bulk_ep_in_buffer, MAXP_SIZE_CDC_BULKIN);
...
u32 cdc_write_data(const usb_dev usb_id, u8 *buf, u32 len)
{
    ...
    if (offset && !(offset % MAXP_SIZE_CDC_BULKIN)) {
...
void cdc_register(const usb_dev usb_id)
{
    ...
    if (!cdc_hdl[usb_id]) {
        ...
        cdc_hdl[usb_id]->cdc_buffer = malloc(MAXP_SIZE_CDC_BULKOUT);
        ...
        cdc_hdl[usb_id]->bulk_ep_in_buffer = usb_alloc_ep_dmabuffer(usb_id, CDC_DATA_EP_IN | USB_DIR_IN, MAXP_SIZE_CDC_BULKIN);
        cdc_hdl[usb_id]->bulk_ep_out_buffer = usb_alloc_ep_dmabuffer(usb_id, CDC_DATA_EP_OUT | USB_DIR_OUT, MAXP_SIZE_CDC_BULKOUT);

#if CDC_INTR_EP_ENABLE
        os_mutex_create(&cdc_hdl[usb_id]->mutex_intr);
        cdc_hdl[usb_id]->intr_ep_in_buffer = usb_alloc_ep_dmabuffer(usb_id, CDC_INTR_EP_IN | USB_DIR_IN, MAXP_SIZE_CDC_INTRIN);
#endif
