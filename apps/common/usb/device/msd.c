...
#elif defined(CONFIG_CPU_WL80)
#define     CPU_NAME    'W','L','8','0'
#elif defined(CONFIG_CPU_WL82)
#define     CPU_NAME    'W','L','8','2'
#else
...
struct usb_msd_handle *msd_handle[USB_MAX_HW_NUM];

#if USB_MALLOC_ENABLE
#else
#if USB_MSD_BULK_DEV_USE_ASYNC
static u8 msd_buf[USB_MAX_HW_NUM][MSD_BUFFER_SIZE * 2] SEC(.mass_storage) __attribute__((aligned(64)));
#else
static u8 msd_buf[USB_MAX_HW_NUM][MSD_BUFFER_SIZE] SEC(.mass_storage) __attribute__((aligned(64)));
#endif
static struct usb_msd_handle _msd_handle[USB_MAX_HW_NUM] SEC(.mass_storage);
#endif

inline static u8 __get_max_msd_dev(void)
{
    u8 cnt = 0;
...
u32 msd_set_wakeup_handle(const usb_dev usb_id, void (*handle)(struct usb_device_t *usb_device))
{
    if (msd_handle[usb_id]) {
        msd_handle[usb_id]->info.msd_wakeup_handle = handle;
        return 1;
    }
    return 0;
}
static void msd_wakeup(struct usb_device_t *usb_device, u32 ep)
{
    ...
    if ((msd_handle[usb_id]) && (msd_handle[usb_id]->info.msd_wakeup_handle)) {
        msd_handle[usb_id]->info.msd_wakeup_handle(usb_device);
    }
}
static u8 get_cardreader_popup(const usb_dev usb_id, u32 lun)
{
    return msd_handle[usb_id]->info.bDisk_popup[lun];//弹出
}
static void set_cardreader_popup(const usb_dev usb_id, u32 lun, u8 status)
{
    if (msd_handle[usb_id]->info.dev_handle[lun] != NULL) {
        msd_handle[usb_id]->info.bDisk_popup[lun] = status;//弹出
    }
}
static void recover_set_cardreader_popup(const usb_dev usb_id, u32 lun)
{
    if (msd_handle[usb_id]->info.dev_handle[lun] != NULL) {
        msd_handle[usb_id]->info.bDisk_popup[lun] = 0;//未弹出
    }
}
static void msd_endpoint_init(struct usb_device_t *usb_device, u32 itf)
{
    ...
    usb_g_ep_config(usb_id, MSD_BULK_EP_IN | USB_DIR_IN, USB_ENDPOINT_XFER_BULK, 0, msd_handle[usb_id]->ep_out_dmabuffer, MAXP_SIZE_BULKIN);

    usb_g_ep_config(usb_id, MSD_BULK_EP_OUT, USB_ENDPOINT_XFER_BULK, 1, msd_handle[usb_id]->ep_out_dmabuffer, MAXP_SIZE_BULKOUT);
...
{
    const usb_dev usb_id = usb_device2id(usb_device);
    if (msd_handle[usb_id] && msd_handle[usb_id]->info.msd_reset_wakeup_handle) {
        msd_handle[usb_id]->info.msd_reset_wakeup_handle(usb_device, itf_num);
    }
}
void msd_set_reset_wakeup_handle(const usb_dev usb_id, void (*handle)(struct usb_device_t *usb_device, u32 itf_num))
{
    if (msd_handle[usb_id]) {
        msd_handle[usb_id]->info.msd_reset_wakeup_handle = handle;
    }
}
...
static void *check_disk_status(const usb_dev usb_id, u8 cur_lun)
{
    u32 ret;
    u32 online_status;
    void *dev_fd = msd_handle[usb_id]->info.dev_handle[cur_lun]  ;
    const char *dev_name = msd_handle[usb_id]->info.dev_name[cur_lun];

#if TCFG_DEV_MANAGER_ENABLE
    if (dev_manager_list_check_by_logo((char *)dev_name)) {
        if (dev_fd == NULL) {
            msd_handle[usb_id]->info.dev_handle[cur_lun] = dev_open(dev_name, NULL);
            if (msd_handle[usb_id]->info.dev_handle[cur_lun]) {
#ifdef CONFIG_EARPHONE_CASE_ENABLE
                dev_ioctl(msd_handle[usb_id]->info.dev_handle[cur_lun], IOCTL_POWER_RESUME, (u32)dev_name);
#endif
            }
        } else {
        ...
        if (get_cardreader_popup(usb_id, cur_lun)) {
            return NULL;
        }
    } else {
        if (dev_fd) {
    ...
    return msd_handle[usb_id]->info.dev_handle[cur_lun];

__dev_offline:
    dev_close(dev_fd);
    recover_set_cardreader_popup(usb_id, cur_lun);
    msd_handle[usb_id]->info.dev_handle[cur_lun] = NULL;
    return NULL;
}
...
{
    const usb_dev usb_id = usb_device2id(usb_device);
    msd_handle[usb_id]->cbw.dCBWSignature = 0;
    memset((u8 *)(&(msd_handle[usb_id]->cbw)), 0, sizeof(struct usb_scsi_cbw));
    u32 rx_len = msd_usb2mcu(usb_device, (u8 *)(&(msd_handle[usb_id]->cbw)), sizeof(struct usb_scsi_cbw));
    if (rx_len == 0) {
        return 1;
    }
    if (msd_handle[usb_id]->cbw.dCBWSignature ^ CBW_SIGNATURE) {
        if ((msd_handle[usb_id]->cbw.dCBWSignature ^ CBW_SIGNATURE) != CBW_SIGNATURE) {
        }
        return 1;
    }
    msd_handle[usb_id]->csw.dCSWTag = msd_handle[usb_id]->cbw.dCBWTag;
    msd_handle[usb_id]->csw.uCSWDataResidue = msd_handle[usb_id]->cbw.dCBWDataTransferLength;
...
static void inquiry(const struct usb_device_t *usb_device)
{
    const usb_dev usb_id = usb_device2id(usb_device);
    u32 err;
    u32 len = min(msd_handle[usb_id]->csw.uCSWDataResidue, sizeof(SCSIInquiryData));
    ...
    err = msd_mcu2usb(usb_device, (u8 *)scsiinquirydata, len);
    if (err == 0) {
        log_error("write usb err %d", __LINE__);
    }
    msd_handle[usb_id]->csw.uCSWDataResidue -= len;
    msd_handle[usb_id]->csw.bCSWStatus = 0;
}
static void send_csw(const struct usb_device_t *usb_device)
{
    const usb_dev usb_id = usb_device2id(usb_device);
    u32 err;
    msd_handle[usb_id]->csw.dCSWSignature = CSW_SIGNATURE;
    err = msd_mcu2usb(usb_device, (u8 *) & (msd_handle[usb_id]->csw), sizeof(struct usb_scsi_csw));
    if (err == 0) {
        log_error("write usb err %d", __LINE__);
    }
}
static void stall_inep(const struct usb_device_t *usb_device)
{
    ...
    while (1) {
        ...
        txcsr = usb_read_txcsr(usb_id, MSD_BULK_EP_IN);
        if (txcsr & TXCSRP_SentStall) {
            usb_write_txcsr(usb_id, MSD_BULK_EP_IN, txcsr & ~(TXCSRP_SendStall | TXCSRP_SentStall));
            break;
        }
    }
}
static void stall_outep(const struct usb_device_t *usb_device)
{
    ...
    while (1) {
        ...
        rxcsr = usb_read_rxcsr(usb_id, MSD_BULK_EP_OUT);
        if (rxcsr & RXCSRP_SentStall) {
            usb_write_rxcsr(usb_id, MSD_BULK_EP_OUT, rxcsr & ~(RXCSRP_SendStall | TXCSRP_SentStall));
            break;
        }
    }
}
static void unknow(const struct usb_device_t *usb_device, u8 rw)
{
    ...
    msd_handle[usb_id]->csw.uCSWDataResidue = 0;
    msd_handle[usb_id]->csw.bCSWStatus = 0x01;
    msd_handle[usb_id]->info.bError = 0x1;
    /* msd_handle[usb_id]->info.bError = 0x2; */
}
static void stall_error(const struct usb_device_t *usb_device, u8 rw, u8 error)
{
    ...
    msd_handle[usb_id]->info.bError = error;
}
static void test_unit_ready(const struct usb_device_t *usb_device)
{
    ...
    if (NULL == check_disk_status(usb_id, cur_lun)) {
        msd_handle[usb_id]->csw.bCSWStatus = 0x1;
        msd_handle[usb_id]->info.bError = 0x0f;
    } else {
        msd_handle[usb_id]->csw.uCSWDataResidue = 0;
        msd_handle[usb_id]->csw.bCSWStatus = 0x0;
    }
}
static void allow_medium_removal(const struct usb_device_t *usb_device)
{
    const usb_dev usb_id = usb_device2id(usb_device);
    msd_handle[usb_id]->csw.bCSWStatus = 0x0;
    if (msd_handle[usb_id]->cbw.lba[2]) {
        msd_handle[usb_id]->info.bError = 0x02;
        msd_handle[usb_id]->csw.bCSWStatus = 0x01;
    }
    msd_handle[usb_id]->csw.uCSWDataResidue = 0;
}
static void request_sense(const struct usb_device_t *usb_device)
{
    ...
    u32 len;
    u8 sense_data[0x12];
    len = min(msd_handle[usb_id]->csw.uCSWDataResidue, 0x12);
    if (msd_handle[usb_id]->csw.uCSWDataResidue == 0) {
        stall_inep(usb_device);
        msd_handle[usb_id]->info.bError = NOT_READY;
        msd_handle[usb_id]->csw.uCSWDataResidue = 0;
        msd_handle[usb_id]->csw.bCSWStatus = 0x01;
        return;
    } else {
        memset(sense_data, 0x0, sizeof(sense_data));
        sense_data[2] = msd_handle[usb_id]->info.bSenseKey;
        sense_data[12] = msd_handle[usb_id]->info.bAdditionalSenseCode;
        sense_data[13] = msd_handle[usb_id]->info.bAddiSenseCodeQualifier;
        sense_data[0] = 0x70;
        sense_data[7] = 0x0a;
...
static void msd_write_10_async(const struct usb_device_t *usb_device, u8 cur_lun, u32 lba, u16 lba_num)
{
    ...
    while (lba_num) {
        ...
        if (msd_handle[usb_id]->csw.uCSWDataResidue >= num * 0x200) {
            msd_handle[usb_id]->csw.uCSWDataResidue -= num * 0x200;
        }
#if 0
        err = msd_usb2mcu_64byte_fast(usb_device, msd_handle[usb_id]->msd_buf + buf_idx * MSD_BUFFER_SIZE, num * 0x200);
#else
        err = msd_usb2mcu(usb_device, msd_handle[usb_id]->msd_buf + buf_idx * MSD_BUFFER_SIZE, num * 0x200);
#endif
        if (err != num * 0x200) {
            log_error("read usb_err %d, dev = %s",
                      __LINE__, msd_handle[usb_id]->info.dev_name[cur_lun]);
        ...
        if (dev_fd) {
            ...
            log_error("write_10 disk offline, dev = %s",
                      msd_handle[usb_id]->info.dev_name[cur_lun]);
            stall_error(usb_device, 1, MEDIUM_ERROR);
            have_send_stall = TRUE;
            break;
        }
...
static void msd_read_10_async(const struct usb_device_t *usb_device, u8 cur_lun, u32 lba, u16 lba_num)
{
    ...
    while (lba_num) {
        ...
        if (msd_handle[usb_id]->csw.uCSWDataResidue >= last_num * 0x200) {
            msd_handle[usb_id]->csw.uCSWDataResidue -= last_num * 0x200;
        }
        dev_fd = check_disk_status(usb_id, cur_lun);
        if (dev_fd) {
            if (num) {
        ...
        /* u8 *p = msd_handle[usb_id]->msd_buf + !buf_idx * MSD_BUFFER_SIZE; */
        /* for (int i = 0; i < last_num * 512; i++) {                */
        /*     p[i] = i;                                             */
        /* }                                                         */

        err = msd_mcu2usb(usb_device, msd_handle[usb_id]->msd_buf + !buf_idx * MSD_BUFFER_SIZE, last_num * 0x200);
...
static void write_10(const struct usb_device_t *usb_device)
{
    ...
    u16 lba_num;
    u16 num = 0;
    lba = read_32(msd_handle[usb_id]->cbw.lba);
...
#if 0
        err = msd_usb2mcu_64byte_fast(usb_device, msd_handle[usb_id]->msd_buf, num * 0x200);
#else
        err = msd_usb2mcu(usb_device, msd_handle[usb_id]->msd_buf, num * 0x200);
#endif
        if (err == 0) {
            stall_error(usb_device, 1, MEDIUM_ERROR);
            log_error("read usb, dev = %s", err, msd_handle[usb_id]->info.dev_name[cur_lun]);
            break;
        }
        dev_fd = check_disk_status(usb_id, cur_lun);
        if (dev_fd) {
            ...
            log_error("write disk offline, dev = %s", msd_handle[usb_id]->info.dev_name[cur_lun]);
            stall_error(usb_device, 1, MEDIUM_ERROR);
            break;
        }
    }
...
static void read_10(const struct usb_device_t *usb_device)
{
    ...
    while (lba_num) {
        ...
        if (dev_fd) {
            ...
            log_error("read disk offline, dev = %s", err, msd_handle[usb_id]->info.dev_name[cur_lun]);
            stall_error(usb_device, 0, MEDIUM_ERROR);
            break;
        }
        err = msd_mcu2usb(usb_device, msd_handle[usb_id]->msd_buf, num * 0x200);
        if (err == 0) {
            log_error("write usb err %d, dev = %s", __LINE__, msd_handle[usb_id]->info.dev_name[cur_lun]);
            stall_error(usb_device, 0, MEDIUM_ERROR);
            break;
        }
        lba += num;
...
static void read_format_capacity(const struct usb_device_t *usb_device)
{
    ...
    msd_handle[usb_id]->csw.uCSWDataResidue -= 12;
    msd_handle[usb_id]->csw.bCSWStatus = 0;
}
static void stop_start(const usb_dev usb_id)
{
    int i;
    u8 loej, start;
    /* set_cardreader_popup(0, !(msd_handle[usb_id]->cbw.lba[2])); */
    loej = msd_handle[usb_id]->cbw.lba[2] & BIT(1);
    start = msd_handle[usb_id]->cbw.lba[2] & BIT(0);
    for (i = 0; i < __get_max_msd_dev(); i++) {
        if (loej && !start) {
            set_cardreader_popup(usb_id, i, 1);
        } else if (loej && start) {
            set_cardreader_popup(usb_id, i, 0);
        }
    }
...
void USB_MassStorage(const struct usb_device_t *usb_device)
{
    ...
    /* log_debug("opcode %x", msd_handle[usb_id]->cbw.operationCode); */
    if (private_scsi_cmd(usb_device, &(msd_handle[usb_id]->cbw))) {
        msd_handle[usb_id]->info.bError = 0;
        msd_handle[usb_id]->csw.uCSWDataResidue = 0;
        msd_handle[usb_id]->csw.bCSWStatus = 0x0;
    } else {
        wdt_clear();
    ...
    switch (msd_handle[usb_id]->info.bError) {
    case 0x00:
        msd_handle[usb_id]->info.bSenseKey = NO_SENSE;
        msd_handle[usb_id]->info.bAdditionalSenseCode = ASC_NO_ADDITIONAL_SENSE_INFORMATION;
        msd_handle[usb_id]->info.bAddiSenseCodeQualifier = ASCQ_NO_ADDITIONAL_SENSE_INFORMATION;
        break;

    case 0x02:
        msd_handle[usb_id]->info.bSenseKey = ILLEGAL_REQUEST;
        msd_handle[usb_id]->info.bAdditionalSenseCode = ASC_INVALID_FIELD_IN_COMMAND_PACKET;
        msd_handle[usb_id]->info.bAddiSenseCodeQualifier = ASCQ_INVALID_FIELD_IN_COMMAND_PACKET;
        break;
    case MEDIUM_ERROR:
        msd_handle[usb_id]->info.bSenseKey = MEDIUM_ERROR;
        msd_handle[usb_id]->info.bAdditionalSenseCode = ASC_WRITE_FAULT;
        msd_handle[usb_id]->info.bAddiSenseCodeQualifier = ASCQ_WRITE_FAULT;
        break;


    case 0x0f:
    case 0x05:
        msd_handle[usb_id]->info.bSenseKey = NOT_READY;
        msd_handle[usb_id]->info.bAdditionalSenseCode = ASC_MEDIUM_PRESENT;
        msd_handle[usb_id]->info.bAddiSenseCodeQualifier = ASCQ_MEDIUM_PRESENT;
        break;
    case 0x06:
        msd_handle[usb_id]->info.bSenseKey = ABORTED_COMMAND;
        msd_handle[usb_id]->info.bAdditionalSenseCode = ASC_INVALID_COMMAND_OPERATION_CODE;
        msd_handle[usb_id]->info.bAddiSenseCodeQualifier = ASCQ_INVALID_COMMAND_OPERATION_CODE;
        break;
    }
    usb_set_intr_rxe(usb_id, MSD_BULK_EP_OUT);
    send_csw(usb_device);
...
u32 msd_register_disk(const usb_dev usb_id, const char *name, void *arg)
{
    int i;
    u32 len, ret = -1;
    if (msd_handle[usb_id]) {
        for (i = 0; i < __get_max_msd_dev(); i++) {
            if (msd_handle[usb_id]->info.dev_name[i][0] == 0) {
                ASSERT(strlen(name) <= MSD_DEV_NAME_LEN, "MSD_DEV_NAME_LEN too small");
                strcpy(msd_handle[usb_id]->info.dev_name[i], name);
                msd_handle[usb_id]->info.dev_handle[i] = NULL;
                break;
            }
        }
...
u32 msd_unregister_disk(const usb_dev usb_id, const char *name)
{
    u32 err;
    int i;
    if (msd_handle[usb_id]) {
        for (i = 0; i < __get_max_msd_dev(); i++) {
            if (!strcmp(msd_handle[usb_id]->info.dev_name[i], name)) {
                msd_handle[usb_id]->info.dev_name[i][0] = 0;
                return 0;
            }
        }
    }
...
u32 msd_unregister_all(const usb_dev usb_id)
{
    u32 err;
    int i;
    if (msd_handle[usb_id]) {
        for (i = 0; i < __get_max_msd_dev(); i++) {
            if (msd_handle[usb_id]->info.dev_handle[i]) {
                err = dev_close(msd_handle[usb_id]->info.dev_handle[i]);
                msd_handle[usb_id]->info.dev_handle[i] = NULL;
                /* memset(msd_handle[usb_id]->info.dev_name[i], 0, MSD_DEV_NAME_LEN); */
            }
        }
    }
    return 0;
...
{
    if (msd_handle[usb_id] == NULL) {
#if USB_MALLOC_ENABLE
        msd_handle[usb_id] = (struct usb_msd_handle *)zalloc(sizeof(struct usb_msd_handle));
        if (msd_handle[usb_id] == NULL) {
            log_error("msd_register err");
            return -1;
        }
#if USB_MSD_BULK_DEV_USE_ASYNC
        msd_handle[usb_id]->msd_buf = (u8 *)malloc(MSD_BUFFER_SIZE * 2);
#else
        msd_handle[usb_id]->msd_buf = (u8 *)malloc(MSD_BUFFER_SIZE);
#endif
        if (msd_handle[usb_id]->msd_buf == NULL) {
            free(msd_handle[usb_id]);
            msd_handle[usb_id] = NULL;
            log_error("msd_register err");
            return -1;
        }
#else
        memset(&_msd_handle[usb_id], 0, sizeof(struct usb_msd_handle));
        _msd_handle[usb_id].msd_buf = msd_buf;
        msd_handle[usb_id] = &_msd_handle[usb_id];
#endif
        log_info("msd_handle[usb_id] = %x", msd_handle[usb_id]);

        msd_handle[usb_id]->ep_out_dmabuffer = usb_alloc_ep_dmabuffer(usb_id, MSD_BULK_EP_OUT, MAXP_SIZE_BULKIN + MAXP_SIZE_BULKOUT);

    }
    return 0;
}
u32 msd_release(const usb_dev usb_id)
{
    if (msd_handle[usb_id]) {
        for (int i = 0; i < __get_max_msd_dev(); i++) {
            void *dev_fd = msd_handle[usb_id]->info.dev_handle[i] ;
            if (dev_fd) {
#ifdef CONFIG_EARPHONE_CASE_ENABLE
        ...
        if (msd_handle[usb_id]->ep_out_dmabuffer) {
            usb_free_ep_dmabuffer(usb_id, msd_handle[usb_id]->ep_out_dmabuffer);
            msd_handle[usb_id]->ep_out_dmabuffer = NULL;
        }
#if USB_MALLOC_ENABLE
        if (msd_handle[usb_id]->msd_buf) {
            free(msd_handle[usb_id]->msd_buf);
            msd_handle[usb_id]->msd_buf = NULL;
        }
        free(msd_handle[usb_id]);
#endif
        msd_handle[usb_id] = NULL;
