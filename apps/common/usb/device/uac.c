...
//Isochronous,Synchronization Type(Asynchronous)
    ...
    UAC_ISO_INTERVAL_1_0,//one packet per frame 0x00,  //Interval    //Asynchronous
    0x00,//unused
    0x00,//unused
...
//Isochronous,Synchronization Type(Asynchronous)
    ...
    UAC_ISO_INTERVAL_1_0,//one packet per frame   //Interval
    0x00,//unused
    0x00,//unused
...
struct uac_info_t *uac_info[USB_MAX_HW_NUM];

#if USB_MALLOC_ENABLE
#else
struct uac_info_t _uac_info[USB_MAX_HW_NUM] SEC(.uac_var);
#endif
...
void uac_get_cur_vol(const usb_dev usb_id, u16 *l_vol, u16 *r_vol)
{
    ...
    if (uac_info[usb_id]) {
        if (!uac_info[usb_id]->spk_mute) {
            if (l_vol) {
                *l_vol = vol_convert(uac_info[usb_id]->spk_left_vol);
            }
            if (r_vol) {
                *r_vol = vol_convert(uac_info[usb_id]->spk_right_vol);
            }
        }
    }
}
...
{
    if (uac_info[usb_id]) {
        if (!uac_info[usb_id]->mic_mute) {
            return vol_convert(uac_info[usb_id]->mic_vol);
        }
    }
    return 0;
}

u32 uac_get_mic_sameplerate(void *priv)
{
    return mic_samplingfrequency;
}
...
static u32 uac_vol_handler(struct usb_device_t *usb_device, struct usb_ctrlrequest *setup)
{
    ...
    case UAC_FU_MUTE:
        if (HIBYTE(setup->wIndex) == SPK_FEATURE_UNIT_ID) {
            ...
            if (mute) {
                ...
                uac_info[usb_id]->spk_mute = 1;
            } else {
                uac_mute_volume(usb_id, SPK_FEATURE_UNIT_ID,
                                vol_convert(uac_info[usb_id]->spk_left_vol),
                                vol_convert(uac_info[usb_id]->spk_right_vol));
                uac_info[usb_id]->spk_mute = 0;
            }
        } else if (HIBYTE(setup->wIndex) == MIC_FEATURE_UNIT_ID) {
            memcpy((u8 *)&mute, read_ep, 1);
            if (mute) {
                uac_mute_volume(usb_id, MIC_FEATURE_UNIT_ID, 0, 0);
                uac_info[usb_id]->mic_mute = 1;
            } else {
                uac_mute_volume(usb_id, MIC_FEATURE_UNIT_ID, vol_convert(uac_info[usb_id]->mic_vol), 0);
                uac_info[usb_id]->mic_mute = 0;
            }
        }
        break;
    case UAC_FU_VOLUME:
        if (HIBYTE(setup->wIndex) == SPK_FEATURE_UNIT_ID) {
            ...
            if (LOBYTE(setup->wValue) == 1) {
                uac_info[usb_id]->spk_left_vol = volume;
            } else if (LOBYTE(setup->wValue) == 2) {
                uac_info[usb_id]->spk_right_vol = volume;
            }
            if (uac_info[usb_id]->ps4_mode) {
                uac_info[usb_id]->spk_left_vol = vol_convert(100);
                uac_info[usb_id]->spk_right_vol = vol_convert(100);
            }
            if (!uac_info[usb_id]->spk_mute && !uac_info[usb_id]->ps4_mode) {
                uac_mute_volume(usb_id, SPK_FEATURE_UNIT_ID,
                                vol_convert(uac_info[usb_id]->spk_left_vol),
                                vol_convert(uac_info[usb_id]->spk_right_vol));
            }
        } else if (HIBYTE(setup->wIndex) == MIC_FEATURE_UNIT_ID) {
            memcpy((u8 *)&uac_info[usb_id]->mic_vol, read_ep, sizeof(uac_info[usb_id]->mic_vol));
            /* if (uac_info[usb_id]->mic_vol == 0x8000) { */
            /* uac_info[usb_id]->mic_vol = 0; */
            /* } else if (uac_info[usb_id]->mic_vol >= 100) { */
            /* uac_info[usb_id]->mic_vol = 99; */
            /* } */
            if (!uac_info[usb_id]->mic_mute) {
                uac_mute_volume(usb_id, MIC_FEATURE_UNIT_ID, vol_convert(uac_info[usb_id]->mic_vol), 0);
            }
    ...
    case UAC_FU_AUTOMATIC_GAIN:
        if (read_ep[0]) {
            uac_info[usb_id]->bAGC = 1;
        } else {
            uac_info[usb_id]->bAGC = 0;
        }
        break;

    case 0: //bSelectorUnitID
        if (HIBYTE(setup->wIndex) == SPK_SELECTOR_UNIT_ID) {
            uac_info[usb_id]->ps4_mode = read_ep[0];
            if (read_ep[0]) {
...
#if USB_ROOT2
static u8 SetInterface_0_Lock[USB_MAX_HW_NUM];
#endif
...
static u32 audio_ac_itf_handler(struct usb_device_t *usb_device, struct usb_ctrlrequest *setup)
{
    ...
    case UAC_GET_CUR:
        ...
        case UAC_FU_MUTE:
            if (HIBYTE(setup->wIndex) == SPK_FEATURE_UNIT_ID) {
                mute = uac_info[usb_id]->spk_mute ;
            } else if (HIBYTE(setup->wIndex) == MIC_FEATURE_UNIT_ID) {
                mute = uac_info[usb_id]->mic_mute ;
            }
            tx_payload = &mute;
        ...
        case UAC_FU_VOLUME:
            if (HIBYTE(setup->wIndex) == SPK_FEATURE_UNIT_ID) {
                if (LOBYTE(setup->wValue) == 1) {
                    tx_payload = (u8 *)&uac_info[usb_id]->spk_left_vol;
                } else if (LOBYTE(setup->wValue) == 2) {
                    tx_payload = (u8 *)&uac_info[usb_id]->spk_right_vol;
                }
            } else if (HIBYTE(setup->wIndex) == MIC_FEATURE_UNIT_ID) {
                tx_payload = (u8 *)&uac_info[usb_id]->mic_vol;
            }
            tx_len = setup->wLength;
            usb_set_data_payload(usb_device, setup, tx_payload, tx_len);
            break;
        case UAC_FU_AUTOMATIC_GAIN:
            if (uac_info[usb_id]->bAGC) {
                tx_payload[0] = 0x01;
            } else {
                tx_payload[0] = 0x00;
            }
    ...
    case UAC_GET_DEF:
        if (HIBYTE(setup->wIndex) == SPK_FEATURE_UNIT_ID) {
            tx_payload = (u8 *)&uac_info[usb_id]->spk_def_vol;
        } else if (HIBYTE(setup->wIndex) == MIC_FEATURE_UNIT_ID) {
            tx_payload = (u8 *)&uac_info[usb_id]->mic_def_vol;
        }
        tx_len = setup->wLength;
        usb_set_data_payload(usb_device, setup, tx_payload, tx_len);
        break;
    case UAC_GET_MAX:
        if (HIBYTE(setup->wIndex) == SPK_FEATURE_UNIT_ID) {
            tx_payload = (u8 *)&uac_info[usb_id]->spk_max_vol;
        } else if (HIBYTE(setup->wIndex) == MIC_FEATURE_UNIT_ID) {
            tx_payload = (u8 *)&uac_info[usb_id]->mic_max_vol;
        }
        tx_len = setup->wLength;
        usb_set_data_payload(usb_device, setup, tx_payload, tx_len);
        break;
    case UAC_GET_MIN:
        if (HIBYTE(setup->wIndex) == SPK_FEATURE_UNIT_ID) {
            tx_payload = (u8 *)&uac_info[usb_id]->spk_min_vol;
        } else if (HIBYTE(setup->wIndex) == MIC_FEATURE_UNIT_ID) {
            tx_payload = (u8 *)&uac_info[usb_id]->mic_min_vol;
        }
        tx_len = setup->wLength;
        usb_set_data_payload(usb_device, setup, tx_payload, tx_len);
        break;
    case UAC_GET_RES:
        if (HIBYTE(setup->wIndex) == SPK_FEATURE_UNIT_ID) {
            tx_payload = (u8 *)&uac_info[usb_id]->spk_vol_res;
        } else if (HIBYTE(setup->wIndex) == MIC_FEATURE_UNIT_ID) {
            tx_payload = (u8 *)&uac_info[usb_id]->mic_vol_res;
        }
        tx_len = setup->wLength;
        usb_set_data_payload(usb_device, setup, tx_payload, tx_len);
        break;
...
{
    const usb_dev usb_id = usb_device2id(usb_device);
    u8 *ep_buffer = uac_info[usb_id]->spk_dma_buffer;
...
static void open_spk(struct usb_device_t *usb_device)
{
    ...
    spk_frame_len += (SPK_AUDIO_RATE % 1000 ? (SPK_AUDIO_RES / 8) * SPK_CHANNEL : 0);
    u8 *ep_buffer = uac_info[usb_id]->spk_dma_buffer;
    uac_speaker_stream_open(usb_id, SPK_AUDIO_RATE, SPK_CHANNEL);
...
static void close_spk(struct usb_device_t *usb_device)
{
    ...
    uac_speaker_stream_close(usb_id);
}
static u8 spk_itf_status[USB_MAX_HW_NUM];
static u32 spk_as_itf_hander(struct usb_device_t *usb_device, struct usb_ctrlrequest *setup)
{
    ...
    case USB_REQ_SET_INTERFACE:
        if (usb_device->bDeviceStates == USB_DEFAULT) {
            ...
            if (SetInterface_0_Lock[usb_id] == 0) {
                usb_set_setup_phase(usb_device, USB_EP0_SET_STALL);
            } else {
                usb_set_setup_phase(usb_device, USB_EP0_STAGE_SETUP);
            ...
            if (setup->wValue == 1) { //alt 1
                open_spk(usb_device);
                spk_itf_status[usb_id] = 1;
            } else if (setup->wValue == 0) { //alt 0
                close_spk(usb_device);
                spk_itf_status[usb_id] = 0;
            } else {
                usb_set_setup_phase(usb_device, USB_EP0_SET_STALL);
            }
#endif
    ...
    case USB_REQ_GET_INTERFACE:
        if (usb_root2_testing()) {
            ...
            if (setup->wValue || (setup->wLength != 1)) {
                ...
                tx_len = 1;
                tx_payload[0] = spk_itf_status[usb_id];
                usb_set_data_payload(usb_device, setup, tx_payload, tx_len);
            }
        }
        break;
...
static u8 mic_no_data[USB_MAX_HW_NUM];
static void mic_transfer(struct usb_device_t *usb_device, u32 ep)
{
    const usb_dev usb_id = usb_device2id(usb_device);
    u8 *ep_buffer = uac_info[usb_id]->mic_dma_buffer;
    ...
    if (len) {
        mic_no_data[usb_id] = 0;
    } else if (mic_no_data[usb_id]) {
        len = mic_frame_len;
...
static void open_mic(struct usb_device_t *usb_device)
{
    ...
    uac_mic_stream_open(usb_id, mic_samplingfrequency, mic_frame_len, MIC_CHANNEL);
...
static void close_mic(struct usb_device_t *usb_device)
{
    ...
    uac_mic_stream_close(usb_id);
}
static u8 mic_itf_status[USB_MAX_HW_NUM];
static u32 mic_as_itf_hander(struct usb_device_t *usb_device, struct usb_ctrlrequest *setup)
{
    ...
    case USB_REQ_SET_INTERFACE:
        if (usb_device->bDeviceStates == USB_DEFAULT) {
            ...
            if (setup->wValue == 1) {//alt 1
                open_mic(usb_device);
                mic_itf_status[usb_id] = 1;
            } else if (setup->wValue == 0) { //alt 0
                close_mic(usb_device);
                mic_itf_status[usb_id] = 0;
            } else {
                usb_set_setup_phase(usb_device, USB_EP0_SET_STALL);
            }
#endif
    ...
    case USB_REQ_GET_INTERFACE:
        if (usb_root2_testing()) {
            ...
            if (setup->wValue || (setup->wLength != 1)) {
                ...
                tx_len = 1;
                tx_payload[0] = mic_itf_status[usb_id];
                usb_set_data_payload(usb_device, setup, tx_payload, tx_len);
            }
        }
        break;
...
#else
    u8 *ep_buffer;
    ep_buffer = uac_info[usb_id]->spk_dma_buffer;
    usb_g_ep_config(usb_id, SPK_ISO_EP_OUT | USB_DIR_OUT, USB_ENDPOINT_XFER_ISOC, 1, ep_buffer, SPK_FRAME_LEN);

    ep_buffer = uac_info[usb_id]->mic_dma_buffer;
    usb_g_ep_config(usb_id, MIC_ISO_EP_IN | USB_DIR_IN, USB_ENDPOINT_XFER_ISOC, 1, ep_buffer, MIC_FRAME_LEN);
#endif
}
...
{
    if (uac_info[usb_id] == NULL) {
#if USB_MALLOC_ENABLE
        uac_info[usb_id] = (struct uac_info_t *)malloc(sizeof(struct uac_info_t));
        if (uac_info[usb_id] == NULL) {
            return -1;
        }
#else
        memset(&_uac_info, 0, sizeof(struct uac_info_t));
        uac_info[usb_id] = &_uac_info[usb_id];
#endif
        uac_info[usb_id]->spk_dma_buffer = usb_alloc_ep_dmabuffer(usb_id, SPK_ISO_EP_OUT, SPK_FRAME_LEN);
        uac_info[usb_id]->mic_dma_buffer = usb_alloc_ep_dmabuffer(usb_id, MIC_ISO_EP_IN | USB_DIR_IN, MIC_FRAME_LEN);
    }
    uac_info[usb_id]->spk_def_vol = vol_convert(uac_get_spk_vol());
    uac_info[usb_id]->spk_left_vol = uac_info[usb_id]->spk_def_vol;
    uac_info[usb_id]->spk_right_vol = uac_info[usb_id]->spk_def_vol;
    uac_info[usb_id]->spk_max_vol = vol_convert(100);
    uac_info[usb_id]->spk_min_vol = vol_convert(0);
    uac_info[usb_id]->spk_vol_res = 0x30;
    uac_info[usb_id]->spk_mute = 0;

    uac_info[usb_id]->mic_max_vol = vol_convert(100);
    uac_info[usb_id]->mic_min_vol = vol_convert(0);
    uac_info[usb_id]->mic_vol_res = 0x30;
    uac_info[usb_id]->mic_def_vol = vol_convert(13 / 14.0 * 100);
    uac_info[usb_id]->mic_vol = uac_info[usb_id]->mic_def_vol;
    uac_info[usb_id]->mic_mute = 0;
    uac_info[usb_id]->ps4_mode = 0;
    return 0;
}
void uac_release(const usb_dev usb_id)
{
    ...
    if (uac_info[usb_id]) {
        close_spk(usb_device);
        close_mic(usb_device);
        if (uac_info[usb_id]->spk_dma_buffer) {
            usb_free_ep_dmabuffer(usb_id, uac_info[usb_id]->spk_dma_buffer);
            uac_info[usb_id]->spk_dma_buffer = NULL;
        }
        if (uac_info[usb_id]->mic_dma_buffer) {
            usb_free_ep_dmabuffer(usb_id, uac_info[usb_id]->mic_dma_buffer);
            uac_info[usb_id]->mic_dma_buffer = NULL;
        }
#if USB_MALLOC_ENABLE
        free(uac_info[usb_id]);
#endif
