    ...
    for (u8 i = 0; i < MAX_HOST_INTERFACE; i++) {
        ...
        if (usb_if &&
            ...
            for (int i = 0; i < usb_if->dev.hid->bNumEndpoints; i++) {
                ...
                if (!hid_intr_ep_in_buf[usb_id]) {
                    hid_intr_ep_in_buf[usb_id] = usb_h_alloc_ep_buffer(id, host_ep | USB_DIR_IN, 64);
                }
                usb_h_ep_config(id,  host_ep | USB_DIR_IN, USB_ENDPOINT_XFER_INT, 1,
                                interval[id][target_ep], hid_intr_ep_in_buf[usb_id], 64);
