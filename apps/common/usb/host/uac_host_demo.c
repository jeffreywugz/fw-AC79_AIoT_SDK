...
int usb_audio_play_put_buf(void *ptr, u32 len)
{
    ...
    if (!play_file) {
        play_file = fopen(CONFIG_ROOT_PATH"raw.pcm", "r"); //单声道
...
int usb_audio_record_get_buf(void *ptr, u32 len)
{
    ...
    static u32 cnt = 0;
    if (!record_file) {
        record_file = fopen(CONFIG_ROOT_PATH"record01.pcm", "w+");
        cnt = 0;
        if (!record_file) {
            log_e("fopen play file faild!\n");
            return -1;
    ...
    putchar('W');
    ret = fwrite(ptr, len, 1, record_file);
    if (ret != len) {
