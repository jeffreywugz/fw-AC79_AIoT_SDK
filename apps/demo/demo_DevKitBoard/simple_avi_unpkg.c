...
#define UNPKG_AVI_ERR() do {\
        printf("\n\nmake sure avi_playback_unpkg_init/avi_preview_unpkg_init func is used before use this function ,"\
        "and use avi_unpkg_exit func int the end!!!!\n\n");}while(0);
...
int avi_playback_unpkg_init(FILE *fd, u8 state)//state : 0 preview , 1 playback
{
    if (fd) {
...
int avi_unpkg_decoder_to_yuv_example(void)
{
    ...
    ret = avi_playback_unpkg_init(fd, 1); //解码初始化,最多10分钟视频
    if (ret) {
