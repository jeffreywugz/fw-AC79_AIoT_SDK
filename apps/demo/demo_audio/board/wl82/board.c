...
{
    //重映射PDM DAT-PH0   PDM CLK-PH1
    extern int gpio_plnk_rx_input(u32 gpio, u8 index, u8 data_sel);
    gpio_plnk_rx_input(IO_PORTH_00, 0, 0);
    gpio_output_channle(IO_PORTH_01, CH0_PLNK0_SCLK_OUT);	//SCLK0使用outputchannel0
    JL_IOMAP->CON3 |= BIT(18);
}
static void plnk0_port_unremap_cb(void)
{
    JL_IOMAP->CON3 &= ~BIT(18);
    gpio_clear_output_channle(IO_PORTH_01, CH0_PLNK0_SCLK_OUT);
    gpio_set_die(IO_PORTH_00, 0);
}
...
static const struct plnk_platform_data plnk0_data = {
    ...
    .high_gain = 1, //满增益输出
    .dc_cancelling_filter = 14, //直流滤波器
    .dump_points_num = 640, //丢弃刚打开硬件时的数据点数
};
static const struct spdif_platform_data spdif_data = {
    .spdif_port = SPDIF_IN_PORT_D,
    .width = 16,
    .width_24_to_16 = 0,
};
...
{
    /* gpio_direction_output(IO_PORTH_07, 1); //PWR控制IO */
