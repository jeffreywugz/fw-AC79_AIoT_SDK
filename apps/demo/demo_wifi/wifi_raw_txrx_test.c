#include "app_config.h"
#include "system/includes.h"
#include "wifi/wifi_connect.h"
#include "lwip.h"
#include "dhcp_srv/dhcp_srv.h"
#include "event/net_event.h"

static int wifi_event_callback(void *network_ctx, enum WIFI_EVENT event)
{
    switch (event) {

    case WIFI_EVENT_MODULE_INIT:


        struct wifi_store_info wifi_default_mode_parm;
        memset(&wifi_default_mode_parm, 0, sizeof(struct wifi_store_info));
        wifi_default_mode_parm.mode = SMP_CFG_MODE;
        wifi_set_default_mode(&wifi_default_mode_parm, 1, 0);
        break;
    default:
        break;
    }

    return 0;
}

__attribute__((aligned(4))) static u8 wifi_udp_pkg[1560] = {
    
};




#define ROLE_TX 0
#define ROLE_RX 1
#define ROLE  0


static u32 time_hdl;
static u32 total_send_bytes;
static u32 recv_cnt, total_cnt, retran_cnt, error_cnt, disorder_cnt;

__attribute__((always_inline)) int wifi_recv_pkg_and_soft_filter(u8 *buf, u32 len)
{

#if ROLE == ROLE_TX
    return -1;
#endif

    u32 *magic = &buf[88];
    if (*magic == 0x12345679) {
        u32 *cnt = &buf[92];
        u16 *crcc = &buf[84];
//            if(*crcc==CRC16(cnt,1560-100))
        {
            if (buf[21]&BIT(3)) { //重传帧
                ++retran_cnt;
            } else {
                if (*cnt > total_cnt) {
                    total_cnt = *cnt;
                    ++recv_cnt;
                } else if (*cnt < total_cnt) {
                    ++disorder_cnt;
                    printf("####^*^###-> %s, %d \r\n", __FUNCTION__, __LINE__);
                }
//                    else
//                        printf("####^*^###-> %s, %d \r\n",__FUNCTION__,__LINE__);
            }
        }
//            else
//            {
//                ++error_cnt;
//                printf("####^*^###-> %s, %d \r\n",__FUNCTION__,__LINE__);
//            }
    }


    return -1;
}


static void wifi_raw_tx_rx_task(void *priv)
{
    wifi_set_event_callback(wifi_event_callback);

    wifi_set_smp_cfg_just_monitor_mode(1);

    wifi_on();

#if ROLE == ROLE_TX
    /* wifi_set_pwr(0); //把WIFI模拟功率调整到最低档位节电 */

    wifi_set_long_retry(0);
    wifi_set_short_retry(0);

    /*u8 dest_macc[6]={0x0c,0x62,0x0e,0xd4,0x46,0x3e};*/

    /*u8 local_mac[6];*/
    /*wifi_get_mac(local_mac);*/

//bssid
    /*memcpy(&wifi_udp_pkg[24],dest_macc,6);*/

//source
    /*memcpy(&wifi_udp_pkg[30],local_mac,6);*/

//dest
    /*memcpy(&wifi_udp_pkg[36],local_mac,6);*/

    u16 *crcc = &wifi_udp_pkg[84];

    u32 *maigc = &wifi_udp_pkg[88];
    *maigc = 0x12345679;

    u32 *send_cnt = &wifi_udp_pkg[92];
    *send_cnt = 0;

#endif


#if  1//ROLE == ROLE_RX
    extern void *usb_get_adapter(void);
    extern void AsicSetBssid(void *pAd, u8 * pBssid);
    u8 bssid[6] = {0x24, 0x8B, 0xA7, 0x24, 0x63, 0x17};
    AsicSetBssid(usb_get_adapter(), bssid);

    wifi_rxfilter_cfg(7);
#endif

    wifi_set_channel(11);


#if ROLE == ROLE_TX
    while (1) {

        if (time_lapse(&time_hdl, 4)) {

            *send_cnt += 1;

            *crcc = CRC16(send_cnt, 1560 - 100);

            wifi_send_data(wifi_udp_pkg, sizeof(wifi_udp_pkg), WIFI_TXRATE_11M);
            total_send_bytes += sizeof(wifi_udp_pkg);
        }

    }
#endif


#if ROLE == ROLE_RX
    while (1) {
        os_time_dly(100);

        if (recv_cnt) {
            printf("cnt = %d , %d, %d%%\r\n", recv_cnt, total_cnt, (total_cnt - recv_cnt) * 100 / total_cnt);
        }

    }
#endif

}


static int c_main(void)
{
    printf("\r\n\r\n\r\n\r\n -------- wifi_raw_send_test run -------------\r\n\r\n\r\n\r\n\r\n");

    os_task_create(wifi_raw_tx_rx_task, NULL, 2, 1000, 0, "wifi_raw_tx_rx_task");

    while (1) {
        os_time_dly(65530);
    }
}
module_initcall(c_main);

