...
static void hid_keyboard_send_onekey(u8 key)
{
    ...
    if ((ascii2kb_remap[key] & 0xff) == (ascii2kb_remap[last_key] & 0xff)) {
        ...
        if ((buf[0] & 0x02) == 0) {
            ...
            buf[2] = 0x00;
            hid_send_buf_async(__this->usb_id, buf, 8);
            //hid_tx_data(__this->usb_id, buf, 8);
        }
    }
    ...
    if ((buf[0] & 0x02) && buf[2] != 0) {
        u8 tmpbuf[8] = {0x02};
        //hid_send_buf_async(__this->usb_id, buf, 8);
        hid_tx_data(__this->usb_id, tmpbuf, 8);
    }
    //hid_send_buf_async(__this->usb_id, buf, 8);
    hid_tx_data(__this->usb_id, buf, 8);
...
static void hid_keyboard_send_wc_gbk(u16 wchar)
{
    ...
    //printf("%c%c, %d, %s\n", ((u8 *)&wchar)[1], ((u8 *)&wchar)[0], wchar, numeric_str);
    p = numeric_str;
    while (*p != 0) {
        buf[0] = BIT(2);  //left alt
