...
extern int yuv420_block_scan(void *info);
...
int video1_width_get(void)
{
    return 240;
}
int video1_height_get(void)
{
    return 320;
}
static int net_video1_event(void *priv, int argc, int *argv)
{
    switch (argv[0]) {
    case VIDEO_SERVER_PKG_ERR:
        net_video1_close();
        log_e("net_video1_close err\n");
        break;
    default:
        break;
    }
...
int net_video1_open(void *net_priv)
{
    ...
    if (__this_net->net_video_rec) {
        ...
        server_register_event_handler(__this_net->net_video_rec, (void *)0, net_video1_event);
    }
    req.rec.bfmode = VIDEO_PPBUF_MODE;
    req.rec.wl80_spec_mode = 0;
    req.rec.picture_mode = 0;//非绘本模式
    req.rec.isc_sbuf = NULL;
    req.rec.sbuf_size = 0;
    req.rec.block_done_cb = NULL;//yuv420_block_scan;
    req.rec.camera_type = VIDEO_CAMERA_NORMAL;
    req.rec.channel = 1;
    req.rec.width 	= video1_width_get();
    req.rec.height 	= video1_height_get();
    ...
    */
    ...
    printf(">>>>>>video1 width=%d    height=%d\n", req.rec.width, req.rec.height);
    return err;
error:
    if (__this_net->net_video_rec) {
...
static void net_rec_dev_server_event_handler(void *priv, int argc, int *argv)
{
    ...
    case VIDEO_SERVER_PKG_NET_ERR:
        ...
        if (__this_net->net_video0_vrt_on) {
            ...
            os_taskq_post(POST_TASK_NAME, 2, APP_CAMERA_IN, NULL);
...
static int net_video_rec0_start()
{
    ...
    if (__this && __this->state == VIDREC_STA_START) {
        ...
        req.rec.packet_cb = stream_packet_cb;
        err = server_request(__this_net->net_video_rec, VIDEO_REQ_SET_PACKET_CALLBACK, &req);
        if (err) {
            return err;
        }
        ...
        if (!addr) {
            sprintf(path, "usr://%s", "192.168.1.");
        } else {
#if (defined CONFIG_NET_UDP_ENABLE)
            sprintf(path, "udp://%s:%d", inet_ntoa(addr->sin_addr.s_addr), _FORWARD_PORT);
#elif (defined CONFIG_NET_TCP_ENABLE)
            sprintf(path, "tcp://%s:%d", inet_ntoa(addr->sin_addr.s_addr), _FORWARD_PORT);
#elif (defined CONFIG_USR_VIDEO_ENABLE)
            sprintf(path, "usr://%s", CONFIG_USR_PATH);
#endif
        }
        err = stream_protocol_task_create(path, NULL);
        if (!err) {
    ...
    */
    ...
    if (!addr) {
        sprintf(req.rec.net_par.netpath, "usr://%s", "192.168.1.");
    } else {
#if (defined CONFIG_NET_UDP_ENABLE)
        sprintf(req.rec.net_par.netpath, "udp://%s:%d"
                , inet_ntoa(addr->sin_addr.s_addr)
...
static int net_video_rec_state_machine(struct application *app, enum app_state state, struct intent *it)
{
    ...
    case APP_STA_START:
...
#if TCFG_LCD_USB_SHOW_COLLEAGUE
            os_taskq_post(POST_TASK_NAME, 2, APP_CAMERA_OUT, NULL);
...
#else
            err = net_video1_open(__this_net->priv);
            if (!err) {
                __this_net->net_video1_vrt_on = 1;
                __this_net->net_videoreq[1].rec.width = video1_width_get();
                __this_net->net_videoreq[1].rec.height = video1_height_get();
                __this_net->net_videoreq[1].rec.real_fps = 25;
            } else {
                printf("ACTION_VIDEO0_OPEN_RT_STREAM err!!!\n\n");
                CTP_CMD_COMBINED(NULL, CTP_RT_OPEN_FAIL, "OPEN_RT_STREAM", "NOTIFY", CTP_RT_OPEN_FAIL_MSG);
            }
        ...
        case ACTION_VIDEO0_CLOSE_RT_STREAM:
            ...
            ctp_cmd_socket_unregister(NULL);//关闭实时流先请求socket退出，以免TCP时产生阻塞
#ifdef CONFIG_SPI_VIDEO_ENABLE
            extern int spi_video_wait_done(void);
            spi_video_wait_done();
#endif
...
static int net_video_key_event_handler(struct key_event *key)
{
    ...
    case KEY_EVENT_CLICK:
        printf("KEY_EVENT_CLICK value = %d\n", key->value);
        cmd_video_rec_ctl(state);
        state = !state;
        ret = true;
        break;
    case KEY_EVENT_LONG:
        printf("KEY_EVENT_LONG value = %d\n", key->value);
