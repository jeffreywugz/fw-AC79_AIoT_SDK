...
int ctp_cmd_response(void *hdl, const char *cmd/*,u16 cmd_len*/, const char *content /*, u32 content_len*/)
{

    int ret;
    char msg[512] = {0};

    struct tutk_client_info *tutk_hdl = (struct tutk_client_info *)hdl;

    if (cmd == NULL || content == NULL) {
        printf("cmd == NULL || content == NULL\n");
        return -1;
    ...
    memcpy(msg, "CTP:", strlen("CTP:"));
    memcpy(msg + strlen("CTP:"), &cmd_len, 2);
    memcpy(msg + strlen("CTP:") + 2, cmd, cmd_len);
    memcpy(msg + strlen("CTP:") + 2 + cmd_len, &content_len, 4);
    memcpy(msg + strlen("CTP:") + 2 + cmd_len + 4, content, content_len);

    printf("msg:%s\n", msg + strlen("CTP:") + 2 + cmd_len + 4);
    /* return 0;    */
    extern int tutk_noitfy_cli(void *hdl, char *msg, int msg_len);

    return tutk_noitfy_cli(hdl, msg, sizeof(msg));
}
...
void __cmd_pro(void *cli, const char *content)
{
    ...
    struct ctp_cmd_struct ctp = {0};
    char buf[256];
    content = strstr(content, "CTP:");

    if (!content) {
        printf("\ncontent err,content is %s\n", content);
        return;
    }

    memcpy(&ctp.topic_len, content + strlen("CTP:"), 2);
    printf("ctp.topic_len=%d\n", ctp.topic_len);
    memcpy(ctp.topic, content + strlen("CTP:") + 2, ctp.topic_len);
    printf("ctp.topic=%s\n", ctp.topic);
    memcpy(&ctp.content_len, content + strlen("CTP:") + 2 + ctp.topic_len, 4);
    printf("ctp.content_len=%d\n", ctp.content_len);
    memcpy(ctp.content, content + strlen("CTP:") + 2 + ctp.topic_len + 4, ctp.content_len);
    printf("ctp.content=%s\n", ctp.content);

    ctp_cmd_analysis(ctp.topic, ctp.content, cli);
...
int send_ctp_string(int cmd_type, char *buf, const char *_req, void *priv)
{
    ...
    return ctp_cmd_response(priv, _req/*,u16 cmd_len*/, buf /*, u32 content_len*/);
...
int cmd_put_system_default(void *priv, char *content)
{
    ...
    if (atoi(def_value)) {
        ...
        sprintf(ssid, WIFI_CAM_PREFIX"%02x%02x%02x%02x%02x%02x"
                ...
                , mac_addr[5]);
#endif
        wifi_store_mode_info(AP_MODE, ssid, WIFI_CAM_WIFI_PWD);
