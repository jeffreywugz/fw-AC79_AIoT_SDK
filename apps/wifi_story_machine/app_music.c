...
#ifdef CONFIG_UI_ENABLE
        os_taskq_post(POST_TASK_NAME, 1, UI_MSG_PAUSE_START);
#endif
    } else {
        led_ui_post_msg("dec_pause");
#ifdef CONFIG_UI_ENABLE
        os_taskq_post(POST_TASK_NAME, 1, UI_MSG_PAUSE_STOP);
#endif
    }
...
#ifdef CONFIG_UI_ENABLE
    os_taskq_post(POST_TASK_NAME, 2, UI_MSG_ALL_TIME, __this->total_time);
#endif
...
#ifdef CONFIG_UI_ENABLE
    os_taskq_post(POST_TASK_NAME, 2, UI_MSG_SET_VOLUME, __this->volume);
#endif
...
#ifdef CONFIG_UI_ENABLE
                extern char ui_show_mp3_name[64];
                memcpy(ui_show_mp3_name, name, sizeof(name));
                os_taskq_post(POST_TASK_NAME, 1, UI_MSG_SONG_NAME);
#endif
            }
            ++i;
            break;
...
#ifdef CONFIG_UI_ENABLE
static void get_upgrade_percent(update_percent_info *info)
{
    printf("######ui_upgrade_percent = %d", info->percent);
    os_taskq_post(POST_TASK_NAME, 2, UI_MSG_SHOW_PERSENT, info->percent);
}
#endif
...
static int sdcard_upgrade(void)
{
    ...
    if (fsdc) {
#ifdef CONFIG_UI_ENABLE
        os_taskq_post(POST_TASK_NAME, 2, UI_MSG_UPDATE, 1);
        register_update_percent_info_callback_handle(get_upgrade_percent);
#endif
        fclose(fsdc);
...
#ifdef CONFIG_BT_ENABLE
        if (__this->bt_connecting == 1 && !__this->bt_emitter_enable) {
...
#ifdef CONFIG_UI_ENABLE
        os_taskq_post(POST_TASK_NAME, 2, UI_MSG_CURRENT_TIME, __this->play_time);
#endif
...
static int app_music_key_long(struct key_event *key)
{
...
#ifdef CONFIG_UI_ENABLE
        os_taskq_post(POST_TASK_NAME, 1, UI_MSG_LAST_SONG);
#endif
        break;
    case KEY_VOLUME_INC:
    case KEY_DOWN:
        ...
        __this->force_switch_file = 0;
#ifdef CONFIG_UI_ENABLE
        os_taskq_post(POST_TASK_NAME, 1, UI_MSG_NEXT_SONG);
#endif
        break;
    case KEY_MODE:
#ifdef CONFIG_NET_ENABLE
    ...
    case KEY_MENU:
        ...
        break;
    case KEY_CANCLE:
        if (__this->mode >= LOCAL_MUSIC_MODE && __this->mode <= UDISK_MUSIC_MODE && !__this->local_play_all) {
...
static int app_music_key_event_handler(struct key_event *key)
{
    ...
    if (__this->call_flag && (key->value != KEY_UP && key->value != KEY_DOWN && key->value != KEY_VOLUME_DEC && key->value != KEY_VOLUME_INC && key->value != KEY_OK)) {
...
#ifdef CONFIG_UI_ENABLE
            os_taskq_post(POST_TASK_NAME, 2, UI_MSG_POWER_LOG, __this->battery_per);
#endif
