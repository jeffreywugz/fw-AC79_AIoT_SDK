...
static void get_one_frame(u8 *inputFrame, u32 len, int width, int height)
{
    ...
    if (is_in_config_network_state()) {
        if (qr_decoder && qr_flag) {
            ...
            YUV420p_Soft_Scaling(inputFrame, inputFrame, width, height, width / 4, height / 3);
#else
...
{
    puts("page_turning_det_init...\n\n");

#ifdef CONFIG_PAGE_TURNING_DET_ENABLE
    int width = YUV_DATA_WIDTH;    //输入源宽度
    int height = YUV_DATA_HEIGHT;   //输入源高度
    ...
    int image_offset = roi_y * 640 + roi_x;
    int hstep = 4;      //检测跳跃水平像素点,
    int vstep = 4;      //检测跳跃垂直像素点,
    int erode_times = 3;    //灵敏度,越小越灵敏
    int max_frame_cnt = 3;  //隔开(n-2)帧检测一次
    int motion_th = 10;     //多少个像素点移动视为有翻页

    ptd_init(&jl_ptd, image_offset, roi_w, roi_h, width, max_frame_cnt, motion_th, erode_times, hstep, vstep);
#endif
