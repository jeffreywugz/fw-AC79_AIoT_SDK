...
#define STRM_USE_NET_VIDEO_BUFF	0 //当使用共用视频buf时:1
...
#define NET_ISC_SBUF_SIZE     	(CONFIG_VIDEO_IMAGE_W*32*3/2)/*720P内部ISC BUFF采用内部ram*/
    ...
    u8 mic = 0;
    syscfg_read(VM_MIC_INDEX, &mic, sizeof(mic));
    req.rec.pkg_mute.aud_mute = !mic;
...
#ifdef CONFIG_VIDEO2_ENABLE
    u8 gap = 0;
    syscfg_read(VM_GAP_INDEX, &gap, sizeof(gap));
    req.rec.tlp_time = gap;
#endif
