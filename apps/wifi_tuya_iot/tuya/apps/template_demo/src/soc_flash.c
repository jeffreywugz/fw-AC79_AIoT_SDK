...
OPERATE_LIGHT opSocFlashErase(VOID)
{
    ...
    vNum2Str(0, SAVE_TYP1, 4, cTemp);
    opRet = ufdelete(cTemp);
    if (opRet != OPRT_OK) {
        PR_ERR("Delete %s file error!", cTemp);
    }

    vNum2Str(0, SAVE_TYP2, 4, cTemp);
    opRet = ufdelete(cTemp);
    if (opRet != OPRT_OK) {
        PR_ERR("Delete %s file error!", cTemp);
    }

    vNum2Str(0, SAVE_TYP3, 4, cTemp);
    opRet = ufdelete(cTemp);
    if (opRet != OPRT_OK) {
        PR_ERR("Delete %s file error!", cTemp);
    }

    vNum2Str(0, SAVE_TYP4, 4, cTemp);
    opRet = ufdelete(cTemp);
    if (opRet != OPRT_OK) {
        PR_ERR("Delete %s file error!", cTemp);
    }
