...
static inline u32 get_jiffies(void)
{
...
static inline u32 UT_OSSemPend(UT_Semaphore *sem, u32 timeout)
{
    u32 _timeout = timeout + get_jiffies();
    while (1) {
        if (*sem) {
            (*sem) --;
            break;
        }
        if ((timeout != -1) && time_before(_timeout, get_jiffies())) {
            return -1;
        }
    }
    return 0;
...
const uart_bus_t *uart_open(const struct uart_platform_data_t *arg);
