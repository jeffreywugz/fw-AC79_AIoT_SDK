...
struct audio_drc_param {
    ...
    u8 channels : 5;           //通道数 (channels|(L_wdrc))或(channels|(R_wdrc))
    u8 online_en : 1;          //是否支持在线调试
    u8 remain_en : 1;          //写1
    u8 stero_div : 1;          //是否左右声道 拆分的  drc效果,一般写0
...
struct audio_drc {
    ...
    u16 channels : 5;           //通道数(channels|(L_wdrc))或(channels|(R_wdrc))
    u16 remain_flag : 1;        //输出数据支持remain
    u16 online_en : 1;          //是否支持在线更新系数
    u16 remain_en : 1;          //是否支持remain输出
    u16 start : 1;              //无效
    u16 run32bit : 1;           //是否使能32bit位宽数据处理1:使能  0：不使能
    u16 stero_div : 1;          //是否左右声道拆分的drc效果,1：是  0：否
