 ...
 */
inline static int
bits_setb(unsigned char *vec, size_t size, unsigned char bit)
{
    if (size <= (bit >> 3)) {
        return -1;
    }

    *(vec + (bit >> 3)) |= (unsigned char)(1 << (bit & 0x07));
    return 1;
}
 ...
 */
inline static int
bits_clrb(unsigned char *vec, size_t size, unsigned char bit)
{
    if (size <= (bit >> 3)) {
        return -1;
    }
 ...
 */
inline static int
bits_getb(const unsigned char *vec, size_t size, unsigned char bit)
{
    if (size <= (bit >> 3)) {
        return -1;
    }
