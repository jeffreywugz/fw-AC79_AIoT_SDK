...
typedef void (*MSG_CALLBACK)(MessageData *md);
...
typedef struct _MqttClient {
    ...
    struct subscribe_topic topic_head;  //add: 订阅链表
...
typedef MqttClient Client;
typedef MqttNet Network;

#include "printf.h"
#include "sock_api/sock_api.h"
#define DEFAULT_CON_TIMEOUT_MS  5000

typedef void (*messageHandler)(struct MessageData *);
...
void MQTTClient(struct _MqttClient *c, Network *network, unsigned int command_timeout_ms, unsigned char *buf, size_t buf_size, unsigned char *readbuf, size_t readbuf_size);
int MQTTConnect(struct _MqttClient  *c, MQTTPacket_connectData *data);
int MQTTDisconnect(struct _MqttClient *c);
int MQTTSubscribe(struct _MqttClient   *c, const char *topicFilter, enum _MqttQoS qos, messageHandler messageHandler);
int MQTTUnsubscribe(struct _MqttClient   *c, const char *topicFilter);
int MQTTPublish(struct _MqttClient   *c, const char *topicName, MQTTMessage *message);
int MQTTYield(struct _MqttClient *c, int timeout_ms);
