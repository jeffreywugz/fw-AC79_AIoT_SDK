...
#define NET_BUF_MALLOC(_name, _net_buf, _net_buf_data, _adv_pool, _count) \
    net_buf_##_name = (struct net_buf *)_net_buf; \
    net_buf_data_##_name = (u8_t *)_net_buf_data; \
    _name.__bufs = (struct net_buf *)_net_buf; \
    _name.buf_count = _count; \
    _name.uninit_count = _count; \
    _name.free_count = _count; \
    adv_pool = _adv_pool;

#define NET_BUF_FREE(_name) \
    net_buf_##_name

#define NET_BUF_POOL_FIXED_DEFINE(_name, _count, _data_size, _destroy)        \
	static struct net_buf *net_buf_##_name __noinit;               \
	static u8_t __noinit *net_buf_data_##_name;        \
	static const struct net_buf_pool_fixed net_buf_fixed_##_name = {      \
		.data_size = _data_size,                                      \
		.data_pool = &net_buf_data_##_name,                    \
	};                                                                    \
 ...
 *  @param buf Buffer to reset.
 */
void net_buffer_reset(struct net_buf *buf);
