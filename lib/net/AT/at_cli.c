...
static void server_cli_parser(void)
{
    ...
    if (server) {
        printf("======== Welcome to using RT-Thread AT command server cli ========\n");
        printf("Input your at command for test server. Press 'ESC' to exit.\n");
        server->parser_entry(server);
    } else {
        printf("AT client not initialized\n");
    }
...
#define DELECT_KEY              0x7F
    ...
    if (client) {
        ...
        if (client_rx_fifo && at_client) {
            printf("======== Welcome to using RT-Thread AT command client cli ========\n");
            printf("Cli will forward your command to server port(%s). Press 'ESC' to exit.\n", client->device->parent.name);
            ...
            printf("No mem for AT cli client\n");
        }
    } else {
        printf("AT client not initialized\n");
    }
...
static void at(int argc, char **argv)
{
    ...
    if (argc != 2 && argc != 3) {
        printf("Please input '<server|client [dev_name]>' \n");
        return;
    }
    ...
    if (!strcmp(argv[1], "server")) {
        ...
        server_cli_parser();
#else
        printf("Not support AT server, please check your configure!\n");
#endif /* AT_USING_SERVER */
        ...
        if (argc == 2) {
            ...
            if (client == RT_NULL) {
                printf("input AT client device name(%s) error.\n", argv[2]);
            } else {
                client_cli_parser(client);
            }
        }
#else
        printf("Not support AT client, please check your configure!\n");
#endif /* AT_USING_CLIENT */
    } else {
        printf("Please input '<server|client [dev_name]>' \n");
    }
