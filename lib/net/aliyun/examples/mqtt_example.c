...
char DEMO_PRODUCT_KEY[IOTX_PRODUCT_KEY_LEN + 1] = "a1mhUx5m6Uq";
char DEMO_DEVICE_NAME[IOTX_DEVICE_NAME_LEN + 1] = "iot-001";
char DEMO_DEVICE_SECRET[IOTX_DEVICE_SECRET_LEN + 1] = "6cba2f877c8683c2341009750b88481b";
...
int example_publish(void *handle)
{
    ...
    char           *topic = NULL;
    int             topic_len = 0;
    char           *payload = "{\"id\":\"161848123\",\"version\":\"1.0\",\"params\":{\"temperature\":16,\"Humidity\":21},\"method\":\"thing.event.property.post\"}";
...
int aliyun_mqtt_test(void)
{
     ...
     */
    mqtt_params.host = "a1YGQti1kxb.iot-as-mqtt.cn-shanghai.aliyuncs.com";
...
extern int thread_fork(const char *thread_name, int prio, int stk_size, unsigned int q_size, int *pid, void (*func)(void *), void *parm);
#define OS_NO_ERR 0

static void aliyun_subscribe(void *priv)
{
    aliyun_mqtt_test();
}

int aliyun_mqtt_start(void)
{
    if (thread_fork("aliyun_subscribe", 17, 2024, 0, 0, aliyun_subscribe, NULL) != OS_NO_ERR) {
        return -1;
    }
    return 0;
}
