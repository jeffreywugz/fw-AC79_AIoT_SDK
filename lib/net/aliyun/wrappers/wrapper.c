...
static const char _firmware_version[IOTX_FIRMWARE_VER_LEN] = "JL-APP-V2021-4-19";
...
{
    strcpy(device_secret, "6cba2f877c8683c2341009750b88481b");
    return (int)1;
}
...
{
    strcpy(product_key, "a1mhUx5m6Uq");
    return (int)1;
}
...
{
    strcpy(product_secret, "a1mhUx5m6Uq");
    return (int)1;
}
    ...
    int *stack_used)
{
    ...
    if (thread_fork(hal_os_thread_param->name, hal_os_thread_param->priority, hal_os_thread_param->stack_size, \
                    0, &handle, work_routine, arg) != OS_NO_ERR) {
        puts("HAL_ThreadCreate : thread_fork");
        return -1;
    }
