...
#  endif /* HAVE_BUILTIN_AVAILABLE */
            ...
            infof(0, "sockfd : %d\n", sockfd);
            infof(0, "[%s] [%d]\n", __func__, __LINE__);
            struct sockaddr_in *sockaddr = (struct sockaddr_in *) &addr.sa_addr;
            struct sockaddr_in sockaddr2;
            memcpy(&sockaddr2, sockaddr, sizeof(struct sockaddr_in));
            infof(0, "addr : %s\n", inet_ntoa(sockaddr2.sin_addr));
            // rc = connect(sockfd, &addr.sa_addr, addr.addrlen);
            rc = connect(sockfd, &addr.sa_addr, sizeof(struct sockaddr));
            infof(0, "rc : %d\n", rc);
        }
...
#else
        if (read(conn->sock[FIRSTSOCKET], &buf, sizeof(buf)) == 0)
#endif //USE_RTOS
        {
            return false;   /* FIN received */
        }
