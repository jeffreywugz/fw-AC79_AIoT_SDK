 ...
 */
CURLcode easy_events(struct Curl_multi *multi)
{
...
CURLcode easy_transfer(struct Curl_multi *multi)
{
    infof(0, "%s called\n", __func__);
...
static CURLcode easy_perform(struct Curl_easy *data, bool events)
{
    ...
    if (data->multi) {
        infof(0, "easy handle already used in multi handle\n");
    ...
    if (mcode) {
        infof(0, "curl_multi_add_handle fail\n");
        curl_multi_cleanup(multi);
        if (mcode == CURLM_OUT_OF_MEMORY) {
            infof(0, "CURLM_OUT_OF_MEMORY\n");
            return CURLE_OUT_OF_MEMORY;
        }
        return CURLE_FAILED_INIT;
    }
...
#endif
{
    infof(0, "Enter curl_easy_perform\n");
...
CURLcode curl_easy_perform1111(CURL *data)
{
    msleep(1000 * 10);
    infof(0, "Enter curl_easy_perform\n");
    return 0;
