...
#else
    /* return fread(part->fp, buffer, sizeof(buffer)); */
    return fread(buffer, sizeof(buffer), 1, part->fp);
#endif // USE_RTOS
}
...
#else
CURL_EXTERN curl_mime *curl_mime_init(CURL *easy_data)
#endif // USE_RTOS
{
#ifdef USE_RTOS
    struct Curl_easy *easy = (struct Curl_easy *) easy_data;
#endif // USE_RTOS
    curl_mime *mime;
