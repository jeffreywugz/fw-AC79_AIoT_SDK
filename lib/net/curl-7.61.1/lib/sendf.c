                           ...
                           size_t olen)
{
    ...
    while (len) {
        ...
        if (writebody) {
            ...
            printf("out :%x wrote:%d  chunklen:%d\n", data->set.out, wrote, chunklen);
...
#else
            /* fwrite(data->set.err, s_infotype[type], sizeof(s_infotype[type])); */
            /* fwrite(data->set.err, ptr, sizeof(ptr)); */
            fwrite(s_infotype[type], sizeof(s_infotype[type]), 1, data->set.err);
            fwrite(ptr, sizeof(ptr), 1, data->set.err);
#endif
#ifdef CURL_DOES_CONVERSIONS
            if (size != conv_size) {
