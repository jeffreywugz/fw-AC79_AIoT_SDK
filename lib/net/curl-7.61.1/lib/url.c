    ...
    if ((url_has_scheme && strncasecompare(data->change.url, "file:", 5)) ||
        ...
        *prot_missing = !url_has_scheme;
        ...
        if (3 > rc) {
...
#if 1
            rc = sscanf(data->change.url, "%[^\n/?#]%[^\n]", conn->host.name, path);
            infof(0, "conn->host.name : %s\n, path : %s\n, rc : %d\n", conn->host.name, path, rc);
#else
            char *text, *ch = "/", comparison[10], *suburl;
            rc = sscanf(data->change.url, "%s%s", conn->host.name, path);
            suburl = strstr(conn->host.name, "com");
            suburl = suburl + 3;
            memcpy(path, suburl, strlen(suburl));
            infof(0, "conn->host.name : %s, path : %s, rc : %d\n", conn->host.name, path, rc);

            text = strtok(conn->host.name, ch);

            while (text != NULL) {
                if (strlen(text) > 3) {
                    memset(comparison, 0, sizeof(comparison));
                    memcpy(comparison, ((text + strlen(text)) - 3), 3);
                    comparison[3] = '\0';
                    if (!strcmp(comparison, "com")) {
                        break;
                    }
                }
                text = strtok(NULL, ch);
            ...
            memset(conn->host.name, 0, strlen(conn->host.name));
            strcpy(conn->host.name, text);
            infof(0, "conn->host.name : %s\n", conn->host.name);

            infof(0, "path : %s\n", path);
            rebuild_url = TRUE;

#endif //USE_RTOS sscanf not support a format of regex.
            if (1 > rc) {
                /*
                                  ...
                                  struct connectdata *conn)
{
    ...
    if (data->set.use_port && data->state.allow_port) {
        ...
        if (portptr) {
            *portptr = '\0';
        } /* cut off the name there anyway - if there was a port
        ...
        *async = FALSE;
        ...
        if (conn->unix_domain_socket) {
            ...
            if (!conn->bits.proxy) {
                ...
                infof(0, "conn->bits.socksproxy : %d\n", conn->bits.socksproxy);
                /* resolve proxy */
#ifdef CURL_TEST
                infof(0, "host->name : %s : %d\n", host->name, (int)conn->port);
#endif // CURL_TEST
                            ...
                            bool *async)
{
    ...
    *async = FALSE;
    ...
    if (!data->change.url) {
        infof(0, "[%s] Return CURLE_URL_MALFORMAT\n", __func__);
