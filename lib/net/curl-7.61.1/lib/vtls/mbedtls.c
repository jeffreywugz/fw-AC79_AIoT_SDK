...
#ifdef USE_RTOS
int mbedtls_x509_crt_parse_file(void *a, char *b)
{
    infof(0, "%s is NULL function\n", __func__);
    return 1;
}
int mbedtls_x509_crl_parse_file(void *a, char *b)
{
    infof(0, "%s is NULL function\n", __func__);
    return 1;
}

int mbedtls_x509_crt_parse_path(void *a, char *b)
{
    infof(0, "%s is NULL function\n", __func__);
    return 1;
}

int mbedtls_pk_parse_keyfile(void *a, char *b, ...)
{
    infof(0, "%s is NULL function\n", __func__);
    return 1;
}
...
{
    crl = (mbedtls_x509_crl *) malloc(sizeof(mbedtls_x509_crl));
    if (crl != NULL) {
        infof(0, "return mbedtls_x509_crl_init\n");
    } else {
        infof(0, "mbedtls_x509_crl_init fail\n");
    }
}
#endif
static CURLcode
                   ...
                   int sockindex)
{
    ...
    infof(0, "Before mbedtls_x509_crl_init %d\n", __LINE__);
    /* Load the CRL */
    ...
    if (ssl_crlfile) {
        ...
        infof(0, "After mbedtls_x509_crl_init %d\n", __LINE__);
        if (ret) {
#ifdef MBEDTLS_ERROR_C
            mbedtls_strerror(ret, errorbuf, sizeof(errorbuf));
#endif /* MBEDTLS_ERROR_C */
    ...
    infof(data, "&BACKEND->config : %p\n", &BACKEND->config);
    mbedtls_ssl_conf_authmode(&BACKEND->config, MBEDTLS_SSL_VERIFY_NONE);
    infof(data, "%s %d -- mark\n", __func__, __LINE__);
    mbedtls_ssl_conf_rng(&BACKEND->config, mbedtls_ctr_drbg_random,
                         &BACKEND->ctr_drbg);
    infof(data, "%s %d -- mark\n", __func__, __LINE__);
    extern int mbedtls_net_recv2(void *ctx, unsigned char *buf, size_t len);
    extern int mbedtls_net_send2(void *ctx, const unsigned char *buf, size_t len);
 ...
 */
int port_mbedtls_ssl_handshake_step(mbedtls_ssl_context *ssl);
int port_mbedtls_ssl_handshake(mbedtls_ssl_context *ssl);
int port_mbedtls_ssl_handshake(mbedtls_ssl_context *ssl)
{
    int ret = 0;
    infof(0, "%s called\n", __func__);
    ...
    while (ssl->state != MBEDTLS_SSL_HANDSHAKE_OVER) {
        infof(0, "%s %d\n", __func__, __LINE__);
        ret = port_mbedtls_ssl_handshake_step(ssl);
...
void mbedtls_x509_crl_free(mbedtls_x509_crl *crl)
{
    ...
    infof(0, "return mbedtls_x509_crl_free\n");
}
#endif
                    ...
                    bool *done)
{
    ...
    infof(data, "%s %d", __func__, __LINE__);
