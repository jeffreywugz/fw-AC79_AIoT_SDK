...
#else
    result = Curl_ssl->connect_rtos(conn, sockindex);
#endif //USE_RTOS
...
#else
    if (Curl_ssl->shutdown_rtos(conn, sockindex))
#endif   //USE_RTOS
...
#else
        /* if ((int) fread(fp, buf, sizeof(buf)) != 1) */
        if ((int) fread(buf, sizeof(buf), 1, fp) != 1)
#endif //USE_RTOS
            break;
...
#else
    return Curl_ssl->connect_rtos(conn, sockindex);
#endif  //USE_RTOS
}
