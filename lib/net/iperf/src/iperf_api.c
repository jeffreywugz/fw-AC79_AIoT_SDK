...
usage_long(void *f)
{
    printf(usage_longstr, UDP_RATE / (1024 * 1024), DURATION, DEFAULT_TCP_BLKSIZE / 1024, DEFAULT_UDP_BLKSIZE / 1024);
}
...
iperf_on_connect(struct iperf_test *test)
{
    ...
    (void) strftime(now_str, sizeof(now_str), rfc1123_fmt, gmtime_r(&now_secs, &result));
    if (test->json_output) {
        iperf_cJSON_AddItemToObject(test->json_start, "timestamp", iperf_json_printf("time: %s  timesecs: %d", now_str, (int64_t) now_secs));
    ...
    if (test->role == 'c') {
        if (test->json_output) {
            iperf_cJSON_AddItemToObject(test->json_start, "connecting_to", iperf_json_printf("host: %s  port: %d", test->server_hostname, (int64_t) test->server_port));
        } else {
        ...
        if (test->json_output) {
            iperf_cJSON_AddItemToObject(test->json_start, "accepted_connection", iperf_json_printf("host: %s  port: %d", ipr, (int64_t) port));
        } else {
            iperf_printf(test, report_accepted, ipr, port);
        }
    }
    if (test->json_output) {
        iperf_cJSON_AddStringToObject(test->json_start, "cookie", test->cookie);
        if (test->protocol->id == SOCK_STREAM) {
            if (test->settings->mss) {
                iperf_cJSON_AddNumberToObject(test->json_start, "tcp_mss", test->settings->mss);
            } else {
                iperf_cJSON_AddNumberToObject(test->json_start, "tcp_mss_default", test->ctrl_sck_mss);
            }
        }
    } else if (test->verbose) {
...
iperf_parse_arguments(struct iperf_test *test, int argc, char **argv)
{
    ...
    while ((flag = getopt_long(argc, argv, "p:f:i:D1VJvsc:ub:t:n:k:l:P:Rw:B:M:N46S:L:ZO:F:A:T:C:dI:hX:", longopts, NULL)) != -1) {
        ...
        case 'J':
            ...
            break;
        case 'v':
            printf("%s (cJSON %s)\n%s\n%s\n", version, iperf_cJSON_Version(), get_system_info(),
    ...
    if (test->logfile) {
        test->outfile = (void *) - 1; //fopen(test->logfile, "a+");
    ...
    printf("~` iperf settings->rate=%llu bps \r\n", test->settings->rate);
...
iperf_init_test(struct iperf_test *test)
{
    ...
    if (test->protocol->iperf_init) {
        if (test->protocol->iperf_init(test) < 0) {
            return -1;
        }
    }
...
iperf_exchange_parameters(struct iperf_test *test)
{
    ...
    if (test->role == 'c') {
        ...
        if ((s = test->protocol->iperf_listen(test)) < 0) {
            if (iperf_set_send_state(test, SERVER_ERROR) != 0) {
                return -1;
            }
            err = htonl(i_errno);
...
send_parameters(struct iperf_test *test)
{
    ...
    if (j == NULL) {
        ...
        iperf_cJSON_AddNumberToObject(j, "omit", test->omit);
        if (test->server_affinity != -1) {
            iperf_cJSON_AddNumberToObject(j, "server_affinity", test->server_affinity);
        }
        if (test->duration) {
            iperf_cJSON_AddNumberToObject(j, "time", test->duration);
        }
        if (test->settings->bytes) {
            iperf_cJSON_AddNumberToObject(j, "num", test->settings->bytes);
        }
        if (test->settings->blocks) {
            iperf_cJSON_AddNumberToObject(j, "blockcount", test->settings->blocks);
        }
        if (test->settings->mss) {
            iperf_cJSON_AddNumberToObject(j, "MSS", test->settings->mss);
        }
        if (test->no_delay) {
            iperf_cJSON_AddTrueToObject(j, "nodelay");
        }
        iperf_cJSON_AddNumberToObject(j, "parallel", test->num_streams);
        if (test->reverse) {
            iperf_cJSON_AddTrueToObject(j, "reverse");
        }
        if (test->settings->socket_bufsize) {
            iperf_cJSON_AddNumberToObject(j, "window", test->settings->socket_bufsize);
        }
        if (test->settings->blksize) {
            iperf_cJSON_AddNumberToObject(j, "len", test->settings->blksize);
        }
        if (test->settings->rate) {
            iperf_cJSON_AddNumberToObject(j, "bandwidth", test->settings->rate);
        }
        if (test->settings->fqrate) {
            iperf_cJSON_AddNumberToObject(j, "fqrate", test->settings->fqrate);
        }
        if (test->settings->pacing_timer) {
            iperf_cJSON_AddNumberToObject(j, "pacing_timer", test->settings->pacing_timer);
        }
        if (test->settings->burst) {
            iperf_cJSON_AddNumberToObject(j, "burst", test->settings->burst);
        }
        if (test->settings->tos) {
            iperf_cJSON_AddNumberToObject(j, "TOS", test->settings->tos);
        }
        if (test->settings->flowlabel) {
            iperf_cJSON_AddNumberToObject(j, "flowlabel", test->settings->flowlabel);
        }
        if (test->title) {
        ...
        if (test->congestion_used) {
            iperf_cJSON_AddStringToObject(j, "congestion_used", test->congestion_used);
        }
        if (test->get_server_output) {
            iperf_cJSON_AddNumberToObject(j, "get_server_output", iperf_get_test_get_server_output(test));
        }
        if (test->udp_counters_64bit) {
            iperf_cJSON_AddNumberToObject(j, "udp_counters_64bit", iperf_get_test_udp_counters_64bit(test));
        }
#if defined(HAVE_SSL)
        if (test->settings->authtoken) {
            iperf_cJSON_AddStringToObject(j, "authtoken", test->settings->authtoken);
        }
#endif // HAVE_SSL
...
get_parameters(struct iperf_test *test)
{
    ...
    if (j == NULL) {
        ...
        if ((j_p = iperf_cJSON_GetObjectItem(j, "server_affinity")) != NULL) {
        ...
        if ((j_p = iperf_cJSON_GetObjectItem(j, "fqrate")) != NULL) {
            test->settings->fqrate = j_p->valueint;
        }
        if ((j_p = iperf_cJSON_GetObjectItem(j, "pacing_timer")) != NULL) {
        ...
        if ((j_p = iperf_cJSON_GetObjectItem(j, "flowlabel")) != NULL) {
            test->settings->flowlabel = j_p->valueint;
        }
        if ((j_p = iperf_cJSON_GetObjectItem(j, "title")) != NULL) {
            test->title = strdup(j_p->valuestring);
        }
        if ((j_p = iperf_cJSON_GetObjectItem(j, "congestion")) != NULL) {
            test->congestion = strdup(j_p->valuestring);
        }
        if ((j_p = iperf_cJSON_GetObjectItem(j, "congestion_used")) != NULL) {
            test->congestion_used = strdup(j_p->valuestring);
        }
        if ((j_p = iperf_cJSON_GetObjectItem(j, "get_server_output")) != NULL) {
            iperf_set_test_get_server_output(test, 1);
        }
        if ((j_p = iperf_cJSON_GetObjectItem(j, "udp_counters_64bit")) != NULL) {
            iperf_set_test_udp_counters_64bit(test, 1);
        }
#if defined(HAVE_SSL)
        if ((j_p = iperf_cJSON_GetObjectItem(j, "authtoken")) != NULL) {
...
send_results(struct iperf_test *test)
{
    ...
    if (j == NULL) {
        ...
        iperf_cJSON_AddNumberToObject(j, "cpu_util_total", test->cpu_util[0]);
        iperf_cJSON_AddNumberToObject(j, "cpu_util_user", test->cpu_util[1]);
        iperf_cJSON_AddNumberToObject(j, "cpu_util_system", test->cpu_util[2]);
        if (! test->sender) {
            sender_has_retransmits = -1;
        } else {
            sender_has_retransmits = test->sender_has_retransmits;
        }
        iperf_cJSON_AddNumberToObject(j, "sender_has_retransmits", sender_has_retransmits);
        if (test->congestion_used) {
            iperf_cJSON_AddStringToObject(j, "congestion_used", test->congestion_used);
        }
        ...
        if (test->role == 's' && test->get_server_output) {
            if (test->json_output) {
                /* Add JSON output */
                iperf_cJSON_AddItemReferenceToObject(j, "server_output_json", test->json_top);
            } else {
                /* Add textual output */
                size_t buflen = 0;
        ...
        if (j_streams == NULL) {
            ...
            SLIST_FOREACH(sp, &test->streams, streams) {
                ...
                if (j_stream == NULL) {
                    ...
                    iperf_cJSON_AddNumberToObject(j_stream, "id", sp->id);
                    iperf_cJSON_AddNumberToObject(j_stream, "bytes", bytes_transferred);
                    iperf_cJSON_AddNumberToObject(j_stream, "retransmits", retransmits);
                    iperf_cJSON_AddNumberToObject(j_stream, "jitter", sp->jitter);
                    iperf_cJSON_AddNumberToObject(j_stream, "errors", sp->cnt_error);
                    iperf_cJSON_AddNumberToObject(j_stream, "packets", sp->packet_count);

                    start_time = timeval_diff(&sp->result->start_time, &sp->result->start_time);
                    end_time = timeval_diff(&sp->result->start_time, &sp->result->end_time);
                    iperf_cJSON_AddNumberToObject(j_stream, "start_time", start_time);
                    iperf_cJSON_AddNumberToObject(j_stream, "end_time", end_time);
...
get_results(struct iperf_test *test)
{
    ...
    if (j == NULL) {
        ...
        j_cpu_util_total = iperf_cJSON_GetObjectItem(j, "cpu_util_total");
        j_cpu_util_user = iperf_cJSON_GetObjectItem(j, "cpu_util_user");
        j_cpu_util_system = iperf_cJSON_GetObjectItem(j, "cpu_util_system");
            ...
            if (j_streams == NULL) {
                ...
                for (i = 0; i < n; ++i) {
                    ...
                    if (j_stream == NULL) {
                        ...
                        j_start_time = iperf_cJSON_GetObjectItem(j_stream, "start_time");
                        j_end_time = iperf_cJSON_GetObjectItem(j_stream, "end_time");
        ...
        j_remote_congestion_used = iperf_cJSON_GetObjectItem(j, "congestion_used");
...
connect_msg(struct iperf_stream *sp)
{
    ...
    if (sp->test->json_output) {
        iperf_cJSON_AddItemToArray(sp->test->json_connected, iperf_json_printf("socket: %d  local_host: %s  local_port: %d  remote_host: %s  remote_port: %d", (int64_t) sp->socket, ipl, (int64_t) lport, ipr, (int64_t) rport));
    } else {
...
iperf_defaults(struct iperf_test *testp)
{
    ...
    tcp->id = Ptcp;
    tcp->name = "TCP";
    tcp->iperf_accept = iperf_tcp_accept;
    tcp->iperf_listen = iperf_tcp_listen;
    tcp->iperf_connect = iperf_tcp_connect;
    tcp->iperf_send = iperf_tcp_send;
    tcp->iperf_recv = iperf_tcp_recv;
    tcp->iperf_init = NULL;
    ...
    udp->id = Pudp;
    udp->name = "UDP";
    udp->iperf_accept = iperf_udp_accept;
    udp->iperf_listen = iperf_udp_listen;
    udp->iperf_connect = iperf_udp_connect;
    udp->iperf_send = iperf_udp_send;
    udp->iperf_recv = iperf_udp_recv;
    udp->iperf_init = iperf_udp_init;
...
iperf_print_intermediate(struct iperf_test *test)
{
    ...
    if (test->json_output) {
        json_interval = iperf_cJSON_CreateObject();
        if (json_interval == NULL) {
            return;
        }
        iperf_cJSON_AddItemToArray(test->json_intervals, json_interval);
        json_interval_streams = iperf_cJSON_CreateArray();
        if (json_interval_streams == NULL) {
            return;
        }
        iperf_cJSON_AddItemToObject(json_interval, "streams", json_interval_streams);
    } else {
        json_interval = NULL;
        ...
        if (sp) {
            ...
            if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
                if (test->sender && test->sender_has_retransmits) {
                    ...
                    if (test->json_output) {
                        iperf_cJSON_AddItemToObject(json_interval, "sum", iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  retransmits: %d  omitted: %b", (double) start_time, (double) end_time, (double) irp->interval_duration, (int64_t) bytes, bandwidth * 8, (int64_t) retransmits, irp->omitted));    /* XXX irp->omitted or test->omitting? */
                    } else {
                    ...
                    if (test->json_output) {
                        iperf_cJSON_AddItemToObject(json_interval, "sum", iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  omitted: %b", (double) start_time, (double) end_time, (double) irp->interval_duration, (int64_t) bytes, bandwidth * 8, test->omitting));
                    } else {
                ...
                if (test->sender) {
                    if (test->json_output) {
                        iperf_cJSON_AddItemToObject(json_interval, "sum", iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  packets: %d  omitted: %b", (double) start_time, (double) end_time, (double) irp->interval_duration, (int64_t) bytes, bandwidth * 8, (int64_t) total_packets, test->omitting));
                    } else {
                    ...
                    if (test->json_output) {
                        iperf_cJSON_AddItemToObject(json_interval, "sum", iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  jitter_ms: %f  lost_packets: %d  packets: %d  lost_percent: %f  omitted: %b", (double) start_time, (double) end_time, (double) irp->interval_duration, (int64_t) bytes, bandwidth * 8, (double) avg_jitter * 1000.0, (int64_t) lost_packets, (int64_t) total_packets, (double) lost_percent, test->omitting));
                    } else {
...
//    struct stat sb;
    ...
    if (test->json_output) {
        json_summary_streams = iperf_cJSON_CreateArray();
        if (json_summary_streams == NULL) {
            return;
        }
        iperf_cJSON_AddItemToObject(test->json_end, "streams", json_summary_streams);
    } else {
    ...
    if (sp) {
        ...
        SLIST_FOREACH(sp, &test->streams, streams) {
            if (test->json_output) {
                json_summary_stream = iperf_cJSON_CreateObject();
                if (json_summary_stream == NULL) {
                    return;
                }
                iperf_cJSON_AddItemToArray(json_summary_streams, json_summary_stream);
            }
            ...
            if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
                if (test->sender_has_retransmits) {
                    ...
                    if (test->json_output) {
                        iperf_cJSON_AddItemToObject(json_summary_stream, "sender", iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f", (int64_t) sp->socket, (double) start_time, (double) sender_time, (double) sender_time, (int64_t) bytes_sent, bandwidth * 8));
                    } else if (test->role == 's' && !test->sender) {
                        if (test->verbose) {
                            iperf_printf(test, report_sender_not_available_format, sp->socket);
                        }
...
//		if (test->json_output)
//		    iperf_cJSON_AddItemToObject(json_summary_stream, "diskfile", iperf_json_printf("sent: %d  size: %d  percent: %d  filename: %s", (int64_t) bytes_sent, (int64_t) sb.st_size, (int64_t) percent, test->diskfile_name));
//		else
...
//	    }
            ...
            if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
                ...
                if (test->json_output) {
                    iperf_cJSON_AddItemToObject(json_summary_stream, "receiver", iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f", (int64_t) sp->socket, (double) start_time, (double) receiver_time, (double) end_time, (int64_t) bytes_received, bandwidth * 8));
                } else if (test->role == 's' && test->sender) {
                    if (test->verbose) {
    ...
    if (test->num_streams > 1 || test->json_output) {
        ...
        if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
            if (test->sender_has_retransmits) {
                ...
                if (test->json_output) {
                    iperf_cJSON_AddItemToObject(test->json_end, "sum_sent", iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  retransmits: %d", (double) start_time, (double) sender_time, (double) sender_time, (int64_t) total_sent, bandwidth * 8, (int64_t) total_retransmits));
                } else if (test->role == 's' && !test->sender) {
                    if (test->verbose) {
                        iperf_printf(test, report_sender_not_available_format, sp->socket);
                    }
                ...
                if (test->json_output) {
                    iperf_cJSON_AddItemToObject(test->json_end, "sum_sent", iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f", (double) start_time, (double) sender_time, (double) sender_time, (int64_t) total_sent, bandwidth * 8));
                } else if (test->role == 's' && !test->sender) {
                    if (test->verbose) {
                        iperf_printf(test, report_sender_not_available_format, sp->socket);
                    }
            ...
            unit_snprintf(nbuf, UNIT_LEN, bandwidth, test->settings->unit_format);
            if (test->json_output) {
                iperf_cJSON_AddItemToObject(test->json_end, "sum_received", iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f", (double) start_time, (double) receiver_time, (double) receiver_time, (int64_t) total_received, bandwidth * 8));
            } else if (test->role == 's' && test->sender) {
                if (test->verbose) {
                    iperf_printf(test, report_receiver_not_available_format, sp->socket);
                }
            ...
            if (test->json_output) {
                iperf_cJSON_AddItemToObject(test->json_end, "sum", iperf_json_printf("start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  jitter_ms: %f  lost_packets: %d  packets: %d  lost_percent: %f", (double) start_time, (double) receiver_time, (double) receiver_time, (int64_t) total_sent, bandwidth * 8, (double) avg_jitter * 1000.0, (int64_t) lost_packets, (int64_t) total_packets, (double) lost_percent));
            } else {
                /*
    ...
    if (test->json_output) {
        iperf_cJSON_AddItemToObject(test->json_end, "cpu_utilization_percent", iperf_json_printf("host_total: %f  host_user: %f  host_system: %f  remote_total: %f  remote_user: %f  remote_system: %f", (double) test->cpu_util[0], (double) test->cpu_util[1], (double) test->cpu_util[2], (double) test->remote_cpu_util[0], (double) test->remote_cpu_util[1], (double) test->remote_cpu_util[2]));
        if (test->protocol->id == Ptcp) {
            ...
            if (snd_congestion) {
                iperf_cJSON_AddStringToObject(test->json_end, "sender_tcp_congestion", snd_congestion);
            }
            if (rcv_congestion) {
                iperf_cJSON_AddStringToObject(test->json_end, "receiver_tcp_congestion", rcv_congestion);
            }
        }
    } else {
        if (test->verbose) {
...
print_interval_results(struct iperf_test *test, struct iperf_stream *sp, cJSON *json_interval_streams)
{
    ...
    if (test->protocol->id == Ptcp || test->protocol->id == Psctp) {
        if (test->sender && test->sender_has_retransmits) {
            ...
            if (test->json_output) {
                iperf_cJSON_AddItemToArray(json_interval_streams, iperf_json_printf("socket: %d  start: %f  end: %f  seconds: %f  bytes: %d  bits_per_second: %f  omitted: %b", (int64_t) sp->socket, (double) st, (double) et, (double) irp->interval_duration, (int64_t) irp->bytes_transferred, bandwidth * 8, irp->omitted));
            } else {
...
iperf_new_stream(struct iperf_test *test, int s)
{
    ...
    if (test->tmp_template) {
        ...
        snprintf(template, sizeof(template) / sizeof(char), "%s/iperf3.XXXXXX", "TMPDIR");
    }
    ...
    sp->buffer_fd = 0;//mkstemp(template);
    if (sp->buffer_fd == -1) {
        i_errno = IECREATESTREAM;
        free(sp->result);
        free(sp);
...
//        return NULL;
//    }
    sp->buffer = malloc(test->settings->blksize);//(char *) mmap(NULL, test->settings->blksize, PROT_READ|PROT_WRITE, MAP_PRIVATE, sp->buffer_fd, 0);
    if (sp->buffer == 0) {
        i_errno = IECREATESTREAM;
        free(sp->result);
        free(sp);
...
//    srandom(time(NULL));
    ...
    sp->snd = test->protocol->iperf_send;
    sp->rcv = test->protocol->iperf_recv;

    if (test->diskfile_name != (char *) 0) {
        sp->diskfile_fd = -1;//open(test->diskfile_name, test->sender ? O_RDONLY : (O_WRONLY|O_CREAT|O_TRUNC), S_IRUSR|S_IWUSR);
        if (sp->diskfile_fd == -1) {
            i_errno = IEFILE;
            free(sp->buffer);
            free(sp->result);
...
{
    test->json_top = iperf_cJSON_CreateObject();
    if (test->json_top == NULL) {
        return -1;
    }
    test->json_start = iperf_cJSON_CreateObject();
    if (test->json_start == NULL) {
        return -1;
    }
    iperf_cJSON_AddItemToObject(test->json_top, "start", test->json_start);
    test->json_connected = iperf_cJSON_CreateArray();
    if (test->json_connected == NULL) {
        return -1;
    }
    iperf_cJSON_AddItemToObject(test->json_start, "connected", test->json_connected);
    test->json_intervals = iperf_cJSON_CreateArray();
    if (test->json_intervals == NULL) {
        return -1;
    }
    iperf_cJSON_AddItemToObject(test->json_top, "intervals", test->json_intervals);
    test->json_end = iperf_cJSON_CreateObject();
    if (test->json_end == NULL) {
        return -1;
    }
    iperf_cJSON_AddItemToObject(test->json_top, "end", test->json_end);
    return 0;
}
...
{
    if (test->title) {
        iperf_cJSON_AddStringToObject(test->json_top, "title", test->title);
    }
    /* Include server output */
    if (test->json_server_output) {
        iperf_cJSON_AddItemToObject(test->json_top, "server_output_json", test->json_server_output);
    }
    if (test->server_output_text) {
        iperf_cJSON_AddStringToObject(test->json_top, "server_output_text", test->server_output_text);
    }
    test->json_output_string = iperf_cJSON_Print(test->json_top);
    if (test->json_output_string == NULL) {
        return -1;
    }
    printf("%s\n", test->json_output_string);
    iflush(test);
    iperf_cJSON_Delete(test->json_top);
