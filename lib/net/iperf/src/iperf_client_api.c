...
iperf_create_streams(struct iperf_test *test)
{
    ...
    for (i = 0; i < test->num_streams; ++i) {
        ...
        if ((s = test->protocol->iperf_connect(test)) < 0) {
            return -1;
        }
...
iperf_connect(struct iperf_test *test)
{
    ...
    if (getsockopt(test->ctrl_sck, IPPROTO_TCP, TCP_MAXSEG, &opt, &len) < 0) {
        test->ctrl_sck_mss = DEFAULT_UDP_BLKSIZE;//0;//modify by shunjian
    } else {
...
iperf_run_client(struct iperf_test *test)
{
    ...
    if (test->json_output) {
        iperf_cJSON_AddItemToObject(test->json_start, "version", iperf_cJSON_CreateString(version));
        iperf_cJSON_AddItemToObject(test->json_start, "system_info", iperf_cJSON_CreateString(get_system_info()));
