...
iperf_udp_buffercheck(struct iperf_test *test, int s)
{
    ...
    if (test->json_output) {
        iperf_cJSON_AddNumberToObject(test->json_start, "sock_bufsize", test->settings->socket_bufsize);
        iperf_cJSON_AddNumberToObject(test->json_start, "sndbuf_actual", sndbuf_actual);
        iperf_cJSON_AddNumberToObject(test->json_start, "rcvbuf_actual", rcvbuf_actual);
    }
