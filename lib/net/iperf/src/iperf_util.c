...
//    if ( ! randomized )
//        random32((int) time(0) ^ 0xaa);

    /* Generate a string based on hostname, time, randomness, and filler. */
    extern int gethostname(char *name, int namelen);
    (void) gethostname(hostname, sizeof(hostname));
    (void) gettimeofday(&tv, 0);
    (void) snprintf(temp, sizeof(temp), "%s.%ld.%06ld.%08lx%08lx.%s", hostname, (unsigned long int) tv.tv_sec, (unsigned long int) tv.tv_usec, (unsigned long int) random32(0), (unsigned long int) random32(0), "1234567890123456789012345678901234567890");
...
iperf_json_printf(const char *format, ...)
{
    ...
    for (cp = format; *cp != '\0'; ++cp) {
        ...
        case '%':
            ...
            switch (*cp) {
            case 'b':
                j = iperf_cJSON_CreateBool(va_arg(argp, int));
                break;
            case 'd':
                j = iperf_cJSON_CreateNumber(va_arg(argp, int64_t));
                break;
            case 'f':
                j = iperf_cJSON_CreateNumber(va_arg(argp, double));
                break;
            case 's':
                j = iperf_cJSON_CreateString(va_arg(argp, char *));
                break;
            default:
                va_end(argp);
                return NULL;
            ...
            iperf_cJSON_AddItemToObject(o, name, j);
            np = name;
            break;
        default:
            *np++ = *cp;
...
void
iperf_dump_fdset(void *fp, char *str, int nfds, fd_set *fds)
{
    int fd;
    int comma;
