...
Nwrite(int fd, const char *buf, size_t count, int prot)
{
    ...
    while (nleft > 0) {//printf("Nwrite nleft = %d \r\n", nleft);
...
getsock_tcp_mss(int inSock)
{
    ...
    assert_cmpt(inSock >= 0); /* print error and exit if this is not true */

    /* query for mss */
    len = sizeof(mss);
    rc = getsockopt(inSock, IPPROTO_TCP, TCP_MAXSEG, (char *)&mss, &len);
    if (rc == -1) {
        printf("perror getsockopt TCP_MAXSEG");
        return -1;
    }
...
set_tcp_options(int sock, int no_delay, int mss)
{
    ...
    if (no_delay == 1) {
        ...
        if (rc == -1) {
            printf("perror setsockopt TCP_NODELAY");
            return -1;
        }
    }
#ifdef TCP_MAXSEG
    if (mss > 0) {
        ...
        if (rc == -1) {
            printf("perror setsockopt TCP_MAXSEG");
            return -1;
        }
        /* verify results */
        ...
        if (new_mss != mss) {
            printf("perror setsockopt value mismatch");
            return -1;
        }
    }
#endif
