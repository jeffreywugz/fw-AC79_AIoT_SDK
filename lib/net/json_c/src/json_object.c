        ...
        const char *format)
{
    ...
    if (isnan(jso->o.c_double)) {
        ...
        size = sprintf(buf, format, jso->o.c_double);
...
int json_object_string_set(struct json_object *jso, const char *s, int len)
{
    if (!jso) {
        return -EINVAL;
    }

    if (jso->o.c_string.len >= LEN_DIRECT_STRING_DATA && jso->o.c_string.str.ptr) {
        free(jso->o.c_string.str.ptr);
    ...
    if (len < LEN_DIRECT_STRING_DATA) {
        ...
        memcpy(jso->o.c_string.str.ptr, s, len);
        jso->o.c_string.str.ptr[len] = '\0';
    }
