...
static void *with_dtls;
void coap_with_dtls(void *priv)
{
    with_dtls = priv;
}
...
void coap_delete_recvqueue(coap_context_t *context, coap_address_t *peer)
{
    coap_queue_t *rcvd = context->recvqueue;
    coap_queue_t *prew = context->recvqueue;

    while (rcvd) {
        if (rcvd->remote.addr.sin.sin_addr.s_addr == peer->addr.sin.sin_addr.s_addr)
//        if(coap_address_equals(peer,&rcvd->remote))
        {
            coap_delete_node(rcvd);
            if (rcvd == context->recvqueue) {
                context->recvqueue = context->recvqueue->next;
                prew = context->recvqueue;
            } else {
                prew->next = rcvd->next;
                prew = rcvd = rcvd->next;
            }
...
void coap_delete_sendqueue(coap_context_t *context, coap_address_t *peer)
{
    coap_queue_t *node;

    node = coap_peek_next(context);
    while (node) {
        node = coap_pop_next(context);
        if (node->remote.addr.sin.sin_addr.s_addr == peer->addr.sin.sin_addr.s_addr)
//        if(coap_address_equals(peer,&node->remote))
        {
...
#ifdef SIMULATE_DROP_PKT
    if ((rand() % 5) == 0) {
        printf("|COAP SEND SIMULATE_DROP_PKT\n");
...
#ifdef WITH_DTLS
    if (with_dtls) {
        bytes_written = dtls_sendto(with_dtls, (u8 *)pdu->hdr, pdu->length, 0, &dst->addr.sa, dst->size);
    } else
#endif
    {
...
int coap_readfrom(coap_context_t *ctx, void *buf, size_t bytes_read, struct sockaddr *from, socklen_t fromlen)
{
    coap_hdr_t *pdu;
    ...
    memset(&src, 0, sizeof(coap_address_t));

    memcpy(&src.addr.sa, from, sizeof(struct sockaddr));
    src.size = fromlen;
...
#ifdef SIMULATE_DROP_PKT
    if ((rand() % 5) == 0) {
        printf("|COAP READ SIMULATE_DROP_PKT\n");
        return 0;
    }
#endif

    return coap_readfrom(ctx, buf, (size_t)bytes_read, &src.addr.sa, src.size);
}
...
get_wkc_len(coap_context_t *context, coap_opt_t *query_filter)
{
    ...
    debug("get_wkc_len: print_wellknown() returned %u\n", len);
