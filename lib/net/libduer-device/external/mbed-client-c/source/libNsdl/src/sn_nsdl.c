...
#ifndef USE_ET_PARAMETER
#define ET_PARAMETER_LEN                2
#else
...
#ifndef USE_ET_PARAMETER
static uint8_t et_parameter[]                   = {'t', '='};      /* Endpoint type */
#else
static uint8_t et_parameter[]                   = {'e', 't', '='};      /* Endpoint type */
#endif
static uint8_t bs_queue_mode[]                  = {'b', '='};

#ifndef DISABLE_CP_PARAMETER
static uint8_t cp_parameter[] = "cp=5683";
static uint32_t cp_parameter_size = sizeof(cp_parameter) - 1;
#endif
...
#else
            unregister_message_ptr->uri_path_len = (RESOURCE_DIR_LEN + 1 + handle->ep_information_ptr->domain_name_len);
#endif
            unregister_message_ptr->uri_path_ptr = handle->sn_nsdl_alloc(unregister_message_ptr->uri_path_len);
            if (!unregister_message_ptr->uri_path_ptr) {
                sn_coap_parser_release_allocated_coap_msg_mem(handle->grs->coap, unregister_message_ptr);
...
#ifdef UPDATE_REGISTRATION_WITH_EP_NAME
...
#else
        register_message_ptr->uri_path_len  =   sizeof(resource_path_ptr) + handle->ep_information_ptr->domain_name_len + 1;    /* = rd/domain */
#endif
...
#ifndef DISABLE_CP_PARAMETER
    return_value += cp_parameter_size;
    number_of_parameters ++;
#endif
...
static int8_t sn_nsdl_fill_uri_query_options(struct nsdl_s *handle, sn_nsdl_ep_parameters_s *parameter_ptr, sn_coap_hdr_s *source_msg_ptr, uint8_t msg_type)
{
    ...
    memcpy(temp_ptr, cp_parameter, cp_parameter_size);
    temp_ptr += cp_parameter_size;
#endif
