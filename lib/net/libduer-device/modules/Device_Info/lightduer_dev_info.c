...
static const struct DevInfoOps *s_dev_info_ops = NULL;
volatile static duer_mutex_t s_lock_dev_info = NULL;

int duer_register_device_info_ops(const struct DevInfoOps *ops)
{
...
void duer_unregister_device_info_ops(void)
{
    if (s_lock_dev_info) {
        duer_mutex_destroy(s_lock_dev_info);
        s_lock_dev_info = NULL;
    }
