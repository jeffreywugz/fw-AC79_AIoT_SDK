...
int duer_ota_uninit_decompression(void)
{
    for (int i = 0; i < DECOMPRESSION_MAX; i++) {
        if (s_decompression[i]) {
            duer_ota_unpacker_unregister_decompression(i);
            s_decompression[i] = NULL;
        }
    }

    if (s_lock_ota_decom) {
        duer_mutex_destroy(s_lock_ota_decom);
        s_lock_ota_decom = NULL;
    }
