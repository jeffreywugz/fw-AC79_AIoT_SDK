...
static int package_handler(void *private, char const *buf, size_t len)
{
    ...
    mbedtls_md5_update(&updater->md5_ctx, (const unsigned char *)buf, len);
...
void duer_app_ota_uninitialize(void)
{
    if (s_ota_updater_handler) {
        duer_events_destroy(s_ota_updater_handler);
        s_ota_updater_handler = NULL;
    }

    s_ota_init_ops = NULL;
    duer_unregister_device_info_ops();
    duer_ota_unregister_package_info_ops();
    duer_ota_uninit_decompression();
    duer_ota_uninit_installer();
    duer_uninit_ota_downloader();

    if (s_lock_ota_updater) {
        duer_mutex_destroy(s_lock_ota_updater);
        s_lock_ota_updater = NULL;
    }
}
