...
#define DUER_PAGE_SIZE                (1 << s_flash_config.page_align_bits)
    ...
    int payload_len)
{
    ...
    page_cache = (char *)DUER_MALLOC(DUER_PAGE_SIZE);
    memset(page_cache, 0, DUER_PAGE_SIZE);

    // Put header into cache.
    if (payload_string) {
        if (DUER_PAGE_SIZE - sizeof(duer_flash_data_header) >= payload_len) {
            i = payload_len;
        } else {
            i = DUER_PAGE_SIZE - sizeof(duer_flash_data_header);
        }
        memcpy(page_cache + sizeof(duer_flash_data_header), payload_string, i);
    } else {
        duer_flash_read(ctx, addr_start, page_cache, DUER_PAGE_SIZE);
    }
    memcpy(page_cache, p_flash_header, sizeof(duer_flash_data_header));
    // write header and some of payload
    duer_flash_write(ctx, addr_start, page_cache, DUER_PAGE_SIZE);
    ...
    if (payload_string && i < payload_len) {
        if (addr_start + sizeof(duer_flash_data_header) + payload_len > ctx->len) {
            part1_len = ctx->len - (addr_start + DUER_PAGE_SIZE);
            part2_len = addr_start + sizeof(duer_flash_data_header) + payload_len - ctx->len;
            if (part1_len > 0) {
                duer_flash_write(ctx, addr_start + DUER_PAGE_SIZE, payload_string + i, part1_len);
            }
            duer_flash_write(ctx, 0, payload_string + i + part1_len, part2_len);
        } else {
            duer_flash_write(ctx, addr_start + DUER_PAGE_SIZE, payload_string + i, payload_len - i);
        }
    }
