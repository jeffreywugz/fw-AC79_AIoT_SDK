...
void Init_LwIP(u8_t lwip_netif)
{
    ...
    if (lwip_netif == WIFI_NETIF) {
        netif_add(&wireless_netif, &ipaddr, &netmask, &gw, NULL, &wireless_ethernetif_init, (netif_input_fn)&tcpip_input);
    } else {
#ifdef HAVE_ETH_WIRE_NETIF
        netif_add(&wire_netif, &ipaddr, &netmask, &gw, NULL, &wired_ethernetif_init, (netif_input_fn)&tcpip_input);
#elif defined HAVE_BT_NETIF
        netif_add(&bt_netif, &ipaddr, &netmask, &gw, NULL, &bt_ethernetif_init, (netif_input_fn)&tcpip_input);
#endif
    }

#if DNS_LOCAL_HOSTLIST
    if (lwip_netif == WIFI_NETIF) {
        err = dns_local_addhost(LOCAL_WIRELESS_HOST_NAME, &wireless_netif.ip_addr);
    }
#ifdef HAVE_ETH_WIRE_NETIF
    else {
        err = dns_local_addhost(LOCAL_WIRE_HOST_NAME, &wire_netif.ip_addr);
    }
#elif defined HAVE_BT_NETIF
    else {
        err = dns_local_addhost(LOCAL_WIRE_HOST_NAME, &bt_netif.ip_addr);
    }
#endif
    if (err != ERR_OK) {
