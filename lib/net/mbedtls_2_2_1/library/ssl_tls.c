...
#endif
#endif /* MBEDTLS_SSL_PROTO_TLS1_2 */
/* unsigned char mac_enc_tmp[64]; */
/* unsigned char mac_dec_tmp[64]; */
int mbedtls_ssl_derive_keys(mbedtls_ssl_context *ssl)
{
    ...
    defined(MBEDTLS_SSL_PROTO_TLS1_2)
        if (ssl->minor_ver >= MBEDTLS_SSL_MINOR_VERSION_1) {
            ...
            memcpy(ssl->mac_dec_tmp, mac_dec, transform->maclen);
            memcpy(ssl->mac_enc_tmp, mac_enc, transform->maclen);
            /* printf("xueyong \n\n"); */
        } else
#endif
        {
    ...
    if (mode == MBEDTLS_MODE_STREAM ||
        ...
        defined(MBEDTLS_SSL_PROTO_TLS1_2)
            if (ssl->minor_ver >= MBEDTLS_SSL_MINOR_VERSION_1) {
                ...
                //mbedtls_md_init(&ssl->transform_out->md_ctx_enc);
                //mbedtls_md_starts(&ssl->transform_out->md_ctx_enc);

                mbedtls_md_hmac_starts(&ssl->transform_out->md_ctx_enc, ssl->mac_enc_tmp, ssl->transform_out->maclen);
    ...
    ( defined(MBEDTLS_AES_C) || defined(MBEDTLS_CAMELLIA_C) )
            if (mode == MBEDTLS_MODE_CBC) {
                ...
                if (ssl->session_in->encrypt_then_mac == MBEDTLS_SSL_ETM_ENABLED) {
                    ...
                    //printf("xueyong \n\n");
                    //mbedtls_md_init(&ssl->transform_in->md_ctx_dec);
                    //				mbedtls_md_starts(&ssl->transform_in->md_ctx_dec);
                    mbedtls_md_hmac_starts(&ssl->transform_in->md_ctx_dec, ssl->mac_dec_tmp, ssl->transform_in->maclen);
    ...
    if (auth_done == 0) {
        ...
        defined(MBEDTLS_SSL_PROTO_TLS1_2)
            if (ssl->minor_ver > MBEDTLS_SSL_MINOR_VERSION_0) {
                ...
                mbedtls_md_hmac_starts(&ssl->transform_in->md_ctx_dec, ssl->mac_dec_tmp, ssl->transform_in->maclen);
                //printf("aaaaaaaaaa\n");
                // mbedtls_md_hmac_starts(&ssl->transform_in->md_ctx_dec, mac_dec, transform->maclen);
                //mbedtls_md_init(&ssl->transform_in->md_ctx_dec);
                //mbedtls_md_starts(&ssl->transform_in->md_ctx_dec);
