...
#define RemSpc (MaxMsg-1-length)
    ...
    case kDNSType_SOA:
        mDNS_snprintf(buffer + length, RemSpc, "%##s %##s %d %d %d %d %d",
    ...
    case kDNSType_SRV:
        mDNS_snprintf(buffer + length, RemSpc, "%u %u %u %##s",
                      rd->srv.priority, rd->srv.weight, mDNSVal16(rd->srv.port), rd->srv.target.c);
        break;
        ...
        while (*cstr && *cstr != '.' && ptr < lim) {				// While we have characters in the label...
            ...
            if (c == '\\') {										// If escape character, check next character
                ...
                if (mDNSIsDigit(cstr[-1]) && mDNSIsDigit(cstr[0]) && mDNSIsDigit(cstr[1])) {
                    ...
                    if (val <= 255) {
                        c = (mDNSu8)val;    // If valid three-digit decimal value, use it
                        cstr += 2;
                    }
                }
            }
...
mDNSexport void ConvertUTF8PstringToRFC1034HostLabel(const mDNSu8 UTF8Name[], domainlabel *const hostlabel)
{
    ...
    while (src < end) {
        ...
        if (src[0] == '\'') {
            src++;    // Standard straight single quote
            continue;
        }
        if (src + 2 < end && src[0] == 0xE2 && src[1] == 0x80 && src[2] == 0x99) {
            src += 3;    // Unicode curly apostrophe
            continue;
        }
        if (ptr < lim) {
...
mDNSexport void AppendLabelSuffix(domainlabel *const name, mDNSu32 val, const mDNSBool RichText)
{
    ...
    if (RichText) while (name->c[name->c[0]] == ' ') {
            name->c[0]--;
        }
...
mDNSexport mDNSu32 RDataHashValue(const ResourceRecord *const rr)
{
    ...
    case kDNSType_NSEC:
        len = sizeof(rdataNSEC);	// Use in-memory length of 32, and fall through default checksum computation below
...
mDNSexport mDNSBool SameRDataBody(const ResourceRecord *const r1, const RDataBody *const r2, DomainNameComparisonFn *samename)
{
    ...
    case kDNSType_SOA:
        return (mDNSBool)(b1->soa.serial   == b2->soa.serial             &&
    ...
    case kDNSType_RT:
    case kDNSType_KX:
        return (mDNSBool)(b1->mx.preference == b2->mx.preference &&
                          samename(&b1->mx.exchange, &b2->mx.exchange));

    case kDNSType_RP:
        return (mDNSBool)(samename(&b1->rp.mbox, &b2->rp.mbox) &&
                          samename(&b1->rp.txt,  &b2->rp.txt));

    case kDNSType_PX:
        return (mDNSBool)(b1->px.preference == b2->px.preference          &&
    ...
    case kDNSType_NSEC:
        return (mDNSPlatformMemSame(b1->data, b2->data, sizeof(rdataNSEC)));
...
mDNSexport mDNSBool SameNameRecordAnswersQuestion(const ResourceRecord *const rr, const DNSQuestion *const q)
{
    ...
    if (!rr->InterfaceID && rr->rDNSServer != q->qDNSServer) {
        return (mDNSfalse);
    }
...
mDNSexport mDNSu16 GetRDLength(const ResourceRecord *const rr, mDNSBool estimate)
{
    ...
    if (rr->rrclass == kDNSQClass_ANY) {
        ...
        case kDNSType_RP:
            return (mDNSu16)(CompressedDomainNameLength(&rd->rp.mbox, name) +
        ...
        case kDNSType_NSEC: {
            int i;
            for (i = sizeof(rdataNSEC); i > 0; i--) if (rd->nsec.bitmap[i - 1]) {
                    break;
                }
    ...
    while (result >= base) {
        ...
        if (result[0] == domname[0] && result[1] == domname[1]) {
            ...
            while (targ + *name < end) {
                ...
                for (i = 0; i <= *name; i++) if (targ[i] != name[i]) {
                        break;
                    }
                if (i <= *name) {
...
mDNSexport mDNSu8 *putRData(const DNSMessage *const msg, mDNSu8 *ptr, const mDNSu8 *const limit, const ResourceRecord *const rr)
{
    ...
    case kDNSType_NSEC: {
        ...
        // followed by the 1-32 data bytes
        int i, j;
        for (i = sizeof(rdataNSEC); i > 0; i--) if (rdb->nsec.bitmap[i - 1]) {
                break;
            }
...
mDNSexport mDNSu8 *PutResourceRecordTTLWithLimit(DNSMessage *const msg, mDNSu8 *ptr, mDNSu16 *count, ResourceRecord *rr, mDNSu32 ttl, const mDNSu8 *limit)
{
    ...
    if (!ptr) {
        LogMsg("PutResourceRecordTTLWithLimit ptr is null");
        return (mDNSNULL);
    }
    ...
    if (!endofrdata) {
        verbosedebugf("Ran out of space in PutResourceRecord for %##s (%s)", rr->name->c, DNSTypeName(rr->rrtype));
        return (mDNSNULL);
    }
        ...
        const mDNSu8 *end, const mDNSInterfaceID InterfaceID, mDNSu8 RecordType, LargeCacheRecord *const largecr)
{
    ...
    if (rr->resrec.rrclass == kDNSQClass_ANY && pktrdlength == 0) {	// Used in update packets to mean "Delete An RRset" (RFC 2136)
        ...
        case kDNSType_A:
            if (pktrdlength != sizeof(mDNSv4Addr)) {
                goto fail;
            }
            rdb->ipv4.b[0] = ptr[0];
            rdb->ipv4.b[1] = ptr[1];
        ...
        case kDNSType_DNAME:
            ...
            if (ptr != end) {
                debugf("GetLargeResourceRecord: Malformed CNAME/PTR RDATA name");
                goto fail;
            }
        ...
        case kDNSType_SOA:
            ...
            if (!ptr)              {
                debugf("GetLargeResourceRecord: Malformed SOA RDATA mname");
                goto fail;
            }
            ptr = getDomainName(msg, ptr, end, &rdb->soa.rname);
            if (!ptr)              {
                debugf("GetLargeResourceRecord: Malformed SOA RDATA rname");
                goto fail;
            }
            if (ptr + 0x14 != end) {
                debugf("GetLargeResourceRecord: Malformed SOA RDATA");
                goto fail;
            }
        ...
        case kDNSType_RP:
            ptr = getDomainName(msg, ptr, end, &rdb->rp.mbox);	// Domainname + domainname
            if (!ptr)       {
                debugf("GetLargeResourceRecord: Malformed RP mbox");
                goto fail;
            }
            ptr = getDomainName(msg, ptr, end, &rdb->rp.txt);
            if (ptr != end) {
                debugf("GetLargeResourceRecord: Malformed RP txt");
                goto fail;
            }
            break;
        ...
        case kDNSType_PX:
            if (pktrdlength < 4) {
                goto fail;    // Preference + domainname + domainname
            }
            rdb->px.preference = (mDNSu16)((mDNSu16)ptr[0] <<  8 | ptr[1]);
            ptr = getDomainName(msg, ptr, end, &rdb->px.map822);
            if (!ptr)       {
                debugf("GetLargeResourceRecord: Malformed PX map822");
                goto fail;
            }
            ptr = getDomainName(msg, ptr, end, &rdb->px.mapx400);
            if (ptr != end) {
                debugf("GetLargeResourceRecord: Malformed PX mapx400");
                goto fail;
            }
            break;

        case kDNSType_AAAA:
            if (pktrdlength != sizeof(mDNSv6Addr)) {
                goto fail;
            }
        ...
        case kDNSType_SRV:
            if (pktrdlength < 7) {
                goto fail;    // Priority + weight + port + domainname
            }
            ...
            if (ptr != end) {
                debugf("GetLargeResourceRecord: Malformed SRV RDATA name");
                goto fail;
            }
            //debugf("%##s SRV %##s rdlen %d", rr->resrec.name.c, rdb->srv.target.c, pktrdlength);
            break;
        ...
        case kDNSType_OPT:	{
            ...
            while (ptr < end && (mDNSu8 *)(opt + 1) < &rr->resrec.rdata->u.data[StandardAuthRDSize]) {
                ...
                if (ptr + 4 > end) {
                    LogInfo("GetLargeResourceRecord: OPT RDATA ptr + 4 > end");
                    goto fail;
                }
                ...
                if (ptr + opt->optlen > end) {
                    LogInfo("GetLargeResourceRecord: ptr + opt->optlen > end");
                    goto fail;
                }
                switch (opt->opt) {
                case kDNSOpt_LLQ:
            ...
            if (ptr != end) {
                LogInfo("GetLargeResourceRecord: Malformed OptRdata");
                goto fail;
            }
            break;
        }
        ...
        case kDNSType_NSEC: {
            ...
            if (!ptr) {
                LogInfo("GetLargeResourceRecord: Malformed NSEC nextname");
                goto fail;
            }
            mDNSPlatformMemZero(rdb->nsec.bitmap, sizeof(rdb->nsec.bitmap));
            if (ptr < end) {
                if (*ptr++ != 0) {
                    debugf("GetLargeResourceRecord: We only handle block zero NSECs");
                    goto fail;
                }
                i = *ptr++;
                if (i > sizeof(rdataNSEC)) {
                    debugf("GetLargeResourceRecord: invalid block length %d", i);
                    goto fail;
                }
                for (j = 0; j < i; j++) {
                    rdb->nsec.bitmap[j] = *ptr++;
                }
            }
            if (ptr != end) {
                debugf("GetLargeResourceRecord: Malformed NSEC");
                goto fail;
            }
            break;
        }
                           ...
                           const mDNSAddr *dstaddr, mDNSIPPort dstport, const DNSMessage *const msg, const mDNSu8 *const end)
{
    ...
    if (!status) {
        ...
        tbuffer[mDNS_snprintf(tbuffer, sizeof(tbuffer), "ERROR %d %sing", status, sent ? "Send" : "Receiv")] = 0;
    }
    if (sent) {
                                      ...
                                      mDNSInterfaceID InterfaceID, UDPSocket *src, const mDNSAddr *dst, mDNSIPPort dstport, TCPSocket *sock, DomainAuthInfo *authInfo)
{
    ...
    if (!newend) { //LogMsg("mDNSSendDNSMessage: putHINFO failed msg %p end %p, limit %p", msg->data, end, limit); // Not fatal
        puts("mDNSSendDNSMessage: putHINFO failed msg\n");
    } else {
        end = newend;
    }
    ...
    if (!end) {
        puts("mDNSSendDNSMessage: DNSDigest_SignMessage failed\n");
        // LogMsg("mDNSSendDNSMessage: DNSDigest_SignMessage failed");
        status = mStatus_NoMemoryErr;
    } else {
        // Send the packet on the wire
//		puts("Send the packet on the wire\n");
        if (!sock) {
//			puts("mDNSPlatformSendUDP\n");
            status = mDNSPlatformSendUDP(m, msg, end, InterfaceID, src, dst, dstport);
        } else {
//			puts("mDNSPlatformWriteTCP\n");
...
//			put_u32hex0(nsent);
            if (nsent != 2) {
//			   puts("mDNSSendDNSMessage:  write msg length failed\n");
                ...
                if (nsent != msglen) {
                    puts("mDNSSendDNSMessage: write msg body failed\n");
            ...
            case 'a' :	{
                ...
                if (!a) {
                    ...
                    if (F.altForm && !F.precision) {
                        ...
                        case  4:
                            i = mDNS_snprintf(mDNS_VACB, sizeof(mDNS_VACB), "%d.%d.%d.%d",
                                              a[0], a[1], a[2], a[3]);
                            break;
                        case  6:
                        ...
                        case 16:
                            ...
                            break;
                        default:
                            i = mDNS_snprintf(mDNS_VACB, sizeof(mDNS_VACB), "%s", "<< ERROR: Must specify"
