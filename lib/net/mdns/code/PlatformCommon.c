...
mDNSlocal mDNSBool GetConfigOption(char *dst, const char *option, int *f)
{
...
//Z	FILE *f = fopen(filename, "r");
    ...
    if (f) {
        if (DomainDiscoveryDisabled && GetConfigOption(buf, "DomainDiscoveryDisabled", f) && !strcasecmp(buf, "true")) {
