...
mDNSlocal void FreeDNSServiceRegistration(mDNS_DirectOP_Register *x)
{
    ...
    if (x->s.SubTypes) {
        mDNSPlatformMemFree(x->s.SubTypes);
    }
...
mDNSlocal void RegCallback(mDNS *const m, ServiceRecordSet *const sr, mStatus result)
{
    ...
    if (!ConvertDomainLabelToCString_unescaped(&name, namestr)) {
        return;
    }
    if (!ConvertDomainNameToCString(&type, typestr)) {
        return;
    }
    if (!ConvertDomainNameToCString(&dom, domstr)) {
        return;
    }
...
)
{
    ...
    if (!name[0]) {
        n = mDNSStorage.nicelabel;
    } else if (!MakeDomainLabelFromLiteralString(&n, name))                              {
        errormsg = "Bad Instance Name";
        goto badparam;
    }
    if (!regtype || !*regtype || !MakeDomainNameFromDNSNameString(&t, regtype))        {
        errormsg = "Bad Service Type";
        goto badparam;
    }
    if (!MakeDomainNameFromDNSNameString(&d, (domain && *domain) ? domain : "local.")) {
        errormsg = "Bad Domain";
        goto badparam;
    }
    if (!MakeDomainNameFromDNSNameString(&h, (host   && *host) ? host   : ""))       {
        errormsg = "Bad Target Host";
        goto badparam;
    }
...
mDNSlocal void FoundInstance(mDNS *const m, DNSQuestion *question, const ResourceRecord *const answer, QC_result AddRecord)
{
    ...
    if (answer->rrtype != kDNSType_PTR) {
        LogMsg("FoundInstance: Should not be called with rrtype %d (not a PTR record)", answer->rrtype);
        return;
    }
...
static void DNSServiceResolveDispose(mDNS_DirectOP *op)
{
    ...
    if (x->qSRV.ThisQInterval >= 0) {
        mDNS_StopQuery(&mDNSStorage, &x->qSRV);
    }
    if (x->qTXT.ThisQInterval >= 0) {
        mDNS_StopQuery(&mDNSStorage, &x->qTXT);
    }
    mDNSPlatformMemFree(x);
}
...
)
{
    ...
    ////Check parameters
    if (!name[0]    || !MakeDomainLabelFromLiteralString(&n, name)) {
        errormsg = "Bad Instance Name";
        goto badparam;
    }
    if (!regtype[0] || !MakeDomainNameFromDNSNameString(&t, regtype)) {
    ...
    if (!domain[0]  || !MakeDomainNameFromDNSNameString(&d, domain)) {
        errormsg = "Bad Domain";
        goto badparam;
    }
    if (!ConstructServiceName(&srv, &n, &t, &d))                      {
    ...
    if (err) {
        ...
        errormsg = "mDNS_StartQuery qSRV";
        goto fail;
    }
    err = mDNS_StartQuery(&mDNSStorage, &x->qTXT);
    if (err) {
        DNSServiceResolveDispose((mDNS_DirectOP *)x);
        errormsg = "mDNS_StartQuery qTXT";
        goto fail;
    }
...
static void DNSServiceQueryRecordDispose(mDNS_DirectOP *op)
{
    ...
    if (x->q.ThisQInterval >= 0) {
        mDNS_StopQuery(&mDNSStorage, &x->q);
    }
    mDNSPlatformMemFree(x);
}
...
//}
//
//static void  DNSServiceGetAddrInfoResponse(
//	DNSServiceRef		inRef,
