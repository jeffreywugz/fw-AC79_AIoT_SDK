...
mDNSlocal AuthEntity *GetAuthEntity(AuthHash *r, const AuthGroup *const PreserveAG)
{
    ...
    if (!r->rrauth_free) {
        ...
        for (slot = 0; slot < AUTH_HASH_SLOTS; slot++) {
            ...
            while (*cp) {
                if ((*cp)->members || (*cp) == PreserveAG) {
                    cp = &(*cp)->next;
                } else {
                    ReleaseAuthGroup(r, cp);
                }
            }
        }
    ...
    if (r->rrauth_free) {	// If there are records in the free list, take one
        ...
        if (++r->rrauth_totalused >= r->rrauth_report) {
            ...
            if (r->rrauth_report <  100) {
                r->rrauth_report += 10;
            } else if (r->rrauth_report < 1000) {
                r->rrauth_report += 100;
            } else {
                r->rrauth_report += 1000;
            }
        }
...
mDNSlocal AuthGroup *GetAuthGroup(AuthHash *r, const mDNSu32 slot, const ResourceRecord *const rr)
{
    ...
    if (!ag) {
        LogMsg("GetAuthGroup: Failed to allocate memory for %##s", rr->name->c);
        return (mDNSNULL);
    }
    ...
    if (namelen > InlineCacheGroupNameSize) {
    ...
    if (AuthGroupForRecord(r, slot, rr)) {
        LogMsg("GetAuthGroup: Already have AuthGroup for %##s", rr->name->c);
    }
    r->rrauth_hash[slot] = ag;
    if (AuthGroupForRecord(r, slot, rr) != ag) {
        LogMsg("GetAuthGroup: Not finding AuthGroup for %##s", rr->name->c);
    }
...
mDNSexport AuthGroup *InsertAuthRecord(mDNS *const m, AuthHash *r, AuthRecord *rr)
{
    ...
    if (!ag) {
        ag = GetAuthGroup(r, slot, &rr->resrec);    // If we don't have a AuthGroup for this name, make one now
    }
    if (ag) {
...
mDNSexport AuthGroup *RemoveAuthRecord(mDNS *const m, AuthHash *r, AuthRecord *rr)
{
    ...
    if (!a) {
        LogMsg("RemoveAuthRecord: ERROR!! AuthGroup not found for %s", ARDisplayString(m, rr));
        return mDNSNULL;
    }
    rp = &(*ag)->members;
    while (*rp) {
...
mDNSexport mDNSBool mDNS_AddressIsLocalSubnet(mDNS *const m, const mDNSInterfaceID InterfaceID, const mDNSAddr *addr)
{
    ...
    if (addr->type == mDNSAddrType_IPv4) {
        ...
        if (mDNSv4AddressIsLinkLocal(&addr->ip.v4)) {
            return (mDNStrue);
        }
    ...
    if (addr->type == mDNSAddrType_IPv6) {
        if (mDNSv6AddressIsLinkLocal(&addr->ip.v6)) {
            return (mDNStrue);
        }
        for (intf = m->HostInterfaces; intf; intf = intf->next)
            if (intf->ip.type == addr->type && intf->InterfaceID == InterfaceID && intf->McastTxRx)
...
mDNSlocal NetworkInterfaceInfo *FirstInterfaceForID(mDNS *const m, const mDNSInterfaceID InterfaceID)
{
    ...
    while (intf && intf->InterfaceID != InterfaceID) {
        intf = intf->next;
    }
    return (intf);
}
...
mDNSlocal void GenerateNegativeResponse(mDNS *const m)
{
    ...
    if (!m->CurrentQuestion) {
        LogMsg("GenerateNegativeResponse: ERROR!! CurrentQuestion not set");
        return;
    }
    q = m->CurrentQuestion;
    ...
    if (m->CurrentQuestion == q) {
        q->ThisQInterval = 0;    // Deactivate this question
    }
...
mDNSlocal mDNSBool SameResourceRecordNameClassInterface(const AuthRecord *const r1, const AuthRecord *const r2)
{
    ...
    if (r1->resrec.InterfaceID &&
        ...
        r1->resrec.InterfaceID != r2->resrec.InterfaceID) {
        return (mDNSfalse);
    }
    return (mDNSBool)(
...
mDNSlocal mDNSBool PacketRRMatchesSignature(const CacheRecord *const pktrr, const AuthRecord *const authrr)
{
    ...
    if (pktrr->resrec.InterfaceID &&
        ...
        pktrr->resrec.InterfaceID != authrr->resrec.InterfaceID) {
        return (mDNSfalse);
    }
    if (!(authrr->resrec.RecordType & kDNSRecordTypeUniqueMask) || authrr->WakeUp.HMAC.l[0])
        if (pktrr->resrec.rrtype != authrr->resrec.rrtype) {
            return (mDNSfalse);
        }
    return (mDNSBool)(
...
mDNSlocal mDNSBool ShouldSuppressKnownAnswer(const CacheRecord *const ka, const AuthRecord *const rr)
{
    ...
    if (!IdenticalResourceRecord(&ka->resrec, &rr->resrec)) {
        return (mDNSfalse);
    }
...
mDNSlocal void SetTargetToHostName(mDNS *const m, AuthRecord *const rr)
{
    ...
    if (!target) {
        LogInfo("SetTargetToHostName: Don't know how to set the target of rrtype %s", DNSTypeName(rr->resrec.rrtype));
    }
...
mDNSexport mStatus mDNS_Register_internal(mDNS *const m, AuthRecord *const rr)
{
    ...
    if ((mDNSs32)rr->resrec.rroriginalttl <= 0) {
        LogMsg("mDNS_Register_internal: TTL %X should be 1 - 0x7FFFFFFF %s", rr->resrec.rroriginalttl, ARDisplayString(m, rr));
        return (mStatus_BadParamErr);
    }

    if (!rr->resrec.RecordType) {
        LogMsg("mDNS_Register_internal: RecordType must be non-zero %s", ARDisplayString(m, rr));
        return (mStatus_BadParamErr);
    }

    if (m->ShutdownTime) {
        LogMsg("mDNS_Register_internal: Shutting down, can't register %s", ARDisplayString(m, rr));
...
//	rr->LastMCInterface   = Set for us in InitializeLastAPTime()
    ...
    if (rr->WakeUp.HMAC.l[0] && !rr->AddressProxy.type) {
        rr->AnnounceCount = 2;
    }
...
mDNSexport mStatus mDNS_Deregister_internal(mDNS *const m, AuthRecord *const rr, mDNS_Dereg_type drt)
{
    ...
    if (*p) {
        ...
        if (drt == mDNS_Dereg_conflict) {	// If this was a conflict, see that all duplicates get the same treatment
            ...
            for (r2 = m->DuplicateRecords; r2; r2 = r2->next) if (RecordIsLocalDuplicate(r2, rr)) {
                    r2->ProbeCount = 0xFF;
                }
        } else {
            ...
            AuthRecord **d = &m->DuplicateRecords;
            while (*d && !RecordIsLocalDuplicate(*d, rr)) {
                d = &(*d)->next;
            }
            if (*d) {
                AuthRecord *dup = *d;
                ...
                *d        = dup->next;		// Cut replacement record from DuplicateRecords list
                if (RRLocalOnly(rr)) {
                    ...
                    if (!InsertAuthRecord(m, &m->rrauth, dup)) {
                        LogMsg("mDNS_Deregister_internal: ERROR!! cannot insert %s", ARDisplayString(m, dup));
                    }
                } else {
    ...
    if (rr->WakeUp.HMAC.l[0] ||
        ...
        if (!dupList && RRLocalOnly(rr)) {
            ...
            if (ag->NewLocalOnlyRecords == rr) {
                ag->NewLocalOnlyRecords = rr->next;
            }
        } else {
            *p = rr->next;					// Cut this record from the list
            if (m->NewLocalRecords == rr) {
                m->NewLocalRecords = rr->next;
            }
        }
        ...
        if (rr->NewRData) {
            CompleteRDataUpdate(m, rr);    // Update our rdata, clear the NewRData pointer, and return memory to the client
        }
...
mDNSlocal mStatus GetIPv4FromName(mDNSAddr *const a, const domainname *const name)
{
    ...
    if (skip < 0) {
        LogMsg("GetIPFromName: Need six labels in IPv4 reverse mapping name %##s", name);
        return mStatus_Invalid;
    }
    if (GetLabelDecimalValue(SkipLeadingLabels(name, skip + 3)->c, &a->ip.v4.b[0]) ||
        ...
        GetLabelDecimalValue(SkipLeadingLabels(name, skip + 0)->c, &a->ip.v4.b[3])) {
        return mStatus_Invalid;
    }
...
mDNSlocal mStatus GetIPv6FromName(mDNSAddr *const a, const domainname *const name)
{
    ...
    if (skip < 0) {
        LogMsg("GetIPFromName: Need 34 labels in IPv6 reverse mapping name %##s", name);
        return mStatus_Invalid;
    }
...
mDNSlocal mDNSs32 ReverseMapDomainType(const domainname *const name)
{
    ...
    if (skip >= 0) {
        ...
        if (SameDomainName(suffix, (const domainname *)"\x7" "in-addr" "\x4" "arpa")) {
            return mDNSAddrType_IPv4;
        }
        if (SameDomainName(suffix, (const domainname *)"\x3" "ip6"     "\x4" "arpa")) {
            return mDNSAddrType_IPv6;
        }
    }
                       ...
                       const mDNSv4Addr *const spa, const mDNSEthAddr *const tha, const mDNSv4Addr *const tpa, const mDNSEthAddr *const dst)
{
    ...
    NetworkInterfaceInfo *intf = FirstInterfaceForID(m, rr->resrec.InterfaceID);
    if (!intf) {
        LogMsg("SendARP: No interface with InterfaceID %p found %s", rr->resrec.InterfaceID, ARDisplayString(m, rr));
        return;
    }
    ...
    *ptr++ = 0x06;
    ...
    // 0x0E ARP header
    *ptr++ = 0x00;
    *ptr++ = 0x01;	// Hardware address space; Ethernet = 1
    *ptr++ = 0x08;
    *ptr++ = 0x00;	// Protocol address space; IP = 0x0800
    *ptr++ = 6;						// Hardware address length
    *ptr++ = 4;						// Protocol address length
    *ptr++ = 0x00;
    *ptr++ = op;		// opcode; Request = 1, Response = 2
                       ...
                       const mDNSv6Addr *const spa, const mDNSEthAddr *const tha, const mDNSv6Addr *const tpa, const mDNSEthAddr *const dst)
{
    ...
    NetworkInterfaceInfo *intf = FirstInterfaceForID(m, rr->resrec.InterfaceID);
    if (!intf) {
        LogMsg("SendNDP: No interface with InterfaceID %p found %s", rr->resrec.InterfaceID, ARDisplayString(m, rr));
        return;
    }
    ...
    *ptr++ = 0x00;
    *ptr++ = 0x00;
    *ptr++ = 0x00;		// Version, Traffic Class, Flow Label
    *ptr++ = 0x00;
    *ptr++ = 0x20;									// Length
    ...
    *ptr++ = 0x00;					// Code
    *ptr++ = 0x00;
    *ptr++ = 0x00;	// Checksum placeholder (0x38, 0x39)
    *ptr++ = flags;
    *ptr++ = 0x00;
    *ptr++ = 0x00;
    *ptr++ = 0x00;
...
{
    if (++rr->UpdateCredits >= kMaxUpdateCredits) {
        rr->NextUpdateCredit = 0;
    } else {
        rr->NextUpdateCredit = NonZeroTime(rr->NextUpdateCredit + kUpdateCreditRefreshInterval);
    }
}
...
mDNSlocal void SendResponses(mDNS *const m)
{
    ...
    for (rr = m->ResourceRecords; rr; rr = rr->next)
        if (rr->ImmedUnicast) {
            ...
            if (!mDNSIPv4AddressIsZero(rr->v4Requester)) {
                SendDelayedUnicastResponse(m, &v4, rr->ImmedAnswer);
            }
            if (!mDNSIPv6AddressIsZero(rr->v6Requester)) {
                SendDelayedUnicastResponse(m, &v6, rr->ImmedAnswer);
            }
            if (rr->ImmedUnicast) {
    ...
    for (rr = m->ResourceRecords; rr; rr = rr->next) {
        while (rr->NextUpdateCredit && m->timenow - rr->NextUpdateCredit >= 0) {
            GrantUpdateCredit(rr);
        }
                ...
                if (!rr->WakeUp.HMAC.l[0]) {
                    ...
                    for (r2 = rr; r2; r2 = r2->next)
                        if (r2->AnnounceCount && r2->resrec.InterfaceID == rr->resrec.InterfaceID && mDNSSameEthAddress(&r2->WakeUp.IMAC, &rr->WakeUp.IMAC)) {
                            ...
                            if (--r2->AnnounceCount <= GoodbyeCount) {
                                r2->WakeUp.HMAC = zeroEthAddr;
                            }
                        }
                }
    ...
    for (rr = m->ResourceRecords; rr; rr = rr->next) {
        ...
        if (rr->ImmedAnswer && rr->resrec.RecordType == kDNSRecordTypeShared && rr->resrec.rrtype == kDNSType_PTR)
            if (ResourceRecordIsValidAnswer(&m->DeviceInfo) && SameDomainLabel(rr->resrec.rdata->u.name.c, m->DeviceInfo.resrec.name->c)) {
                if (!m->DeviceInfo.ImmedAnswer) {
                    m->DeviceInfo.ImmedAnswer = rr->ImmedAnswer;
                } else {
                    m->DeviceInfo.ImmedAnswer = mDNSInterfaceMark;
                }
            }
    }
    ...
    while (intf) {
        ...
        for (rr = m->ResourceRecords; rr; rr = rr->next) {
            ...
            if ((rr->SendRNow == intf->InterfaceID) &&
                ...
                if (rr->NewRData && active) {
                    ...
                    if (ResourceRecordIsValidAnswer(rr) && rr->resrec.RecordType == kDNSRecordTypeShared && rr->RequireGoodbye) {
                        ...
                        if (newptr) {
                            ...
                            continue;    // If this packet is already too full to hold the goodbye for this record, skip it for now and we'll retry later
                        }
                    }
                ...
                if (newptr) {
                    ...
                    if (rr->resrec.RecordType == kDNSRecordTypeDeregistering) {
                        numDereg++;
                    } else if (rr->LastAPTime == m->timenow) {
                        numAnnounce++;
                    } else {
                        numAnswer++;
                    }
                ...
                for (r2 = m->ResourceRecords; r2; r2 = r2->next)
                    if (ResourceRecordIsValidAnswer(r2) && SameResourceRecordNameClassInterface(r2, rr)) {
                        if (r2->resrec.rrtype >= kDNSQType_ANY) {
                            LogMsg("Can't create NSEC for record %s", ARDisplayString(m, r2));
                            break;
                        } else {
                            nsec.rdatastorage.u.nsec.bitmap[r2->resrec.rrtype >> 3] |= 128 >> (r2->resrec.rrtype & 7);
                        }
                    }
                newptr = responseptr;
        ...
        if (m->omsg.h.numAnswers || m->omsg.h.numAdditionals) {
            ...
            if (OwnerRecordSpace) {
                ...
                if (newptr) {
                    responseptr = newptr;
                    LogSPS("SendResponses put   %s", ARDisplayString(m, &opt));
            ...
            if (intf->IPv4Available) {
                mDNSSendDNSMessage(m, &m->omsg, responseptr, intf->InterfaceID, mDNSNULL, &AllDNSLinkGroup_v4, MulticastDNSPort, mDNSNULL, mDNSNULL);
            }
            if (intf->IPv6Available) {
                mDNSSendDNSMessage(m, &m->omsg, responseptr, intf->InterfaceID, mDNSNULL, &AllDNSLinkGroup_v6, MulticastDNSPort, mDNSNULL, mDNSNULL);
            }
            if (!m->SuppressSending) {
                m->SuppressSending = NonZeroTime(m->timenow + (mDNSPlatformOneSecond + 9) / 10);
            }
            if (++pktcount >= 1000) {
...
mDNSlocal mStatus mDNS_Reconfirm_internal(mDNS *const m, CacheRecord *const rr, mDNSu32 interval)
{
    ...
    if (RRExpireTime(rr) - m->timenow > (mDNSs32)((interval * 4) / 3)) {
        ...
        if (!m->RandomReconfirmDelay) {
            m->RandomReconfirmDelay = 1 + mDNSRandom(0x3FFFFFFF);
        }
...
mDNSlocal const CacheRecord *CacheHasAddressTypeForName(mDNS *const m, const domainname *const name, const mDNSu32 namehash)
{
    ...
    while (cr && !RRTypeIsAddressType(cr->resrec.rrtype)) {
        cr = cr->next;
    }
    return (cr);
}
...
{
    int i;
    for (i = 0; i < DupSuppressInfoSize; i++) if (ds[i].Time - time < 0) {
            ds[i].InterfaceID = mDNSNULL;
        }
}
...
{
    int i;
    for (i = 0; i < DupSuppressInfoSize; i++) if (ds[i].InterfaceID == InterfaceID && ds[i].Time - time < 0) {
            ds[i].InterfaceID = mDNSNULL;
        }
}
...
mDNSlocal mDNSBool SuppressOnThisInterface(const DupSuppressInfo ds[DupSuppressInfoSize], const NetworkInterfaceInfo *const intf)
{
    ...
    for (i = 0; i < DupSuppressInfoSize; i++)
        if (ds[i].InterfaceID == intf->InterfaceID) {
            if (ds[i].Type == mDNSAddrType_IPv4) {
                v4 = mDNStrue;
            } else if (ds[i].Type == mDNSAddrType_IPv6) {
                v6 = mDNStrue;
            }
            if (v4 && v6) {
                return (mDNStrue);
...
mDNSlocal int RecordDupSuppressInfo(DupSuppressInfo ds[DupSuppressInfoSize], mDNSs32 Time, mDNSInterfaceID InterfaceID, mDNSs32 Type)
{
    ...
    for (i = 0; i < DupSuppressInfoSize; i++) if (ds[i].InterfaceID == InterfaceID && ds[i].Type == Type) {
            break;
        }
    ...
    if (TimeToSendThisQuestion(q, m->timenow + q->ThisQInterval / 2)) {
        ...
        for (rr = cg ? cg->members : mDNSNULL; rr; rr = rr->next)				// If we have a resource record in our cache,
            if (rr->resrec.rdlength <= SmallRecordLimit &&					// which is small enough to sensibly fit in the packet
                ...
                forecast += 12 + rr->resrec.rdestimate;
                if (forecast >= 512) {
                    return (mDNSfalse);    // If this would add 512 bytes or more to the packet, don't accelerate
                }
            }
        return (mDNStrue);
    }
...
mDNSlocal void SendQueries(mDNS *const m)
{
    ...
    while (m->CurrentRecord) {
        ...
        if (!AuthRecord_uDNS(ar) && ar->resrec.RecordType == kDNSRecordTypeUnique) {	// For all records that are still probing...
            ...
            // and update m->NextScheduledResponse so it will be announced
                ...
                if (!ar->Acknowledged) {
                    AcknowledgeRecord(m, ar);    // Defensive, just in case it got missed somehow
                }
    ...
    while (intf) {
        ...
        if (!KnownAnswerList) {
            ...
            // puts("==Put query questions in this packet==\n");
            ...
            // puts("==Put probe questions in this packet==\n");
        ...
        while (KnownAnswerList) {
            ...
            if (newptr) {
                //	printf("SendQueries:   Put %##s (%s) at %d - %d",
                ...
                if (m->omsg.h.numQuestions > 1)
                    //	printf("SendQueries:   Put %d answers; No more space for known answers", m->omsg.h.numAnswers);
                {
        ...
        for (ar = m->ResourceRecords; ar; ar = ar->next)
            if (ar->IncludeInProbe) {
                ...
                if (newptr) {
                    ...
                    LogMsg("SendQueries:   How did we fail to have space for the Update record %s", ARDisplayString(m, ar));
                }
            }
        ...
        if (queryptr > m->omsg.data) {
            ...
            if (intf->IPv4Available) {
//                puts("==mDNSSendDNSMessage==\n");
                mDNSSendDNSMessage(m, &m->omsg, queryptr, intf->InterfaceID, mDNSNULL, &AllDNSLinkGroup_v4, MulticastDNSPort, mDNSNULL, mDNSNULL);
            }
            if (intf->IPv6Available) {
                mDNSSendDNSMessage(m, &m->omsg, queryptr, intf->InterfaceID, mDNSNULL, &AllDNSLinkGroup_v6, MulticastDNSPort, mDNSNULL, mDNSNULL);
            }
            if (!m->SuppressSending) {
                m->SuppressSending = NonZeroTime(m->timenow + (mDNSPlatformOneSecond + 9) / 10);
            }
            ...
            // puts("queryptr > m->omsg.data\n");
        } else {	// Nothing more to send on this interface; go to next
//            puts("Nothing more to send on this interface; go to next\n");
            const NetworkInterfaceInfo *next = GetFirstActiveInterface(intf->next);
#if MDNS_DEBUGMSGS && 0
    ...
    for (q = m->Questions; q; q = q->next)
        if (q->SendQNow) {
            DNSQuestion *x;
            for (x = m->NewQuestions; x; x = x->next) if (x == q) {
                ...
                if (!q->qDNSServer && !q->DuplicateOf && rr->resrec.RecordType == kDNSRecordTypePacketNegative) {
                    ...
                    for (qptr = q->next ; qptr; qptr = qptr->next)
                        if (qptr->DuplicateOf == q) {
                            qptr->validDNSServers = q->validDNSServers;
                            qptr->qDNSServer = q->qDNSServer;
                        }
                }
                q->CurrentAnswers++;
                if (rr->resrec.rdlength > SmallRecordLimit) {
                    q->LargeAnswers++;
                }
...
mDNSlocal void ReleaseCacheRecord(mDNS *const m, CacheRecord *r)
{
    ...
    if (r->resrec.rdata && r->resrec.rdata != (RData *)&r->smallrdatastorage) {
...
mDNSlocal void CheckCacheExpiration(mDNS *const m, const mDNSu32 slot, CacheGroup *const cg)
{
    ...
    if (cg->rrcache_tail != rp) {
        verbosedebugf("CheckCacheExpiration: Updating CacheGroup tail from %p to %p", cg->rrcache_tail, rp);
    }
    cg->rrcache_tail = rp;
    m->lock_rrcache = 0;
}
...
mDNSlocal void AnswerNewQuestion(mDNS *const m)
{
    ...
    if (cg) {
        CheckCacheExpiration(m, slot, cg);
    }
    if (m->NewQuestions != q) {
        LogInfo("AnswerNewQuestion: Question deleted while doing CheckCacheExpiration");
        goto exit;
    }
    ...
    if (m->CurrentQuestion != q) {
        LogInfo("AnswerNewQuestion: Question deleted while while giving LocalOnly record answers");
        goto exit;
    }
    ...
    if (m->CurrentQuestion != q) {
        debugf("AnswerNewQuestion: Question deleted while giving negative answer");
        goto exit;
    }
...
mDNSlocal CacheEntity *GetCacheEntity(mDNS *const m, const CacheGroup *const PreserveCG)
{
    ...
    if (m->rrcache_free) {	// If there are records in the free list, take one
        ...
        if (++m->rrcache_totalused >= m->rrcache_report) {
            ...
            if (m->rrcache_report <  100) {
                m->rrcache_report += 10;
            } else if (m->rrcache_report < 1000) {
                m->rrcache_report += 100;
            } else {
                m->rrcache_report += 1000;
            }
        }
        mDNSPlatformMemZero(e, sizeof(*e));
    }
...
mDNSlocal CacheRecord *GetCacheRecord(mDNS *const m, CacheGroup *cg, mDNSu16 RDLength)
{
    ...
    if (r) {
        ...
        if (RDLength > InlineCacheRDSize) {		// If RDLength is too big, allocate extra storage
            ...
            if (r->resrec.rdata) {
                r->resrec.rdata->MaxRDLength = r->resrec.rdlength = RDLength;
            } else {
                ReleaseCacheEntity(m, (CacheEntity *)r);
                r = mDNSNULL;
            }
...
mDNSlocal CacheGroup *GetCacheGroup(mDNS *const m, const mDNSu32 slot, const ResourceRecord *const rr)
{
    ...
    if (!cg) {
        LogMsg("GetCacheGroup: Failed to allocate memory for %##s", rr->name->c);
        return (mDNSNULL);
    }
    ...
    if (CacheGroupForRecord(m, slot, rr)) {
        LogMsg("GetCacheGroup: Already have CacheGroup for %##s", rr->name->c);
    }
    m->rrcache_hash[slot] = cg;
    if (CacheGroupForRecord(m, slot, rr) != cg) {
        LogMsg("GetCacheGroup: Not finding CacheGroup for %##s", rr->name->c);
    }
...
mDNSexport mDNSs32 mDNS_TimeNow(const mDNS *const m)
{
    ...
    if (m->mDNS_busy) {
        ...
        if (!m->timenow) {
            LogMsg("mDNS_TimeNow: m->mDNS_busy is %ld but m->timenow not set", m->mDNS_busy);
        }
    }
...
mDNSexport mDNSs32 mDNS_Execute(mDNS *const m)
{
    ...
    if (m->timenow - m->NextScheduledEvent >= 0) {
        ...
        for (i = 0; m->NewQuestions && i < 1000; i++) {
            if (m->NewQuestions->DelayAnswering && m->timenow - m->NewQuestions->DelayAnswering < 0) {
                break;
            }
            AnswerNewQuestion(m);
        }
        if (i >= 1000) {
            LogMsg("mDNS_Execute: AnswerNewQuestion exceeded loop limit");
        }
        ...
        if (i >= 1000) {
            LogMsg("mDNS_Execute: m->LocalRemoveEvents exceeded loop limit");
        }
            ...
            if (LocalRecordReady(rr)) {
                ...
                if (*p) {
                    *p = rr->next;    // Cut this record from the list
                    ...
                    LogMsg("mDNS_Execute: ERROR!! Cannot find record %s in ResourceRecords list", ARDisplayString(m, rr));
                    break;
                }
                if (!head) {
                    while (*p) {
        ...
        if (i >= 1000) {
            LogMsg("mDNS_Execute: m->NewLocalRecords exceeded loop limit");
        }
                ...
                for (ag = m->rrauth.rrauth_hash[slot]; ag; ag = ag->next) {
                    for (i = 0; i < 100 && ag->NewLocalOnlyRecords; i++) {
                        ...
                        if (LocalRecordReady(rr)) {
                            ...
                            LogMsg("mDNS_Execute: LocalOnlyRecord %s not ready", ARDisplayString(m, rr));
                        }
                    }
                    // We limit about 100 per AuthGroup that can be serviced at a time
                    if (i >= 100) {
                        LogMsg("mDNS_Execute: ag->NewLocalOnlyRecords exceeded loop limit");
                    }
                }
        }
        ...
        if (m->mDNSPlatformStatus == mStatus_NoError && (m->SuppressSending == 0 || m->timenow - m->SuppressSending >= 0)) {
            ...
            if (m->timenow - m->NextScheduledQuery >= 0 || m->timenow - m->NextScheduledProbe >= 0) {
//                puts("------SendQueries----\n");
                SendQueries(m);
            }
        ...
        if (m->NextScheduledStopTime && m->timenow - m->NextScheduledStopTime >= 0) {
            TimeoutQuestions(m);
        }
#ifndef UNICAST_DISABLED
        if (m->NextSRVUpdate && m->timenow - m->NextSRVUpdate >= 0) {
            UpdateAllSRVRecords(m);
        }
...
mDNSlocal void ActivateUnicastQuery(mDNS *const m, DNSQuestion *const question, mDNSBool ScheduleImmediately)
{
    ...
    if (!question->DuplicateOf) {
        ...
        if (question->nta) {
            CancelGetZoneData(m, question->nta);
        ...
        CallbackBeforeStartQuery BeforeStartCallback, void *context)
{
    ...
    while (m->RestartQuestion) {
        ...
        if (q->qtype != kDNSType_A && q->qtype != kDNSType_AAAA && q->qtype != kDNSType_CNAME) {
            continue;
        }
        ...
        if (!SearchDomainsChanged || q->AppendSearchDomains) {
            ...
            if (!CacheRecordRmvEventsForQuestion(m, q)) {
                LogInfo("mDNSCoreRestartAddressQueries: Question deleted while delivering Cache Record RMV events");
                continue;
            }
            ...
            if (!LocalRecordRmvEventsForQuestion(m, q)) {
                LogInfo("mDNSCoreRestartAddressQueries: Question deleted while delivering Local Record RMV events");
                continue;
            }
...
exit:
    if (mDNSOpaque16IsZero(id) && intf->NextSPSAttempt < 8) {
        intf->NextSPSAttempt++;
    }
}
...
mDNSlocal mDNSBool RecordIsFirstOccurrenceOfOwner(mDNS *const m, const AuthRecord *const rr)
{
    ...
    for (ar = m->ResourceRecords; ar && ar != rr; ar = ar->next)
        if (mDNSPlatformMemSame(&rr->WakeUp, &ar->WakeUp, sizeof(rr->WakeUp))) {
            return mDNSfalse;
        }
    return mDNStrue;
}
...
mDNSlocal void NetWakeResolve(mDNS *const m, DNSQuestion *question, const ResourceRecord *const answer, QC_result AddRecord)
{
    ...
    if (answer->rrtype == kDNSType_SRV) {
        ...
        mDNS_Lock(m);
        if (sps == intf->NextSPSAttempt / 3) {
            SendSPSRegistration(m, intf, zeroID);    // If we're ready for this result, use it now
        }
        mDNS_Unlock(m);
...
mDNSlocal void BeginSleepProcessing(mDNS *const m)
{
    ...
    } else if (!mDNSCoreHaveAdvertisedMulticastServices(m)) {
                ...
                if (!sps[0]) LogSPS("BeginSleepProcessing: %-6s %#a No Sleep Proxy Server found (Next Browse Q in %d, interval %d)",
                    ...
                    for (i = 0; i < 3; i++) {
                        ...
                        if (intf->SPSAddr[i].type) {
                            LogMsg("BeginSleepProcessing: %s %d intf->SPSAddr[i].type %d", intf->ifname, i, intf->SPSAddr[i].type);
                            *(long *)0 = 0;
                        }
                        if (intf->NetWakeResolve[i].ThisQInterval >= 0) {
                            LogMsg("BeginSleepProcessing: %s %d intf->NetWakeResolve[i].ThisQInterval %d", intf->ifname, i, intf->NetWakeResolve[i].ThisQInterval);
                            *(long *)0 = 0;
                        }
#endif
                        intf->SPSAddr[i].type = mDNSAddrType_None;
                        if (intf->NetWakeResolve[i].ThisQInterval >= 0) {
                            mDNS_StopQuery(m, &intf->NetWakeResolve[i]);
                        }
    ...
    if (sleep && !m->SleepState) {	// Going to sleep
        ...
        for (rr = m->ResourceRecords; rr; rr = rr->next)
            if (AuthRecord_uDNS(rr)) {
                ...
                if (rr->resrec.RecordType == kDNSRecordTypeVerified && !rr->DependentOn) {
...
mDNSexport mDNSBool mDNSCoreReadyForSleep(mDNS *m, mDNSs32 now)
{
    ...
    if (m->SleepLimit - now > 0 && m->NextScheduledSPRetry - now > 0) {
        goto notready;
    }
    ...
    for (intf = GetFirstActiveInterface(m->HostInterfaces); intf; intf = GetFirstActiveInterface(intf->next))
        if (intf->NextSPSAttempt >= 0) {
            ...
            } else if (m->NextScheduledSPRetry - intf->NextSPSAttemptTime > 0) {
                m->NextScheduledSPRetry = intf->NextSPSAttemptTime;
            }
        }
    ...
    for (rr = m->ResourceRecords; rr; rr = rr->next)
        if (!AuthRecord_uDNS(rr))
            if (!mDNSOpaque16IsZero(rr->updateid)) {
                LogSPS("mDNSCoreReadyForSleep: waiting for SPS Update ID %d %s", mDNSVal16(rr->updateid), ARDisplayString(m, rr));
                goto spsnotready;
            }
    ...
    for (rr = m->ResourceRecords; rr; rr = rr->next)
        if (AuthRecord_uDNS(rr)) {
            if (rr->state == regState_Refresh && rr->tcp) {
                LogSPS("mDNSCoreReadyForSleep: waiting for Record Update ID %d %s", mDNSVal16(rr->updateid), ARDisplayString(m, rr));
                goto notready;
            }
#if APPLE_OSX_mDNSResponder
            if (!RecordReadyForSleep(m, rr)) {
                LogSPS("mDNSCoreReadyForSleep: waiting for %s", ARDisplayString(m, rr));
                goto notready;
            }
#endif
        }
        ...
        for (i = 0; i < query->h.numQuestions; i++) {					// For each question...
            ...
            for (rr = ResponseRecords; rr; rr = rr->NextResponse) {	// and search our list of proposed answers
                if (rr->NR_AnswerTo == ptr) {						// If we're going to generate a record answering this question
                    ...
                    if (!responseptr) {
                        debugf("GenerateUnicastResponse: Ran out of space for questions!");
                        return (mDNSNULL);
                    }
    ...
    for (rr = ResponseRecords; rr; rr = rr->NextResponse)
        if (rr->NR_AnswerTo) {
            ...
            if (p) {
                ...
                debugf("GenerateUnicastResponse: Ran out of space for answers!");
                response->h.flags.b[0] |= kDNSFlag0_TC;
            }
        }
    ...
    for (rr = ResponseRecords; rr; rr = rr->NextResponse)
        if (rr->NR_AdditionalTo && !rr->NR_AnswerTo) {
            ...
            if (p) {
                ...
                debugf("GenerateUnicastResponse: No more space for additionals");
            }
        }
...
mDNSlocal int CompareRData(const AuthRecord *const our, const CacheRecord *const pkt)
{
    ...
    while (ourptr < ourend && pktptr < pktend && *ourptr == *pktptr) {
        ourptr++;
        pktptr++;
    }
    if (ourptr >= ourend && pktptr >= pktend) {
...
mDNSlocal mDNSBool PacketRRConflict(const mDNS *const m, const AuthRecord *const our, const CacheRecord *const pktrr)
{
    ...
    if (!(our->resrec.RecordType & kDNSRecordTypeUniqueMask)) {
        return (mDNSfalse);
    }

    // If a dependent record, not a conflict
    if (our->DependentOn || MatchDependentOn(m, pktrr, our)) {
        return (mDNSfalse);
    } else {
    ...
    for (i = 0; i < query->h.numAuthorities; i++) {
        ...
        if (m->rec.r.resrec.RecordType != kDNSRecordTypePacketNegative && ResourceRecordAnswersQuestion(&m->rec.r.resrec, q)) {
            ...
            if (PacketRRConflict(m, our, &m->rec.r)) {
                ...
                if (!result) {
                    result = (int)our->resrec.rrtype  - (int)m->rec.r.resrec.rrtype;
                }
                if (!result) {
                    result = CompareRData(our, &m->rec.r);
                }
                if (result) {
...
mDNSlocal CacheRecord *FindIdenticalRecordInCache(const mDNS *const m, const ResourceRecord *const pktrr)
{
    ...
    for (rr = cg ? cg->members : mDNSNULL; rr; rr = rr->next) {
        ...
        if (match && IdenticalSameNameRecord(pktrr, &rr->resrec)) {
            break;
        }
    }
    return (rr);
            ...
            for (rr = m->ResourceRecords; rr; rr = rr->next) {
                ...
                if (rr->ImmedAnswer == InterfaceID && ShouldSuppressKnownAnswer(&m->rec.r, rr)) {
                    if (srcaddr->type == mDNSAddrType_IPv4) {
                        if (mDNSSameIPv4Address(rr->v4Requester, srcaddr->ip.v4)) {
                            rr->v4Requester = zerov4Addr;
                        }
                    } else if (srcaddr->type == mDNSAddrType_IPv6) {
                        if (mDNSSameIPv6Address(rr->v6Requester, srcaddr->ip.v6)) {
                            rr->v6Requester = zerov6Addr;
                        }
                    }
    ...
    for (rr = ResponseRecords; rr; rr = rr->NextResponse) {
        if (rr->NR_AnswerTo) {
            ...
            if (m->timenow - (rr->LastMCTime + TicksTTL(rr) / 4) >= 0) {
                ...
                if (rr->NR_AnswerTo == (mDNSu8 *)~1) {
                    rr->NR_AnswerTo = (mDNSu8 *)~0;
                }
            }

            // If the client insists on a multicast response, then we'd better send one
            if (rr->NR_AnswerTo == (mDNSu8 *)~0) {
                SendMulticastResponse = mDNStrue;
            } else if (rr->NR_AnswerTo == (mDNSu8 *)~1) {
            ...
            if (query->h.flags.b[0] & kDNSFlag0_TC) {
                delayresponse = mDNSPlatformOneSecond * 20;    // Divided by 50 = 400ms
...
mDNSlocal mDNSBool TrustedSource(const mDNS *const m, const mDNSAddr *const srcaddr)
{
    ...
    for (s = m->DNSServers; s; s = s->next)
        if (mDNSSameAddress(srcaddr, &s->addr)) {
            return (mDNStrue);
        }
    return (mDNSfalse);
}
...
mDNSexport CacheRecord *CreateNewCacheEntry(mDNS *const m, const mDNSu32 slot, CacheGroup *cg, mDNSs32 delay)
{
    ...
    if (!m->rec.r.resrec.InterfaceID) {
        debugf("CreateNewCacheEntry %s", CRDisplayString(m, &m->rec.r));
    }
    ...
    if (!cg) {
        cg = GetCacheGroup(m, slot, &m->rec.r.resrec);    // If we don't have a CacheGroup for this name, make one now
    }
    if (cg) {
        rr = GetCacheRecord(m, cg, RDLength);    // Make a cache record, being careful not to recycle cg
    }
    if (!rr) {
...
mDNSlocal mDNSu32 GetEffectiveTTL(const uDNS_LLQType LLQType, mDNSu32 ttl)		// TTL in seconds
{
    if (LLQType == uDNS_LLQ_Entire) {
        ...
        if (ttl > 0x60000000UL / mDNSPlatformOneSecond) {
            ttl = 0x60000000UL / mDNSPlatformOneSecond;
        }
                                       ...
                                       const mDNSInterfaceID InterfaceID)
{
    ...
    for (i = 0; i < totalrecords && ptr && ptr < end; i++) {
        ...
        if (!ptr) {
            goto exit;    // Break out of the loop and clean up our CacheFlushRecords list before exiting
        }
        if (m->rec.r.resrec.RecordType == kDNSRecordTypePacketNegative) {
        ...
        if (!ResponseMCast) {
            if (LLQType) {
                ...
                if (llqMatch != mDNSNULL) {
                    m->rec.r.resrec.rDNSServer = uDNSServer = llqMatch->qDNSServer;
                }
        ...
        if (mDNSOpaque16IsZero(response->h.id) && m->rec.r.resrec.rrtype != kDNSType_NSEC) {
            ...
            while (m->CurrentRecord) {
                ...
                if (!AcceptableResponse && !(ResponseSrcLocal && rr->resrec.RecordType == kDNSRecordTypeUnique)) {
                    continue;
                }
                ...
                if (PacketRRMatchesSignature(&m->rec.r, rr)) {	// If interface, name, type (if shared record) and class match...
                    ...
                    if (IdenticalSameNameRecord(&m->rec.r.resrec, &rr->resrec)) {
                        ...
                        if (m->rec.r.resrec.rroriginalttl >= rr->resrec.rroriginalttl / 2 || m->SleepState) {
                            ...
                            if (rr->ImmedAnswer == InterfaceID) {
                                rr->ImmedAnswer = mDNSNULL;
                                rr->ImmedUnicast = mDNSfalse;
                            }
                        } else {
                            if (rr->ImmedAnswer == mDNSNULL)    {
                                rr->ImmedAnswer = InterfaceID;
                                m->NextScheduledResponse = m->timenow;
                            } else if (rr->ImmedAnswer != InterfaceID) {
                                rr->ImmedAnswer = mDNSInterfaceMark;
                                m->NextScheduledResponse = m->timenow;
                            }
                        }
        ...
        if (!AcceptableResponse) {
            LogInfo("mDNSCoreReceiveResponse ignoring %s", CRDisplayString(m, &m->rec.r));
        }
            ...
            for (rr = cg ? cg->members : mDNSNULL; rr; rr = rr->next) {
                ...
                if (match && IdenticalSameNameRecord(&m->rec.r.resrec, &rr->resrec)) {
                    ...
                    if (m->rec.r.resrec.RecordType & kDNSRecordTypePacketUniqueMask) {
                        ...
                        if (!(rr->resrec.RecordType & kDNSRecordTypePacketUniqueMask)) {
                            DNSQuestion *q;
                            for (q = m->Questions; q; q = q->next) if (ResourceRecordAnswersQuestion(&rr->resrec, q)) {
                                    q->UniqueAnswers++;
                                }
...
exit:
    ...
    while (CacheFlushRecords != (CacheRecord *)1) {
        ...
        if (r1->DelayDelivery) {	// If we were planning to delay delivery of this record, see if we still need to
            ...
            if (!r1->DelayDelivery) {
                CacheRecordDeferredAdd(m, r1);
            } else {
                ScheduleNextCacheCheckTime(m, slot, r1->DelayDelivery);
            }
        }
    }
        ...
        if (ptr && (qptr = ExpectingUnicastResponseForQuestion(m, dstport, response->h.id, &q, !dstaddr))) {
            ...
            for (rr = cg ? cg->members : mDNSNULL; rr; rr = rr->next)
                if (SameNameRecordAnswersQuestion(&rr->resrec, qptr)) {
                    ...
                    if (RRExpireTime(rr) - m->timenow > 0) {
                        break;
                    }
            ...
            if (!InterfaceID && q.qtype != kDNSType_SOA && IsLocalDomain(&q.qname)) {
                ...
                if (qptr->AppendSearchDomains && !rr) {
                    ...
                    LogInfo("mDNSCoreReceiveResponse: Skipping check to see if we need to generate a negative cache entry for %##s (%s)", q.qname.c, DNSTypeName(q.qtype));
                }
            } else {
                if (!rr) {
                    ...
                    if (q.qtype == kDNSType_SOA && SameDomainName(&q.qname, &localdomain)) {
                        negttl = 60 * 60 * 24;
                    }
...
{
    if (!e->l[0]) {
        LogMsg("ScheduleWakeup ERROR: Target HMAC is zero");
        return;
    }
                                     ...
                                     const mDNSInterfaceID InterfaceID)
{
    ...
    if (!InterfaceID || !m->SPSSocket || !mDNSSameIPPort(dstport, m->SPSSocket->port)) {
        return;
    }
    ...
    if (ptr) {
        ...
        if (ptr && m->rec.r.resrec.RecordType != kDNSRecordTypePacketNegative && m->rec.r.resrec.rrtype == kDNSType_OPT) {
            ...
            for (o = &m->rec.r.resrec.rdata->u.opt[0]; o < e; o++) {
                ...
                } else if (o->opt == kDNSOpt_Owner && o->u.owner.vers == 0) {
                    owner       = o->u.owner;
                }
            }
        }
            ...
            if (ptr && m->rec.r.resrec.RecordType != kDNSRecordTypePacketNegative) {
                ...
                if (!ar) {
                    ...
                    if (m->rec.r.resrec.rrtype == kDNSType_PTR) {
                        ...
                        if (t == mDNSAddrType_IPv4) {
                            GetIPv4FromName(&ar->AddressProxy, m->rec.r.resrec.name);
                        } else if (t == mDNSAddrType_IPv6) {
                            GetIPv6FromName(&ar->AddressProxy, m->rec.r.resrec.name);
                        }
                        debugf("mDNSCoreReceiveUpdate: PTR %d %d %#a %s", t, ar->AddressProxy.type, &ar->AddressProxy, ARDisplayString(m, ar));
                        if (ar->AddressProxy.type) {
                    ...
                    // new ones for sleeping clients, so we'll we send deletions for our SPS clients' AAAA records.
                    if (m->KnownBugs & mDNS_KnownBug_LimitedIPv6)
                        if (ar->resrec.rrtype == kDNSType_AAAA) {
                            ar->resrec.rroriginalttl = 0;
                        }
                    m->ProxyRecords++;
                    mDNS_UpdateAllowSleep(m);
    ...
    if (p) {
        mDNSSendDNSMessage(m, &m->omsg, p, InterfaceID, m->SPSSocket, srcaddr, srcport, mDNSNULL, mDNSNULL);
    }
}
...
#ifdef _LEGACY_NAT_TRAVERSAL_
    else if (m->SSDPSocket && mDNSSameIPPort(dstport, m->SSDPSocket->port)) {
        debugf("Ignoring SSDP response from %#a:%d", srcaddr, mDNSVal16(srcport));
        return;
    }
#endif
...
//        puts("DNS Message fromlength  too short");
    ...
    if (!m) {
        puts("mDNSCoreReceive ERROR m is NULL");
        return;
    }
...
//		puts("Unknown DNS packet type\n");
        ...
        if (mDNS_LoggingEnabled) {
            ...
            while (i < end - (mDNSu8 *)pkt) {
                ...
                do if (i < end - (mDNSu8 *)pkt) {
                        p += mDNS_snprintf(p, sizeof(buffer), " %02X", ((mDNSu8 *)pkt)[i]);
                    }
                while (++i & 15);
                LogInfo("%s", buffer);
            }
    ...
    while (*p) {	// Check if we already have this {interface, domain} tuple registered
        if ((*p)->interface == interface && SameDomainName(&(*p)->domain, d)) {
            if (!((*p)->flags & DNSServer_FlagDelete)) {
                LogMsg("Note: Mcast Resolver domain %##s (%p) registered more than once", d->c, interface);
            }
                                 ...
                                 int bestcount)
{
    ...
    if (namecount < newcount || newcount < bestcount) {
        return -1;
    }
...
mDNSexport mDNSBool DomainEnumQuery(const domainname *qname)
{
    ...
    if (CountLabels(qname) < 4) {
        debugf("DomainEnumQuery: question %##s, not enough labels", qname->c);
        return mDNSfalse;
    }

    label = (const mDNSu8 *)d;
    while (mDNS_DEQLabels[i] != (const mDNSu8 *)mDNSNULL) {
        if (SameDomainLabel(mDNS_DEQLabels[i], label)) {
...
mDNSlocal mDNSBool ShouldSuppressQuery(mDNS *const m, domainname *qname, mDNSu16 qtype, mDNSInterfaceID InterfaceID)
{
    ...
    if (qtype == kDNSType_A) {
        iptype = mDNSAddrType_IPv4;
    } else if (qtype == kDNSType_AAAA) {
        iptype = mDNSAddrType_IPv6;
    } else {
        LogInfo("ShouldSuppressQuery: Query not suppressed for %##s, qtype %s, not A/AAAA type", qname, DNSTypeName(qtype));
        return mDNSfalse;
    }
    ...
    // check here.
    if (InterfaceID != mDNSInterface_Unicast && IsLocalDomain(qname)) {
        LogInfo("ShouldSuppressQuery: Query not suppressed for %##s, qtype %s, Local question", qname, DNSTypeName(qtype));
        return mDNSfalse;
    }
    ...
    if (AuthInfo && !AuthInfo->deltime && AuthInfo->AutoTunnel) {
        LogInfo("ShouldSuppressQuery: Query not suppressed for %##s, qtype %s, Private Domain", qname, DNSTypeName(qtype));
        return mDNSfalse;
    }
    ...
    for (rr = cg ? cg->members : mDNSNULL; rr; rr = rr->next) {
        ...
        if (SameNameRecordAnswersQuestion(&rr->resrec, q)) {
            ...
            if (rr->CRActiveQuestion == q) {
                ...
                if (!qptr) {
                    m->rrcache_active--;    // If no longer active, decrement rrcache_active count
                }
            }
            AnswerCurrentQuestionWithResourceRecord(m, rr, QC_rmv);
            if (m->CurrentQuestion != q) {
...
mDNSlocal mDNSBool CacheRecordRmvEventsForQuestion(mDNS *const m, DNSQuestion *q)
{
    ...
    if (!IsQuestionNew(m, q) && !q->LOAddressAnswers) {
        ...
        LogInfo("CacheRecordRmvEventsForQuestion: Question %p %##s (%s) is a new question", q, q->qname.c, DNSTypeName(q->qtype));
    }
    return mDNStrue;
}
        ...
        if (!mDNSOpaque16IsZero(q->TargetQID) && q->SuppressUnusable) {
            ...
            if (q->SuppressQuery != old) {
                ...
                if (q->SuppressQuery) {
                    ...
                    q->SuppressQuery = mDNSfalse;
                    if (!CacheRecordRmvEventsForQuestion(m, q)) {
                        LogInfo("CheckSuppressUnusableQuestions: Question deleted while delivering RMV events");
                        continue;
                    }
                    q->SuppressQuery = mDNStrue;
                }
...
mDNSexport mStatus mDNS_StartQuery_internal(mDNS *const m, DNSQuestion *const question)
{
    ...
    if (!question->Target.type) {
        question->TargetPort = zeroIPPort;    // If no question->Target specified clear TargetPort
    }
...
mDNSexport mStatus mDNS_StopQuery_internal(mDNS *const m, DNSQuestion *const question)
{
    ...
    if (m->NewLocalOnlyQuestions == question) {
    ...
    if (question->LocalSocket) {
        mDNSPlatformUDPClose(question->LocalSocket);
        question->LocalSocket = mDNSNULL;
    }
...
mDNSexport mStatus mDNS_StopQueryWithRemoves(mDNS *const m, DNSQuestion *const question)
{
    ...
    for (qq = m->NewQuestions; qq; qq = qq->next) if (qq == question) {
            break;
        }
...
mDNSexport mStatus mDNS_Reconfirm(mDNS *const m, CacheRecord *const cr)
{
    ...
    if (status == mStatus_NoError) {
        ReconfirmAntecedents(m, cr->resrec.name, cr->resrec.namehash, 0);
    }
    mDNS_Unlock(m);
    return (status);
}
...
mDNSlocal mDNSBool MachineHasActiveIPv6(mDNS *const m)
{
    NetworkInterfaceInfo *intf;
    for (intf = m->HostInterfaces; intf; intf = intf->next)
        if (intf->ip.type == mDNSAddrType_IPv6) {
            return (mDNStrue);
        }
    return (mDNSfalse);
}

static void FoundServiceInfoSRV(mDNS *const m, DNSQuestion *question, const ResourceRecord *const answer, QC_result AddRecord)
{
    ...
    if (!query->GotSRV) {
        ...
        if (MachineHasActiveIPv6(m)) {
            mDNS_StartQuery(m, &query->qAv6);
        }
    }
...
mDNSlocal void FoundServiceInfoTXT(mDNS *const m, DNSQuestion *question, const ResourceRecord *const answer, QC_result AddRecord)
{
    ...
    if (answer->rdlength > sizeof(query->info->TXTinfo)) {
        return;
    }
...
mDNSexport void    mDNS_StopResolveService(mDNS *const m, ServiceInfoQuery *q)
{
    ...
    if (q->qSRV.ThisQInterval >= 0) {
        mDNS_StopQuery_internal(m, &q->qSRV);
    }
    if (q->qTXT.ThisQInterval >= 0) {
        mDNS_StopQuery_internal(m, &q->qTXT);
    }
    if (q->qAv4.ThisQInterval >= 0) {
        mDNS_StopQuery_internal(m, &q->qAv4);
    }
    if (q->qAv6.ThisQInterval >= 0) {
        mDNS_StopQuery_internal(m, &q->qAv6);
    }
    mDNS_Unlock(m);
}
                                   ...
                                   const mDNSInterfaceID InterfaceID, mDNSQuestionCallback *Callback, void *Context)
{
    ...
    question->QuestionCallback = Callback;
    question->QuestionContext  = Context;
    if (DomainType > mDNS_DomainTypeMax) {
        return (mStatus_BadParamErr);
    }
    if (!MakeDomainNameFromDNSNameString(&question->qname, mDNS_DomainTypeNames[DomainType])) {
        return (mStatus_BadParamErr);
    }
    if (!dom) {
        dom = &localdomain;
    }
    if (!AppendDomainName(&question->qname, dom)) {
        return (mStatus_BadParamErr);
    }
                               ...
                               const mDNSu16 newrdlength, RData *const newrdata, mDNSRecordUpdateCallback *Callback)
{
    ...
    if (RRLocalOnly(rr) || (rr->resrec.rroriginalttl == newttl &&
        ...
        if (!rr->UpdateBlocked && rr->UpdateCredits) {
            rr->UpdateCredits--;
        }
        if (!rr->NextUpdateCredit) {
            rr->NextUpdateCredit = NonZeroTime(m->timenow + kUpdateCreditRefreshInterval);
        }
        if (rr->AnnounceCount > rr->UpdateCredits + 1) {
            rr->AnnounceCount = (mDNSu8)(rr->UpdateCredits + 1);
        }
        if (rr->UpdateCredits <= 5) {
            mDNSu32 delay = 6 - rr->UpdateCredits;		// Delay 1 second, then 2, then 3, etc. up to 6 seconds maximum
            if (!rr->UpdateBlocked) {
                rr->UpdateBlocked = NonZeroTime(m->timenow + (mDNSs32)delay * mDNSPlatformOneSecond);
            }
            rr->ThisAPInterval *= 4;
...
mDNSlocal void AdvertiseInterface(mDNS *const m, NetworkInterfaceInfo *set)
{
    ...
    if (!primary) {
        primary = set;    // If no existing advertised interface, this new NetworkInterfaceInfo becomes our new primary
    }
...
mDNSlocal void DeadvertiseInterface(mDNS *const m, NetworkInterfaceInfo *set)
{
    ...
    if (set->RR_A.    resrec.RecordType) {
        mDNS_Deregister_internal(m, &set->RR_A,     mDNS_Dereg_normal);
    }
    if (set->RR_PTR.  resrec.RecordType) {
        mDNS_Deregister_internal(m, &set->RR_PTR,   mDNS_Dereg_normal);
    }
    if (set->RR_HINFO.resrec.RecordType) {
        mDNS_Deregister_internal(m, &set->RR_HINFO, mDNS_Dereg_normal);
    }
}
...
mDNSexport void mDNS_SetFQDN(mDNS *const m)
{
    ...
    if (!AppendDomainLabel(&newmname, &m->hostlabel))  {
        puts("ERROR: mDNS_SetFQDN: Cannot create MulticastHostname");
        return;
    }
    if (!AppendLiteralLabelString(&newmname, "local")) {
        puts("ERROR: mDNS_SetFQDN: Cannot create MulticastHostname");
        return;
    }
    ...
    if (SameDomainNameCS(&m->MulticastHostname, &newmname)) {
        puts("mDNS_SetFQDN - hostname unchanged");
    } else {
//        puts("AssignDomainName\n");
...
//		    puts("AdvertiseInterface");
        }

    }

    // 3. Make sure that any AutoTarget SRV records (and the like) get updated
    for (rr = m->ResourceRecords;  rr; rr = rr->next) if (rr->AutoTarget) {
            SetTargetToHostName(m, rr);
        }
    for (rr = m->DuplicateRecords; rr; rr = rr->next) if (rr->AutoTarget) {
            SetTargetToHostName(m, rr);
        }
...
mDNSlocal void UpdateInterfaceProtocols(mDNS *const m, NetworkInterfaceInfo *active)
{
    ...
    for (intf = m->HostInterfaces; intf; intf = intf->next)
        if (intf->InterfaceID == active->InterfaceID) {
            if (intf->ip.type == mDNSAddrType_IPv4 && intf->McastTxRx) {
                active->IPv4Available = mDNStrue;
            }
            if (intf->ip.type == mDNSAddrType_IPv6 && intf->McastTxRx) {
                active->IPv6Available = mDNStrue;
            }
        }
}
...
mDNSexport void mDNS_DeactivateNetWake_internal(mDNS *const m, NetworkInterfaceInfo *set)
{
    ...
    if (set->NetWakeBrowse.ThisQInterval >= 0) {
        ...
        for (i = 0; i < 3; i++) if (set->NetWakeResolve[i].ThisQInterval >= 0) {
                mDNS_StopQuery_internal(m, &set->NetWakeResolve[i]);
            }
...
//     puts("\n--mDNS_RegisterInterface--\n");
    ...
    while (*p) {
        if (*p == set) {
            puts("mDNS_RegisterInterface: Error! Tried to register a NetworkInterfaceInfo that's already in the list");
            mDNS_Unlock(m);
            return (mStatus_AlreadyRegistered);
        }
        ...
        if ((*p)->InterfaceID == set->InterfaceID) {
            ...
            if (set->ip.type == (*p)->ip.type) {
                FirstOfType = mDNSfalse;
            }
            if (set->ip.type == mDNSAddrType_IPv4 && set->McastTxRx) {
                (*p)->IPv4Available = mDNStrue;
            }
            if (set->ip.type == mDNSAddrType_IPv6 && set->McastTxRx) {
                (*p)->IPv6Available = mDNStrue;
            }
        }
...
//	printf("mDNS_RegisterInterface: InterfaceID  %s %s %s", set->InterfaceID, set->ifname, &set->ip,
//		set->InterfaceActive ?
...
//        puts("FirstOfType--InterfaceActive\n");
        DNSQuestion *q;
        ...
        if (!m->SuppressSending) {
            m->SuppressSending = m->timenow + (mDNSs32)mDNSRandom((mDNSu32)InitialQuestionInterval);
        }

        /* if (flapping) ;//printf("mDNS_RegisterInterface: Frequent transitions for interface %s (%#a)", set->ifname, &set->ip); */
//        printf("set->ifname %s ", set->ifname);
//        put_u32hex0(set->ip);
        //printf("mDNS_RegisterInterface: %s %s probedelay %d", set->ifname, &set->ip, probedelay);
...
//		puts("mDNS_RegisterInterface: Setting AnnounceOwner");
        ...
        for (q = m->Questions; q; q = q->next)								// Scan our list of questions
            if (mDNSOpaque16IsZero(q->TargetQID))
                if (!q->InterfaceID || q->InterfaceID == set->InterfaceID) {	// If non-specific Q, or Q on this specific interface,
                    ...
                    if (dodelay) {
                        puts("No cache records expired for");    // printf("No cache records expired for %##s (%s); okay to delay questions a little", q->qname.c, DNSTypeName(q->qtype));
                    }
        ...
        for (rr = m->ResourceRecords; rr; rr = rr->next)
            if (!AuthRecord_uDNS(rr))
                if (!rr->resrec.InterfaceID || rr->resrec.InterfaceID == set->InterfaceID) {
                    ...
                    rr->ProbeCount     = DefaultProbeCountForRecordType(rr->resrec.RecordType);
                    if (rr->AnnounceCount < numannounce) {
                        rr->AnnounceCount  = numannounce;
                    }
                    rr->SendNSECNow    = mDNSNULL;
                    InitializeLastAPTime(m, rr);
                }
...
mDNSexport void mDNS_DeregisterInterface(mDNS *const m, NetworkInterfaceInfo *set, mDNSBool flapping)
{
    ...
    if (!*p) {
        debugf("mDNS_DeregisterInterface: NetworkInterfaceInfo not found in list");
        mDNS_Unlock(m);
        return;
    }
    ...
    *p = (*p)->next;
    ...
    if (!set->InterfaceActive) {
        ...
        if (intf) {
            ...
            if (intf->NetWake) {
                mDNS_ActivateNetWake_internal(m, intf);
            }
...
mDNSlocal void ServiceCallback(mDNS *const m, AuthRecord *const rr, mStatus result)
{
    ...
    if (result == mStatus_NoError && rr != &sr->RR_SRV) {
        return;
    }
    ...
    if (result == mStatus_MemFree) {
        ...
        if (sr->RR_SRV.resrec.RecordType != kDNSRecordTypeUnregistered) {
            return;
        }
        if (sr->RR_TXT.resrec.RecordType != kDNSRecordTypeUnregistered) {
            return;
        }
        if (sr->RR_PTR.resrec.RecordType != kDNSRecordTypeUnregistered) {
            return;
        }
        if (sr->RR_ADV.resrec.RecordType != kDNSRecordTypeUnregistered) {
            return;
        }
        for (i = 0; i < sr->NumSubTypes; i++) if (sr->SubTypes[i].resrec.RecordType != kDNSRecordTypeUnregistered) {
                return;
            }

        while (e) {
            if (e->r.resrec.RecordType != kDNSRecordTypeUnregistered) {
                return;
            }
            e = e->next;
        }
                                        ...
                                        mDNSInterfaceID InterfaceID, mDNSServiceCallback Callback, void *Context, mDNSu32 flags)
{
    ...
    if (ConstructServiceName(&sr->RR_PTR.namestorage, mDNSNULL, type, domain) == mDNSNULL) {
        return (mStatus_BadParamErr);
    }
    if (ConstructServiceName(&sr->RR_SRV.namestorage, name,     type, domain) == mDNSNULL) {
        return (mStatus_BadParamErr);
    }
    ...
    for (i = 0; i < NumSubTypes; i++) {
        ...
        if (ConstructServiceName(&sr->SubTypes[i].namestorage, mDNSNULL, &st, domain) == mDNSNULL) {
            return (mStatus_BadParamErr);
        }
    ...
    if (host && host->c[0]) {
        AssignDomainName(&sr->RR_SRV.resrec.rdata->u.srv.target, host);
    } else {
        sr->RR_SRV.AutoTarget = Target_AutoHost;
        sr->RR_SRV.resrec.rdata->u.srv.target.c[0] = '\0';
    }
    ...
    for (i = 0; i < NumSubTypes; i++) if (!err) {
...
mDNSexport mStatus mDNS_RenameAndReregisterService(mDNS *const m, ServiceRecordSet *const sr, const domainlabel *newname)
{
    ...
    if (SameDomainName(&domain, &localdomain)) {
        ...
        debugf("%##s service (domain %##s) renamed from \"%#s\" to \"%#s\"", type.c, domain.c, name1.c, newname->c);
    }
...
mDNSexport mStatus mDNS_DeregisterService_drt(mDNS *const m, ServiceRecordSet *sr, mDNS_Dereg_type drt)
{
    ...
    if (mDNSIPPortIsZero(sr->RR_SRV.resrec.rdata->u.srv.port)) {
        return (mDNS_DeregisterNoSuchService(m, &sr->RR_SRV));
    }
        ...
        const mDNSInterfaceID InterfaceID, mDNSRecordCallback Callback, void *Context, mDNSBool includeP2P)
{
    ...
    if (ConstructServiceName(&rr->namestorage, name, type, domain) == mDNSNULL) {
        return (mStatus_BadParamErr);
    }
    ...
    if (host && host->c[0]) {
        AssignDomainName(&rr->resrec.rdata->u.srv.target, host);
    } else {
        ...
        mDNS_DomainType DomainType, const mDNSInterfaceID InterfaceID, char *domname)
{
    ...
    if (!MakeDomainNameFromDNSNameString(&rr->namestorage, mDNS_DomainTypeNames[DomainType])) {
        return (mStatus_BadParamErr);
    }
    if (!MakeDomainNameFromDNSNameString(&rr->resrec.rdata->u.name, domname)) {
        return (mStatus_BadParamErr);
    }
    return (mDNS_Register(m, rr));
}
...
{
    DNSQuestion *q;
    for (q = m->Questions; q; q = q->next) if (mDNSSameOpaque16(id, q->TargetQID)) {
            return mDNStrue;
        }
    return mDNSfalse;
}
...
mDNSexport mDNSOpaque16 mDNS_NewMessageID(mDNS *const m)
{
    ...
    for (i = 0; i < 10; i++) {
        ...
        if (!mDNS_IdUsedInResourceRecordsList(m, id) && !mDNS_IdUsedInQuestionsList(m, id)) {
            break;
        }
    }
...
#define XX wake ? "Received" : "Ignoring", end-p
    case 0x01:
        LogSPS("Ignoring %d-byte ICMP from %#a to %#a", end - p, src, dst);
        break;
...
#define SSH_AsNumber 22
        ...
        if (!mDNSSameIPPort(port, SSH) && !(t->tcp.flags & 2)) {
            wake = mDNSfalse;
        }
...
#define ARD_AsNumber 3283
        ...
        if (udplen >= sizeof(UDPHeader)) {
            ...
            if (mDNSSameIPPort(port, IPSECPort)) {
                ...
                if (datalen == 1 && end >= &t->bytes[9] && t->bytes[8] == 0xFF) {
                    wake = mDNSfalse;
                } else {
            ...
            if (mDNSSameIPPort(port, ARD)) {
                wake = (datalen >= 110 && end >= &t->bytes[10] && t->bytes[8] == 0x13 && t->bytes[9] == 0x88);
            }
    ...
    case 0x3A:
        if (&t->bytes[len] <= end) {
            ...
            if (!checksum) {
                mDNSCoreReceiveRawND(m, sha, &src->ip.v6, &t->ndp, &t->bytes[len], InterfaceID);
            } else {
                LogInfo("IPv6CheckSum bad %04X %02X%02X from %#a to %#a", checksum, t->bytes[2], t->bytes[3], src, dst);
            }
        }
        break;

    default:
        LogSPS("Ignoring %d-byte IP packet unknown protocol %d from %#a to %#a", end - p, protocol, src, dst);
        break;
    }
    ...
    if (wake) {
        ...
        for (rr = m->ResourceRecords; rr; rr = rr->next)
            if (rr->resrec.InterfaceID == InterfaceID &&
                ...
                if (!r2 && mDNSSameIPPort(port, IPSECPort)) {
                    r2 = rr;    // So that we wake for BTMM IPSEC packets, even without a matching SRV record
                }
                if (r2) {
...
#define RequiredCapLen(P) ((P)==0x01 ? 4 : (P)==0x06 ? 20 : (P)==0x11 ? 8 : (P)==0x3A ? 24 : 0)
    ...
    // Is IPv4 with zero fragmentation offset? Length must be at least 14 + 20 = 34 bytes
        ...
        src.ip.v4 = pkt->v4.src;
        dst.type = mDNSAddrType_IPv4;
        dst.ip.v4 = pkt->v4.dst;
    ...
    // Is IPv6? Length must be at least 14 + 28 = 42 bytes
        ...
        src.ip.v6 = pkt->v6.src;
        dst.type = mDNSAddrType_IPv6;
        dst.ip.v6 = pkt->v6.dst;
...
mDNSexport void mDNSCoreBeSleepProxyServer_internal(mDNS *const m, mDNSu8 sps, mDNSu8 port, mDNSu8 marginalpower, mDNSu8 totpower)
{
    ...
    if (!sps && m->SPSSocket) {
        mDNSPlatformUDPClose(m->SPSSocket);
    ...
    if (m->SPSState == 1 && sps != m->SPSType) {
        m->SPSState = 2;
        mDNS_DeregisterService_drt(m, &m->SPSRecords, sps ? mDNS_Dereg_rapid : mDNS_Dereg_normal);
    }
    ...
    if (sps) {
        if (!m->SPSSocket) {
            ...
            if (!m->SPSSocket) {
                LogMsg("mDNSCoreBeSleepProxyServer: Failed to allocate SPSSocket");
                goto fail;
            }
        }
        if (m->SPSState == 0) {
            SleepProxyServerCallback(m, &m->SPSRecords, mStatus_MemFree);
        }
    } else if (m->SPSState) {
                             ...
                             mDNSBool AdvertiseLocalAddresses, mDNSCallback *Callback, void *Context)
{
    ...
    CHECK_WCF_FUNCTION(WCFConnectionNew) {
        ...
        if (!m->WCF) {
            LogMsg("WCFConnectionNew failed");
            return -1;
        }
    }
#endif
...
mDNSlocal void CacheRecordResetDNSServer(mDNS *const m, DNSQuestion *q, DNSServer *new)
{
    ...
    if ((found && foundNew) || (!found && foundNew)) {
        if (newQuestion) {
            ...
            for (qptr = q->next; qptr; qptr = qptr->next)
                if (qptr->DuplicateOf == q) {
                    qptr->deliverAddEvents = mDNStrue;
                }
        }
        return;
    }
...
mDNSexport mStatus uDNS_SetupDNSConfig(mDNS *const m)
{
        ...
        if (!mDNSOpaque16IsZero(q->TargetQID)) {
            ...
            if (t != s) {
                ...
                if (!QuerySuppressed(q)) {
                    ...
                    for (qptr = q->next ; qptr; qptr = qptr->next) {
                        if (qptr->DuplicateOf == q) {
                            ActivateUnicastQuery(m, qptr, mDNStrue);
                        }
                    }
                }
            } else {
    ...
    if ((m->DNSServers != mDNSNULL) != (oldServers != mDNSNULL)) {
        int count = 0;
        FORALL_CACHERECORDS(slot, cg, cr) if (!cr->resrec.InterfaceID) {
            mDNS_PurgeCacheResourceRecord(m, cr);
            count++;
        }
...
mDNSexport void mDNS_StartExit(mDNS *const m)
{
    ...
    CHECK_WCF_FUNCTION(WCFConnectionDealloc) {
        if (m->WCF) {
            WCFConnectionDealloc((WCFConnection *)m->WCF);
        }
    }
#endif
#endif
...
#ifndef UNICAST_DISABLED
    {
        ...
        while (m->Hostnames) {
            mDNS_RemoveDynDNSHostName(m, &m->Hostnames->fqdn);
        }
        ...
        for (s = mDNS_SearchList; s; s = s->next)
            while (s->AuthRecs) {
    ...
    if (m->ResourceRecords) {
        ...
        LogInfo("mDNS_StartExit: No deregistering records remain");
    }
