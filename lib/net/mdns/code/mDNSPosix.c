                                       ...
                                       mDNSIPPort dstPort)
{
    ...
    if (dstPort.NotAnInteger == 0) {
        /* puts("mDNSPlatformSendUDP: Invalid argument -dstPort is set to 0"); */
...
//	assert(m    != NULL);
//	assert(skt  >= 0);
    // puts("SocketDataReady\n");

    fromLen = sizeof(from);
    flags   = 0;

//	packetLen = recvfrom_flags(skt, &packet, sizeof(packet), &flags, (struct sockaddr *) &from, &fromLen, &packetInfo, &ttl);
//    put_buf((struct sockaddr *) &from,fromLen);
    packetLen = recvfrom(skt, &packet, sizeof(packet), flags,
                         (struct sockaddr *)&from, &fromLen);


//    if (packetInfo)
    {
        memset(&packetInfo, 0, sizeof(struct my_in_pktinfo));
        packetInfo.ipi_ifindex = -1;
    }
...
//Z	FILE *fp = fopen(filePath, "r");
    ...
    while (fgets(line, sizeof(line), fp)) {
        ...
        if (sscanf(line, "%10s %15s", keyword, nameserver) != 2) {
            continue;    // it will skip whitespaces
        }
        if (strncasecmp(keyword, "nameserver", 10)) {
            continue;
        }
...
//    put_u8hex0(*sktPtr);
    if (*sktPtr < 0) {
        err = errno;
        goto __err;
        // puts("SetupSocket err\n");
        // perror((intfAddr->sa_family == AF_INET) ? "socket AF_INET" : "socket AF_INET6");
    }
...
//  SO_REUSEADDR IP_PKTINFO IP_ADD_MEMBERSHIP IP_TTL IP_MULTICAST_TTL
    ...
    if (intfAddr->sa_family == AF_INET) {
        ...
        if (err == 0 && JoinMulticastGroup) {
            //puts("setsockopt IP_ADD_MEMBERSHIP\n");
        ...
        if (err == 0 && JoinMulticastGroup) {
            // puts("setsockopt IP_MULTICAST_IF\n");
        ...
        if (err == 0) {
            //puts("setsockopt IP_MULTICAST_TTL\n");
...
mDNSlocal int SetupOneInterface(mDNS *const m, struct sockaddr *intfAddr, struct sockaddr *intfMask, const char *intfName, int intfIndex)
{
    ...
    if (err == 0) {
        ...
        sin = (struct sockaddr_in *)intfAddr;
        // puts("intfAddr intfMask\n");
//        put_buf(intfAddr,sizeof(struct sockaddr));
//        put_buf(intfMask,sizeof(struct sockaddr));
//        printf("mdns ip address: %s\n", inet_ntoa(*((ip_addr_t*)&(sin->sin_addr))));
        // printf("intfName=%s",intfName);
    ...
    if (err == 0) {
//        put_u8hex0(alias->multicastSocket4);
//        put_u8hex0(intfAddr->sa_family);
...
//		puts("\n---SetupOneInterface----\n");
        ...
        SetupOneInterface(m, i->ifi_addr, i->ifi_netmask, i->ifi_name, i->ifi_index);
//		while (i)
//			{
...
mDNSlocal mDNSBool mDNSPlatformInit_CanReceiveUnicast(void)
{
    ...
    closesocket(s);
//	if (err) puts("No unicast UDP responses\n");
//	else     puts("Unicast UDP responses okay\n");
    return (err == 0);
}
...
mDNSexport mStatus mDNSPlatformInit(mDNS *const m)
{
    ...
    if (m->nicelabel.c[0] == 0) {
        MakeDomainLabelFromLiteralString(&m->nicelabel, "wifi-story-machine");
    }
    ...
    if (m->hostlabel.c[0] == 0) {
        MakeDomainLabelFromLiteralString(&m->hostlabel, "wifi-story-machine");
    }
...
{
    struct timeval tv;
    static unsigned long long  tv_sec, tv_usec;
    tv_sec = tv_sec + 1;
    tv_usec = tv_usec + 1;
    tv.tv_sec = tv_sec;
    tv.tv_usec = tv_usec;
...
//z	(void) sigprocmask(SIG_BLOCK, &gEventSignalSet, (sigset_t*) NULL);
