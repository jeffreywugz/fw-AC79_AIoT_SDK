...
mDNSexport SearchListElem *mDNS_SearchList = mDNSNULL;
        ...
        const domainname *domain, const domainname *keyname, const char *b64keydata, const domainname *hostname, mDNSIPPort *port, const char *autoTunnelPrefix)
{
    ...
    DomainAuthInfo **p = &m->AuthInfoList;
    if (!info || !b64keydata) {
        LogMsg("mDNS_SetSecretForDomain: ERROR: info %p b64keydata %p", info, b64keydata);
    ...
    if (*p) {
        LogInfo("mDNS_SetSecretForDomain: Domain %##s Already in list", (*p)->domain.c);
...
mDNSlocal mStatus uDNS_SendNATMsg(mDNS *m, NATTraversalInfo *info)
{
    ...
    if (!mDNSIPv4AddressIsZero(m->Router.ip.v4) && mDNSv4AddrIsRFC1918(&m->Router.ip.v4)) {
        ...
        } u = { { NATMAP_VERS, NATOp_AddrRequest } } ;
        ...
        if (mDNSIPPortIsZero(m->UPnPRouterPort) || mDNSIPPortIsZero(m->UPnPSOAPPort)) {
            ...
            err = LNT_MapPort(m, info);
        } else {
            err = LNT_GetExternalAddress(m);
        }
...
mDNSexport void natTraversalHandlePortMapReply(mDNS *const m, NATTraversalInfo *n, const mDNSInterfaceID InterfaceID, mDNSu16 err, mDNSIPPort extport, mDNSu32 lease)
{
    ...
    if (err || lease == 0 || mDNSIPPortIsZero(extport)) {
        ...
        if (err == NATErr_Refused) {
            n->NewResult = mStatus_NATPortMappingDisabled;
        } else if (err > NATErr_None && err <= NATErr_Opcode) {
            n->NewResult = mStatus_NATPortMappingUnsupported;
        }
    } else {
...
mDNSexport mStatus mDNS_StartNATOperation_internal(mDNS *const m, NATTraversalInfo *traversal)
{
    ...
    if (!traversal->NATLease) {
        traversal->NATLease = NATMAP_DEFAULT_LEASE;
    }
...
mDNSexport mStatus mDNS_StopNATOperation_internal(mDNS *m, NATTraversalInfo *traversal)
{
    ...
    while (*ptr && *ptr != traversal) {
        ptr = &(*ptr)->next;
    }
    if (*ptr) {
        *ptr = (*ptr)->next;    // If we found it, cut this NATTraversalInfo struct from our list
    } else {
...
mDNSlocal mDNSu8 *putLLQ(DNSMessage *const msg, mDNSu8 *ptr, const DNSQuestion *const question, const LLQOptData *const data)
{
    ...
    if (!ptr) {
        LogMsg("ERROR: putLLQ - putQuestion");
        return mDNSNULL;
    }
    ...
    if (!ptr) {
        LogMsg("ERROR: putLLQ - PutResourceRecordTTLJumbo");
        return mDNSNULL;
    }
...
mDNSlocal void sendChallengeResponse(mDNS *const m, DNSQuestion *const q, const LLQOptData *llq)
{
    ...
    if (q->tcp) {
        LogMsg("sendChallengeResponse: ERROR!!: question %##s (%s) tcp non-NULL", q->qname.c, DNSTypeName(q->qtype));
        return;
    }

    if (PrivateQuery(q)) {
        LogMsg("sendChallengeResponse: ERROR!!: Private Query %##s (%s)", q->qname.c, DNSTypeName(q->qtype));
        return;
    }
...
mDNSlocal void recvSetupResponse(mDNS *const m, mDNSu8 rcode, DNSQuestion *const q, const LLQOptData *const llq)
{
    if (rcode && rcode != kDNSFlag1_RC_NXDomain) {
        LogMsg("ERROR: recvSetupResponse %##s (%s) - rcode && rcode != kDNSFlag1_RC_NXDomain", q->qname.c, DNSTypeName(q->qtype));
        return;
    }

    if (llq->llqOp != kLLQOp_Setup) {
        LogMsg("ERROR: recvSetupResponse %##s (%s) - bad op %d", q->qname.c, DNSTypeName(q->qtype), llq->llqOp);
        return;
    }

    if (llq->vers != kLLQ_Vers) {
        LogMsg("ERROR: recvSetupResponse %##s (%s) - bad vers %d", q->qname.c, DNSTypeName(q->qtype), llq->vers);
        return;
    }
    ...
    if (q->state == LLQ_InitialRequest) {
        ...
        if (llq->err) {
            LogMsg("recvSetupResponse - received llq->err %d from server", llq->err);
            StartLLQPolling(m, q);
            return;
        }
        ...
        if (llq->err) {
            LogMsg("ERROR: recvSetupResponse %##s (%s) code %d from server", q->qname.c, DNSTypeName(q->qtype), llq->err);
            StartLLQPolling(m, q);
            return;
        }
        if (!mDNSSameOpaque64(&q->id, &llq->id)) {
            LogMsg("recvSetupResponse - ID changed.  discarding");    // this can happen rarely (on packet loss + reordering)
            return;
        }
    ...
    if (msg->h.numQuestions && getQuestion(msg, msg->data, end, 0, &pktQ)) {
        ...
        for (q = m->Questions; q; q = q->next) {
            if (!mDNSOpaque16IsZero(q->TargetQID) && q->LongLived && q->qtype == pktQ.qtype && q->qnamehash == pktQ.qnamehash && SameDomainName(&q->qname, &pktQ.qname)) {
                ...
                if (q->state == LLQ_Poll) {
                    debugf("uDNS_LLQ_Events: q->state == LLQ_Poll msg->h.id %d q->TargetQID %d", mDNSVal16(msg->h.id), mDNSVal16(q->TargetQID));
                }
                ...
                // Note: In LLQ Event packets, the msg->h.id does not match our q->TargetQID, because in that case the msg->h.id nonce is selected by the server
                    ...
                    if (ackEnd) {
                        mDNSSendDNSMessage(m, &m->omsg, ackEnd, mDNSInterface_Any, q->LocalSocket, srcaddr, srcport, mDNSNULL, mDNSNULL);
                    }
                    m->rec.r.resrec.RecordType = 0;		// Clear RecordType to show we're not still using it
                ...
                if (opt && mDNSSameOpaque16(msg->h.id, q->TargetQID)) {
                    if (q->state == LLQ_Established && opt->u.llq.llqOp == kLLQOp_Refresh && mDNSSameOpaque64(&opt->u.llq.id, &q->id) && msg->h.numAdditionals && !msg->h.numAnswers) {
                        if (opt->u.llq.err != LLQErr_NoError) {
                            LogMsg("recvRefreshReply: received error %d from server", opt->u.llq.err);
                        } else {
                            ...
                            if (opt->u.llq.llqlease == 0 && m->SleepLimit) {
...
mDNSlocal void tcpCallback(TCPSocket *sock, void *context, mDNSBool ConnectionEstablished, mStatus err)
{
    ...
    if (ConnectionEstablished) {
        ...
        if (tcpInfo->nread < 2) {		// First read the two-byte length preceeding the DNS message
            ...
            if (n < 0) {
                ...
                if (tcpInfo->numReplies == 0) {
                    ...
                    if (backpointer) {
                        *backpointer = mDNSNULL;    // Clear client backpointer FIRST so we don't risk double-disposing our tcpInfo_t
                    }
        ...
        if ((tcpInfo->nread - 2) == tcpInfo->replylen) {
            ...
            if (q && q->tcp) {
                srcPort = q->tcp->SrcPort;
                q->tcpSrcPort = srcPort;
            }
                                 ...
                                 DNSQuestion *const question, AuthRecord *const rr)
{
    ...
    if ((flags & kTCPSocketFlags_UseTLS) && (!hostname || !hostname->c[0])) {
        LogMsg("MakeTCPConn: TLS connection being setup with NULL hostname");
        return mDNSNULL;
    }

    info = (tcpInfo_t *)mDNSPlatformMemAllocate(sizeof(tcpInfo_t));
    if (!info) {
        LogMsg("ERROR: MakeTCP - memallocate failed");
        return (mDNSNULL);
    }
    ...
    if (!info->sock) {
        LogMsg("MakeTCPConn: unable to create TCP socket");
        mDNSPlatformMemFree(info);
        return (mDNSNULL);
    }
    ...
    if (err == mStatus_ConnEstablished) {
        tcpCallback(info->sock, info, mDNStrue, mStatus_NoError);
    } else if (err != mStatus_ConnPending) {
        LogInfo("MakeTCPConn: connection failed");
        DisposeTCPConn(info);
        return (mDNSNULL);
    }
    return (info);
...
mDNSexport void startLLQHandshake(mDNS *m, DNSQuestion *q)
{
    ...
    if (PrivateQuery(q)) {
        if (q->tcp) {
            LogInfo("startLLQHandshake: Disposing existing TCP connection for %##s (%s)", q->qname.c, DNSTypeName(q->qtype));
        }
        if (q->tcp) {
...
mDNSexport const domainname *GetServiceTarget(mDNS *m, AuthRecord *const rr)
{
    ...
    if (!rr->AutoTarget) {	// If not automatically tracking this host's current name, just return the existing target
        ...
        if (m->StaticHostname.c[0]) {
            return (&m->StaticHostname);
        } else {
            GetStaticHostname(m);    // asynchronously do reverse lookup for primary IPv4 address
        }
...
mDNSlocal void GetZoneData_QuestionCallback(mDNS *const m, DNSQuestion *question, const ResourceRecord *const answer, QC_result AddRecord)
{
    ...
    if (AddRecord == QC_addnocache && answer->rdlength == 0) {
        return;    // Don't care about transient failure indications
    }
...
mDNSexport ZoneData *StartGetZoneData(mDNS *const m, const domainname *const name, const ZoneService target, ZoneDataCallback callback, void *ZoneDataContext)
{
    ...
    if (!zd) {
        LogMsg("ERROR: StartGetZoneData - mDNSPlatformMemAllocate failed");
        return mDNSNULL;
    }
    ...
    if (AuthInfo && AuthInfo->AutoTunnel && !mDNSIPPortIsZero(AuthInfo->port)) {
        ...
        if (AuthInfo && AuthInfo->AutoTunnel) {
            LogInfo("StartGetZoneData: Not Bypassing SOA, SRV query for %##s", AuthInfo->domain.c);
        }
        AssignDomainName(&zd->question.qname, zd->CurrentSOA);
        GetZoneData_StartQuery(m, zd, kDNSType_SOA);
    }
...
mDNSlocal void UpdateAllServiceRecords(mDNS *const m, AuthRecord *rr, mDNSBool reg)
{
    ...
    if (rr->resrec.rrtype != kDNSType_SRV) {
        LogMsg("UpdateAllServiceRecords:ERROR!! ResourceRecord not a service record %s", ARDisplayString(m, rr));
        return;
    }

    if (reg && rr->state == regState_NoTarget) {
        LogMsg("UpdateAllServiceRecords:ERROR!! SRV record %s in noTarget state during registration", ARDisplayString(m, rr));
        return;
    }
...
mDNSlocal void CompleteRecordNatMap(mDNS *m, NATTraversalInfo *n)
{
    ...
    if (!rr) {
        LogMsg("CompleteRecordNatMap called with unknown AuthRecord object");
        return;
    }
    if (!n->NATLease) {
        LogMsg("CompleteRecordNatMap No NATLease for %s", ARDisplayString(m, rr));
        return;
    }

    if (rr->resrec.rrtype != kDNSType_SRV) {
        LogMsg("CompleteRecordNatMap: Not a service record %s", ARDisplayString(m, rr));
        return;
    }

    if (rr->resrec.RecordType == kDNSRecordTypeDeregistering) {
        LogInfo("CompleteRecordNatMap called for %s, Service deregistering", ARDisplayString(m, rr));
        return;
    }

    if (rr->state == regState_DeregPending) {
        LogInfo("CompleteRecordNatMap called for %s, record in DeregPending", ARDisplayString(m, rr));
        return;
    }
...
mDNSlocal void StartRecordNatMap(mDNS *m, AuthRecord *rr)
{
    ...
    if (SameDomainLabel(p, (mDNSu8 *)"\x4" "_tcp")) {
        protocol = NATOp_MapTCP;
    } else if (SameDomainLabel(p, (mDNSu8 *)"\x4" "_udp")) {
        protocol = NATOp_MapUDP;
    } else {
        LogMsg("StartRecordNatMap: could not determine transport protocol of service %##s", rr->resrec.name->c);
        return;
    }
...
mDNSlocal void hostnameGetPublicAddressCallback(mDNS *m, NATTraversalInfo *n)
{
    ...
    if (!n->Result) {
        if (mDNSIPv4AddressIsZero(n->ExternalAddress) || mDNSv4AddrIsRFC1918(&n->ExternalAddress)) {
            return;
        }

        if (h->arv4.resrec.RecordType) {
            if (mDNSSameIPv4Address(h->arv4.resrec.rdata->u.ipv4, n->ExternalAddress)) {
...
mDNSlocal void AdvertiseHostname(mDNS *m, HostnameInfo *h)
{
    if (!mDNSIPv4AddressIsZero(m->AdvertisedV4.ip.v4) && h->arv4.resrec.RecordType == kDNSRecordTypeUnregistered) {
        ...
        if (mDNSv4AddrIsRFC1918(&m->AdvertisedV4.ip.v4)) {
            ...
            if (h->natinfo.clientContext) {
                mDNS_StopNATOperation_internal(m, &h->natinfo);
            }
...
mDNSlocal void HostnameCallback(mDNS *const m, AuthRecord *const rr, mStatus result)
{
    ...
    if (!hi) {
        LogMsg("HostnameCallback invoked with orphaned address record");
        return;
    }
    if (rr->resrec.rrtype == kDNSType_A) {
...
mDNSlocal void GetStaticHostname(mDNS *m)
{
    ...
    if (mDNSIPv4AddressIsZero(m->AdvertisedV4.ip.v4)) {
        return;
    }
    ...
    if (!MakeDomainNameFromDNSNameString(&q->qname, buf)) {
...
mDNSexport void mDNS_AddDynDNSHostName(mDNS *m, const domainname *fqdn, mDNSRecordCallback *StatusCallback, const void *StatusContext)
{
    ...
    while (*ptr && !SameDomainName(fqdn, &(*ptr)->fqdn)) {
        ptr = &(*ptr)->next;
    }
    if (*ptr) {
        LogMsg("DynDNSHostName %##s already in list", fqdn->c);
        return;
    }
    ...
    *ptr = mDNSPlatformMemAllocate(sizeof(**ptr));
    if (!*ptr) {
        LogMsg("ERROR: mDNS_AddDynDNSHostName - malloc");
        return;
    }
...
mDNSexport void mDNS_SetPrimaryInterfaceInfo(mDNS *m, const mDNSAddr *v4addr, const mDNSAddr *v6addr, const mDNSAddr *router)
{
    ...
    if (v4addr) {
        m->AdvertisedV4 = *v4addr;
    } else {
        m->AdvertisedV4.ip.v4 = zerov4Addr;
    }
    if (v6addr) {
        m->AdvertisedV6 = *v6addr;
    } else {
        m->AdvertisedV6.ip.v6 = zerov6Addr;
    }
    if (router) {
        m->Router       = *router;
    } else {
        m->Router      .ip.v4 = zerov4Addr;
    }
...
mDNSlocal mStatus ParseTSIGError(mDNS *const m, const DNSMessage *const msg, const mDNSu8 *const end, const domainname *const displayname)
{
    ...
    for (i = 0; i < msg->h.numAdditionals; i++) {
        ...
        if (m->rec.r.resrec.RecordType != kDNSRecordTypePacketNegative && m->rec.r.resrec.rrtype == kDNSType_TSIG) {
            ...
            if (rd + sizeof(mDNSOpaque16) > rdend) {
                goto finish;
            }
            ...
            if (err == TSIG_ErrBadSig)  {
                LogMsg("%##s: bad signature", displayname->c);
                err = mStatus_BadSig;
            } else if (err == TSIG_ErrBadKey)  {
                LogMsg("%##s: bad key", displayname->c);
                err = mStatus_BadKey;
            } else if (err == TSIG_ErrBadTime) {
                LogMsg("%##s: bad time", displayname->c);
                err = mStatus_BadTime;
            } else if (err)                    {
                LogMsg("%##s: unknown tsig error %d", displayname->c, err);
                err = mStatus_UnknownErr;
            }
            goto finish;
        }
...
mDNSlocal mDNSu32 RRAdditionalSize(mDNS *const m, DomainAuthInfo *AuthInfo)
{
    ...
    tsigSize = 0;
    if (AuthInfo) {
        tsigSize = DomainNameLength(&AuthInfo->keyname) + rr_base_size + 58;
    }
...
mDNSlocal mDNSu8 *BuildUpdateMessage(mDNS *const m, mDNSu8 *ptr, AuthRecord *rr, mDNSu8 *limit)
{
    ...
    if (rr->resrec.RecordType != kDNSRecordTypeAdvisory) {
    ...
    if (rr->state == regState_UpdatePending) {
        ...
        if (!(ptr = putDeletionRecordWithLimit(&m->omsg, ptr, &rr->resrec, limit))) {
            goto exit;    // delete old rdata
        }

        // add new RData
        SetNewRData(&rr->resrec, rr->InFlightRData, rr->InFlightRDLen);
        if (!(ptr = PutResourceRecordTTLWithLimit(&m->omsg, ptr, &m->omsg.h.mDNS_numUpdates, &rr->resrec, rr->resrec.rroriginalttl, limit))) {
            goto exit;
        }
    } else {
...
mDNSlocal void SendRecordRegistration(mDNS *const m, AuthRecord *rr)
{
    ...
    if (rr->Private) {
        ...
        if (rr->tcp) {
            LogInfo("SendRecordRegistration: Disposing existing TCP connection for %s", ARDisplayString(m, rr));
        }
        if (rr->tcp) {
        ...
        if (!rr->nta) {
            LogMsg("SendRecordRegistration:Private:ERROR!! nta is NULL for %s", ARDisplayString(m, rr));
            return;
        }
        ...
        if (!rr->nta) {
            LogMsg("SendRecordRegistration:ERROR!! nta is NULL for %s", ARDisplayString(m, rr));
            return;
        }
...
mDNSlocal mDNSBool IsRecordMergeable(mDNS *const m, AuthRecord *rr, mDNSs32 time)
{
    ...
    if (rr->LastAPTime + rr->ThisAPInterval - time > 0) {
        debugf("IsRecordMergeable: Time %d not reached for %s", rr->LastAPTime + rr->ThisAPInterval - m->timenow, ARDisplayString(m, rr));
        return mDNSfalse;
    }
    ...
    if (info && info->deltime && m->timenow - info->deltime >= 0) {
        debugf("IsRecordMergeable: Domain %##s will be deleted soon", info->domain.c);
        return mDNSfalse;
    }
    ...
    if (rr->state != regState_DeregPending && rr->state != regState_Pending && rr->state != regState_Registered && rr->state != regState_Refresh && rr->state != regState_UpdatePending) {
        debugf("IsRecordMergeable: state %d not right  %s", rr->state, ARDisplayString(m, rr));
        return mDNSfalse;
    }
    ...
    if (rr->mState == mergeState_DontMerge) {
        debugf("IsRecordMergeable Dontmerge true %s", ARDisplayString(m, rr));
        return mDNSfalse;
    }
...
mDNSlocal mDNSBool AreRecordsMergeable(mDNS *const m, AuthRecord *currentRR, AuthRecord *rr, mDNSs32 time)
{
    ...
    if (!IsRecordMergeable(m, rr, time)) {
        return mDNSfalse;
    }

    if (!SameDomainName(currentRR->zone, rr->zone)) {
        debugf("AreRecordMergeable zone mismatch current rr Zone %##s, rr zone  %##s", currentRR->zone->c, rr->zone->c);
        return mDNSfalse;
    }

    if (!mDNSSameIPv4Address(currentRR->nta->Addr.ip.v4, rr->nta->Addr.ip.v4)) {
        return mDNSfalse;
    }

    if (!mDNSSameIPPort(currentRR->nta->Port, rr->nta->Port)) {
        return mDNSfalse;
    }
...
mDNSlocal void SendGroupRRMessage(mDNS *const m, AuthRecord *anchorRR, mDNSu8 *ptr, DomainAuthInfo *info)
{
    mDNSu8 *limit;
    if (!anchorRR) {
        debugf("SendGroupRRMessage: Could not merge records");
        return;
    }
    ...
    if (anchorRR->Private) {
        if (anchorRR->tcp) {
            debugf("SendGroupRRMessage: Disposing existing TCP connection for %s", ARDisplayString(m, anchorRR));
        }
        if (anchorRR->tcp) {
        ...
        if (!anchorRR->nta) {
            LogMsg("SendGroupRRMessage:ERROR!! nta is NULL for %s", ARDisplayString(m, anchorRR));
            return;
        }
        ...
        if (!anchorRR->tcp) {
            LogInfo("SendGroupRRMessage: Cannot establish TCP connection for %s", ARDisplayString(m, anchorRR));
        } else {
            LogInfo("SendGroupRRMessage: Sent a group update ID: %d start %p, end %p, limit %p", mDNSVal16(m->omsg.h.id), m->omsg.data, ptr, limit);
        }
    } else {
        mStatus err = mDNSSendDNSMessage(m, &m->omsg, ptr, mDNSInterface_Any, mDNSNULL, &anchorRR->nta->Addr, anchorRR->nta->Port, mDNSNULL, info);
        if (err) {
            LogInfo("SendGroupRRMessage: Cannot send UDP message for %s", ARDisplayString(m, anchorRR));
        } else {
            LogInfo("SendGroupRRMessage: Sent a group UDP update ID: %d start %p, end %p, limit %p", mDNSVal16(m->omsg.h.id), m->omsg.data, ptr, limit);
        }
    }
    return;
}
...
mDNSlocal AuthRecord *MarkRRForSending(mDNS *const m)
{
    ...
    for (rr = m->ResourceRecords; rr; rr = rr->next) {
        if (!firstRR) {
            ...
            firstRR = rr;
        } else if (!AreRecordsMergeable(m, firstRR, rr, m->timenow + MERGE_DELAY_TIME)) {
            continue;
        }

        if (rr->SendRNow) {
            LogMsg("MarkRRForSending: Resourcerecord %s already marked for sending", ARDisplayString(m, rr));
        }
    ...
    while (startRR) {
        ...
        for (rr = startRR; rr; rr = rr->next) {
            ...
            if (!anchorRR) {
                ...
                if (AuthInfo && AuthInfo->AutoTunnel) {
                    spaceleft = AbsoluteMaxDNSMessageData;
                } else {
            ...
            for (rr = m->ResourceRecords; rr; rr = nextRR) {
                ...
                if (rr->SendRNow == mDNSInterfaceMark) {
                    ...
                    if (rr->LastAPTime + rr->ThisAPInterval - m->timenow > 0) {
                        LogMsg("CheckGroupRecordUpdates: ERROR!! Resourcerecord %s not ready", ARDisplayString(m, rr));
                        continue;
                    }
                    rr->SendRNow = mDNSNULL;
...
mDNSlocal void hndlRecordUpdateReply(mDNS *m, AuthRecord *rr, mStatus err, mDNSu32 random)
{
    ...
    if (rr->state == regState_UpdatePending) {
        ...
        if (rr->UpdateCallback) {
            rr->UpdateCallback(m, rr, rr->OrigRData, rr->OrigRDLen);
        }
        SetNewRData(&rr->resrec, rr->InFlightRData, rr->InFlightRDLen);
    ...
    if (rr->SRVChanged) {
        if (rr->resrec.rrtype == kDNSType_SRV) {
            ...
            rr->SRVChanged = mDNSfalse;
            if (rr->state != regState_DeregPending) {
                LogMsg("hndlRecordUpdateReply: ResourceRecord %s not in DeregPending state %d", ARDisplayString(m, rr), rr->state);
            }
            rr->state = regState_NoTarget;	// Wait for the next target change
        }
        return;
...
mDNSexport void uDNS_ReceiveNATPMPPacket(mDNS *m, const mDNSInterfaceID InterfaceID, mDNSu8 *pkt, mDNSu16 len)
{
    ...
    if (!AddrReply->err && len < 8) {
        LogMsg("NAT Traversal message too short (%d bytes)", len);
        return;
    }
    if (AddrReply->vers != NATMAP_VERS) {
        LogMsg("Received NAT Traversal response with version %d (expected %d)", pkt[0], NATMAP_VERS);
        return;
    }
    ...
    if (AddrReply->upseconds < m->LastNATupseconds || nat_elapsed + 2 < our_elapsed - our_elapsed / 8) {
        LogMsg("NAT gateway %#a rebooted", &m->Router);
    ...
    if (AddrReply->opcode == NATOp_AddrResponse) {
...
#endif
        if (!AddrReply->err && len < sizeof(NATAddrReply)) {
        ...
        LogMsg("Received NAT Traversal response with version unknown opcode 0x%X", AddrReply->opcode);
        return;
    }

    // Don't need an SSDP socket if we get a NAT-PMP packet
    if (m->SSDPSocket) {
        debugf("uDNS_ReceiveNATPMPPacket destroying SSDPSocket %p", &m->SSDPSocket);
...
mDNSlocal mDNSBool NoTestQuery(DNSQuestion *q)
{
    ...
    mDNSu8 *p = q->qname.c;
    if (q->AuthInfo) {
        return (mDNStrue);    // Don't need a test query for private queries sent directly to authoritative server over TLS/TCP
    }
    if (q->qtype != kDNSType_PTR) {
        return (mDNStrue);    // Don't need a test query for any non-PTR queries
    }
    for (i = 0; i < 4; i++) {	// If qname does not begin with num.num.num.num, can't skip the test query
        ...
        if (p[0] >= 2 && (p[2] < '0' || p[2] > '9')) {
            return (mDNSfalse);
        }
        if (p[0] >= 3 && (p[3] < '0' || p[3] > '9')) {
            return (mDNSfalse);
        }
        p += 1 + p[0];
    }
        ...
        const mDNSAddr *const srcaddr, const mDNSIPPort srcport)
{
    ...
    if (pktq.qtype != kDNSType_PTR || pktq.qclass != kDNSClass_IN) {
        return (mDNSfalse);
    }
    if (!SameDomainName(&pktq.qname, DNSRelayTestQuestion)) {
        return (mDNSfalse);
    }
...
mDNSexport void sendLLQRefresh(mDNS *m, DNSQuestion *q)
{
    ...
    if (!end) {
        LogMsg("sendLLQRefresh: putLLQ failed %##s (%s)", q->qname.c, DNSTypeName(q->qtype));
        return;
    }
    ...
    if (!end) {
        LogMsg("sendLLQRefresh: putHINFO failed %##s (%s)", q->qname.c, DNSTypeName(q->qtype));
        return;
    }
    ...
    if (PrivateQuery(q)) {
        ...
        if (!end) {
            LogMsg("sendLLQRefresh: DNSDigest_SignMessage failed %##s (%s)", q->qname.c, DNSTypeName(q->qtype));
            return;
        }
    }
    ...
    if (PrivateQuery(q) && !q->tcp) {
        ...
        if (!q->nta) {
            LogMsg("sendLLQRefresh:ERROR!! q->nta is NULL for %##s (%s)", q->qname.c, DNSTypeName(q->qtype));
            return;
        }
...
mDNSexport void LLQGotZoneData(mDNS *const m, mStatus err, const ZoneData *zoneInfo)
{
    ...
    if (!err && zoneInfo && !mDNSIPPortIsZero(zoneInfo->Port) && !mDNSAddressIsZero(&zoneInfo->Addr) && zoneInfo->Host.c[0]) {
        ...
        if (!PrivateQuery(q)) {
            ...
            if (q->nta) {
                if (q->nta != zoneInfo) {
                    LogMsg("LLQGotZoneData: nta (%p) != zoneInfo (%p)  %##s (%s)", q->nta, zoneInfo, q->qname.c, DNSTypeName(q->qtype));
                }
                CancelGetZoneData(m, q->nta);
                q->nta = mDNSNULL;
            }
...
mDNSlocal void PrivateQueryGotZoneData(mDNS *const m, mStatus err, const ZoneData *zoneInfo)
{
    ...
    if (q->nta != zoneInfo) {
        LogMsg("PrivateQueryGotZoneData:ERROR!!: nta (%p) != zoneInfo (%p)  %##s (%s)", q->nta, zoneInfo, q->qname.c, DNSTypeName(q->qtype));
    }
    ...
    if (!q->nta) {
        LogMsg("PrivateQueryGotZoneData:ERROR!! nta is NULL for %##s (%s)", q->qname.c, DNSTypeName(q->qtype));
        return;
    }
...
mDNSexport void RecordRegistrationGotZoneData(mDNS *const m, mStatus err, const ZoneData *zoneData)
{
    ...
    for (ptr = m->ResourceRecords; ptr; ptr = ptr->next) if (ptr == newRR) {
            break;
        }
    if (!ptr) {
    ...
    if (!zoneData) {
        LogMsg("ERROR: RecordRegistrationGotZoneData invoked with NULL result and no error");
        return;
    }
    ...
    if (newRR->resrec.rrtype == kDNSType_SRV && !mDNSIPPortIsZero(newRR->resrec.rdata->u.srv.port) &&
        ...
        if (!newRR->NATinfo.clientContext) {
            ...
            LogInfo("RecordRegistrationGotZoneData: StartRecordNatMap for %s, state %d, context %p", ARDisplayString(m, newRR), newRR->state, newRR->NATinfo.clientContext);
        }
    }
    mDNS_Lock(m);
    // We want IsRecordMergeable to check whether it is a record whose update can be
...
mDNSlocal void SendRecordDeregistration(mDNS *m, AuthRecord *rr)
{
    ...
    if (rr->Private) {
        ...
        if (rr->tcp) {
            LogInfo("SendRecordDeregistration: Disposing existing TCP connection for %s", ARDisplayString(m, rr));
        }
        if (rr->tcp) {
            DisposeTCPConn(rr->tcp);
            rr->tcp = mDNSNULL;
        ...
        if (!rr->nta) {
            LogMsg("SendRecordDeregistration:Private:ERROR!! nta is NULL for %s", ARDisplayString(m, rr));
            return;
        }
        rr->tcp = MakeTCPConn(m, &m->omsg, ptr, kTCPSocketFlags_UseTLS, &rr->nta->Addr, rr->nta->Port, &rr->nta->Host, mDNSNULL, rr);
    } else {
        ...
        if (!rr->nta) {
            LogMsg("SendRecordDeregistration:ERROR!! nta is NULL for %s", ARDisplayString(m, rr));
            return;
        }
        err = mDNSSendDNSMessage(m, &m->omsg, ptr, mDNSInterface_Any, mDNSNULL, &rr->nta->Addr, rr->nta->Port, mDNSNULL, GetAuthInfoForName_internal(m, rr->resrec.name));
        if (err) {
...
mDNSexport mStatus uDNS_DeregisterRecord(mDNS *const m, AuthRecord *const rr)
{
    ...
    if (!mDNSOpaque16IsZero(rr->updateid)) {
        ...
        if (!found) {
            LogInfo("uDNSDeregisterRecord: Cannot find the anchor Resource Record for %s, not an error", ARDisplayString(m, rr));
        }
    }
    ...
    if (IsRecordMergeable(m, rr, m->timenow + MERGE_DELAY_TIME)) {
        ...
        if (info && info->deltime) {
            rr->LastAPTime += (2 * MERGE_DELAY_TIME);
        } else {
            rr->LastAPTime += MERGE_DELAY_TIME;
        }
    }
...
mDNSexport mStatus uDNS_UpdateRecord(mDNS *m, AuthRecord *rr)
{
    ...
    case regState_NoTarget:
        ...
        if (rr->UpdateCallback) {
            rr->UpdateCallback(m, rr, rr->resrec.rdata, rr->resrec.rdlength);
        }
...
mDNSexport void uDNS_CheckCurrentQuestion(mDNS *const m)
{
    ...
    if (m->timenow - NextQSendTime(q) < 0) {
        return;
    }

    if (q->LongLived) {
        switch (q->state) {
        case LLQ_InitialRequest:
            startLLQHandshake(m, q);
            break;
        ...
        if (q->qDNSServer && q->qDNSServer->teststate != DNSServer_Disabled) {
            ...
            if (end > m->omsg.data && (q->qDNSServer->teststate != DNSServer_Failed || NoTestQuery(q))) {
                ...
                if (private) {
                    ...
                    if (!q->LocalSocket) {
                        err = mStatus_NoMemoryErr;    // If failed to make socket (should be very rare), we'll try again next time
                    } else {
            ...
            if (err) {
                debugf("ERROR: uDNS_idle - mDNSSendDNSMessage - %d", err);    // surpress syslog messages if we have no network
            } else {
...
mDNSexport void CheckNATMappings(mDNS *m)
{
    ...
    if (m->NATTraversals && rfc1918) {		// Do we need to open NAT-PMP socket to receive multicast announcements from router?
        ...
        if (m->SSDPSocket)      {
            debugf("CheckNATMappings destroying SSDPSocket %p", &m->SSDPSocket);
            mDNSPlatformUDPClose(m->SSDPSocket);
            m->SSDPSocket = mDNSNULL;
        }
    }
        ...
        if (HaveRoutable) {	// If not RFC 1918 address, our own address and port are effectively our external address and port
            ...
            if (m->timenow - cur->retryPortMap >= 0) {					// Time to do something with this mapping
                ...
                if (cur->ExpiryTime) {					// If have active mapping then set next renewal time halfway to expiry
                    ...
                    if (cur->retryInterval < NATMAP_INIT_RETRY) {
                        cur->retryInterval = NATMAP_INIT_RETRY;
                    } else if (cur->retryInterval < NATMAP_MAX_RETRY_INTERVAL / 2) {
                        cur->retryInterval *= 2;
                    } else {
                        cur->retryInterval = NATMAP_MAX_RETRY_INTERVAL;
                    }
...
mDNSlocal mDNSs32 CheckRecordUpdates(mDNS *m)
{
    ...
    for (rr = m->ResourceRecords; rr; rr = rr->next) {
        ...
        if (rr->state == regState_NoTarget) {
            debugf("CheckRecordUpdates: Record %##s in NoTarget", rr->resrec.name->c);
            continue;
        }
        // While we are waiting for the port mapping, we have nothing to do. The port mapping callback
        // will take care of this
        if (rr->state == regState_NATMap) {
            debugf("CheckRecordUpdates: Record %##s in NATMap", rr->resrec.name->c);
            continue;
        }
        if (rr->state == regState_Pending || rr->state == regState_DeregPending || rr->state == regState_UpdatePending ||
            ...
            if (rr->LastAPTime + rr->ThisAPInterval - m->timenow <= 0) {
                ...
                if (!rr->nta || mDNSIPv4AddressIsZero(rr->nta->Addr.ip.v4)) {
                    ...
                    //
                    SetRecordRetry(m, rr, 0);
                } else if (rr->state == regState_DeregPending) {
    ...
    for (d = m->DNSServers; d; d = d->next)
        if (d->penaltyTime) {
            if (m->timenow - d->penaltyTime >= 0) {
                ...
                d->penaltyTime = 0;
            } else if (m->NextuDNSEvent - d->penaltyTime > 0) {
...
mDNSexport void mDNS_AddSearchDomain(const domainname *const domain, mDNSInterfaceID InterfaceID)
{
    ...
    for (p = &mDNS_SearchList; *p; p = &(*p)->next)
    ...
    if (tmp) {
        ...
        *p = mDNSPlatformMemAllocate(sizeof(SearchListElem));
        if (!*p) {
            LogMsg("ERROR: mDNS_AddSearchDomain - malloc");
            return;
        }
...
mDNSlocal void FoundDomain(mDNS *const m, DNSQuestion *question, const ResourceRecord *const answer, QC_result AddRecord)
{
    ...
    if (question == &slElem->BrowseQ) {
        name = mDNS_DomainTypeNames[mDNS_DomainTypeBrowse];
    } else if (question == &slElem->DefBrowseQ) {
        name = mDNS_DomainTypeNames[mDNS_DomainTypeBrowseDefault];
    } else if (question == &slElem->AutomaticBrowseQ) {
        name = mDNS_DomainTypeNames[mDNS_DomainTypeBrowseAutomatic];
    } else if (question == &slElem->RegisterQ) {
        name = mDNS_DomainTypeNames[mDNS_DomainTypeRegistration];
    } else if (question == &slElem->DefRegisterQ) {
        name = mDNS_DomainTypeNames[mDNS_DomainTypeRegistrationDefault];
    } else {
        LogMsg("FoundDomain - unknown question");
        return;
    }
    ...
    if (AddRecord) {
        ...
        if (!arElem) {
            LogMsg("ERROR: FoundDomain out of memory");
            return;
        }
...
mDNSexport void udns_validatelists(void *const v)
{
    ...
    SearchListElem *ptr;
    for (ptr = mDNS_SearchList; ptr; ptr = ptr->next)
...
{
    SearchListElem **p = &mDNS_SearchList, *ptr;
    mStatus err;
...
{
    SearchListElem *p = mDNS_SearchList;
    int count = *searchIndex;
    (void) m; // unused

    if (count < 0) {
        LogMsg("uDNS_GetNextSearchDomain: count %d less than zero", count);
        return mDNSNULL;
    }
