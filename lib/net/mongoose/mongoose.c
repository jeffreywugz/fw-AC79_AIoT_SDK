                         ...
                         struct mg_dns *dnsc, bool ipv6)
{
    ...
    if (dnsc->c == NULL) {
        ...
        LOG(LL_INFO, ("%lu resolving %.*s, txnid %hu", c->id,
                   ...
                   const char *pass)
{
    ...
    if (c->send.size >= need) {
        ...
        for (i = 0; i < u.len; i++) {
            n = mg_base64_update(u.ptr[i], buf, n);
        }
        if (p.len > 0) {
            n = mg_base64_update(':', buf, n);
            for (i = 0; i < p.len; i++) {
                n = mg_base64_update(p.ptr[i], buf, n);
            }
        }
...
int mg_http_parse(const char *s, size_t len, struct mg_http_message *hm)
{
    ...
    // Sanity check
    if (hm->method.len == 0 || hm->uri.len == 0 || hm->proto.len == 0) {
        return -1;
    }
                   ...
                   const char *dir)
{
    ...
    if (name[0] == '\0') {
        ...
        if ((fp = mg_fopen(path, oft == 0 ? "wb" : "ab")) == NULL) {
            ...
            fwrite((void *)hm->body.ptr, 1, hm->body.len, fp);
            fclose(fp);
            mg_http_reply(c, 200, "", "");
            return hm->body.len;
        }
...
#define MIME_ENTRY(_ext, _type) \
  { _ext, sizeof(_ext) - 1, _type }
    ...
    for (t = types; t->ext != NULL; t++) {
        ...
        if (mg_ncasecmp(t->ext, &filename[n - t->ext_len], t->ext_len)) {
            continue;
        }
        return t->type;
    }
    ...
    if (v != NULL && v->len > 6 && memcmp(v->ptr, "Basic ", 6) == 0) {
        ...
        for (i = 0; i < v->len - 13; i++) {
            if (memcmp(&v->ptr[i], "access_token=", 13) == 0) {
                ...
                while (p2 < &v->ptr[v->len] && p2[0] != ';' && p2[0] != ' ') {
                    p2++;
                }
...
static void mg_log_stdout(const void *buf, int len, void *userdata)
{
    (void) userdata;
    /*modify*/
    printf("mg_log_stdout len %d : %s\r\n", len, buf);
    /* fwrite(buf, 1, len, stdout); */
}

/* static const char *s_spec = "2"; */
static const char *s_spec = "3"; //modify
...
bool mg_log_prefix(int level, const char *file, int line, const char *fname)
{
    ...
    if (level <= max) {
        char timebuf[21], buf[128] = "";  //modify
        time_t t = time(NULL);
...
void mg_connect_resolved(struct mg_connection *c)
{
    ...
    if (err != ERR_OK) {
        mg_error(c, "%p failed, err %d", c->fd, err);
    }
}
                                ...
                                mg_event_handler_t fn, void *fn_data)
{
    ...
    /*modify*/
    mg_aton(host, &ipaddr);
    if (!mg_vcasecmp(&host, "localhost")) {
        ipaddr = mg_htonl(0x7f000001);
    }
                       ...
                       struct mg_mqtt_opts *opts)
{
    ...
    if (opts->clean || opts->client_id.len == 0) {
...
#if MG_ENABLE_IPV6
        if (addr.is_ip6) {
            af = AF_INET6, slen = sizeof(usa.sin6);
        }
#endif
...
void mg_connect_resolved(struct mg_connection *c)
{
    ...
    if (type == SOCK_STREAM) {
        ...
        /*modify:不知道为什么这里连接失败，也要将is_connecting置1*/
        /* if (rc < 0) c->is_connecting = 1; */

        /*modify:这里改为当连接成功后，才将is_connecting置1*/
        if (!fail) {
            c->is_connecting = 1;
        }
    }
...
static char *mg_ssi(const char *path, const char *root, int depth)
{
    ...
    if (fp != NULL) {
        ...
        if (b.len > 0) {
            mg_iobuf_append(&b, "", 1, align);    // nul-terminate
        }
        fclose(fp);
    }
    (void) depth;
...
EXTERN_C int mg_mbedtls_net_recv(void *, unsigned char *, size_t);
EXTERN_C int mg_mbedtls_net_send(void *, const unsigned char *, size_t);
...
int mg_mbedtls_net_send(void *ctx, const unsigned char *buf, size_t len)
{
    int ret;
    int *pt = (int *)ctx;
    int fd = (int)(*pt);

    ret = send((int)fd,  buf, len, 0);
    if (ret < 0) {
...
int mg_mbedtls_net_recv(void *ctx, unsigned char *buf, size_t len)
{
    int ret;
    int *pt = (int *)ctx;
    int fd = (int)(*pt);
...
int mg_tls_handshake(struct mg_connection *c)
{
    ...
    mbedtls_ssl_set_bio(&tls->ssl, &c->fd, mg_mbedtls_net_send, mg_mbedtls_net_recv, 0);
...
int mg_tls_init(struct mg_connection *c, struct mg_tls_opts *opts)
{
    ...
    if (opts->ca != NULL && opts->ca[0] != '\0') {
        ...
        /*modify*/
        rc = mbedtls_x509_crt_parse(&tls->ca, (uint8_t *) opts->ca, strlen(opts->ca) + 1);
        if (rc != 0) {
        ...
        /*modify: add*/
        /* rc = mbedtls_x509_crt_info(buf, sizeof(buf) - 1, "  ", &tls->ca);	 */
        /* if(rc > 0) */
        /* { */
        /* buf[rc] = '\0'; 	 */
        /* printf("%s\r\n", buf); */
    ...
    if (opts->cert != NULL && opts->cert[0] != '\0') {
        ...
        /*modify*/
        if ((rc = mbedtls_x509_crt_parse(&tls->cert, (uint8_t *) opts->cert, strlen(opts->cert) + 1)) != 0) {
            mg_error(c, "parse(%s) err %#x", opts->cert, -rc);
            goto fail;
        }

        /*modify*/
        if ((rc = mbedtls_pk_parse_key(&tls->pk, (const unsigned char *)key, strlen(key) + 1, NULL, 0)) != 0) {
...
unsigned short mg_url_port(const char *url)
{
    ...
    if (memcmp(url, "http:", 5) == 0 || memcmp(url, "ws:", 3) == 0) {
        port = 80;
    }
    if (memcmp(url, "wss:", 4) == 0 || memcmp(url, "https:", 6) == 0) {
        port = 443;
    }
    if (memcmp(url, "mqtt:", 5) == 0) {
        port = 1883;
    }
    if (memcmp(url, "mqtts:", 6) == 0) {
        port = 8883;
    }
    if (u.port) {
        port = atoi(url + u.port);
    }
    return port;
}
...
bool mg_file_write(const char *path, const void *buf, size_t len)
{
    ...
    if (fp != NULL) {
        printf("FIXME:Not Support, %s, %d\r\n", __FUNCTION__, __LINE__);
        result = fwrite((void *)buf, 1, len, fp) == len;
        fclose(fp);
        if (result) {
            printf("FIXME:Not Support, %s, %d\r\n", __FUNCTION__, __LINE__);
            /* remove(path); */
...
{
    int kvlen, kl;
    for (kvlen = 0; ofs + kvlen < slen && s[ofs + kvlen] != ',';) {
        kvlen++;
    }
...
char *mg_hexdump(const void *buf, size_t len)
{
    ...
    for (i = 0; i < len; i++) {
        ...
        if (idx == 0) {
            if (i > 0 && dlen > n) {
                n += snprintf(dst + n, dlen - n, "  %s\n", ascii);
            }
            if (dlen > n) {
                n += snprintf(dst + n, dlen - n, "%04x ", (int)(i + ofs));
            }
        }
        if (dlen < n) {
            break;
