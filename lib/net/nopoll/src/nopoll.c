...
extern int vsnprintf(char *buf, unsigned long size, const char *fmt, __builtin_va_list args);
extern int vsprintf(char *str, const char *format, va_list ap);

static int usleep(unsigned int t)
{
    os_time_dly(t / 10000);
    return 0;
}
                                 ...
                                 int          *output_size)
{
    ...
    size_t len = 0;
    if (mbedtls_base64_encode((unsigned char *)output, *output_size, &len, (const unsigned char *)content, length) != 0) {
...
#if defined(NOPOLL_MBEDTLS_ENABLED)
    size_t len = 0;
    if (mbedtls_base64_decode((unsigned char *)output, *output_size, &len, (const unsigned char *)content, length) != 0) {
        printf("mbedtls_base64_decode failed\n");
        return nopoll_false;
    } else {
        return nopoll_true;
    }
