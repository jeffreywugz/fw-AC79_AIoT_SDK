...
#if defined(NOPOLL_MBEDTLS_ENABLED)
#include "lwip/sockets.h"
#include "lwip/netdb.h"
#include "nopoll_mbedtls.h"
#else
#include "openssl/ssl.h"
#endif // NOPOLL_MBEDTLS_ENABLED
...
static int nopoll_net_would_block(int fd)
{
...
static int nopoll_mbedtls_net_send(void *ctx, const unsigned char *buf, size_t len)
{
    int ret;
    int fd = *((int *)ctx);
    int flags;
    ...
    if (ret < 0)     {
        if (nopoll_net_would_block(fd) != 0) {
 ...
 */
static int nopoll_mbedtls_net_recv(void *ctx, unsigned char *buf, size_t len)
{
    int ret;
    int fd = *((int *)ctx);
    int flags;
...
nopoll_bool                 nopoll_conn_set_sock_block(NOPOLL_SOCKET socket,
        nopoll_bool   enable)
{
#if defined(NOPOLL_OS_UNIX) || defined(NOPOLL_LWIP_ENABLED)
    int  flags;
#endif
...
#if !defined(NOPOLL_LWIP_ENABLED)
/**
...
{
#if defined(NOPOLL_MBEDTLS_ENABLED)
    return mbedtls_ssl_read(&conn->ssl_ctx->ssl, (unsigned char *)buffer, buffer_size);
#else
    int res;
...
{
#if defined(NOPOLL_MBEDTLS_ENABLED)
    return mbedtls_ssl_write(&conn->ssl_ctx->ssl, (unsigned char *)buffer, buffer_size);
#else
    int         res;
    nopoll_bool needs_retry;
                                     ...
                                     const char       *origin)
{
    ...
    if (enable_tls) {
        ...
        int ready;
        int ret;
        char *pers = "wss_client";

        /* found TLS connection request, enable it */
        conn->ssl_ctx = nopoll_new(struct nopoll_ssl, 1);
        if (conn->ssl_ctx == NULL) {
            printf("nopoll_new fail, %s, %d\n", __FUNCTION__, __LINE__);
            nopoll_conn_shutdown(conn);
        ...
        mbedtls_ssl_init(&conn->ssl_ctx->ssl);
        mbedtls_ssl_config_init(&conn->ssl_ctx->conf);
        mbedtls_x509_crt_init(&conn->ssl_ctx->own_cert);
        mbedtls_pk_init(&conn->ssl_ctx->pkey);
        mbedtls_entropy_init(&conn->ssl_ctx->entropy);
        mbedtls_ctr_drbg_init(&conn->ssl_ctx->ctr_drbg);
        mbedtls_ssl_conf_dbg(&conn->ssl_ctx->conf, mbedtls_debug, 0);

        printf("  . 1 mbedtls init ... ok \r\n");

        if (0 != (ret = mbedtls_ctr_drbg_seed(&conn->ssl_ctx->ctr_drbg, mbedtls_entropy_func, &conn->ssl_ctx->entropy, \
                                              (const unsigned char *)pers, strlen(pers)))) {
            //nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "  . ! mbedtls_ctr_drbg_seed returned err !!! %d\r\n", ret);
            printf("  . ! mbedtls_ctr_drbg_seed returned err !!! %d\r\n", ret);
            goto ssl_exit;
        }

        if (options && options->certificate) {
            if ((ret = mbedtls_x509_crt_parse(&conn->ssl_ctx->own_cert, (const unsigned char *)options->certificate, \
                                              options->certificate_size))) {
                //nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "  . ! mbedtls_x509_crt_parse returned err !!! -0x%x\r\n", -ret);
                printf("  . ! [%s]mbedtls_x509_crt_parse returned err !!! -0x%x\r\n", "certificate", -ret);
                goto ssl_exit;
            }
        }

        if (options && options->chain_certificate) {
            if ((ret = mbedtls_x509_crt_parse(&conn->ssl_ctx->own_cert, (const unsigned char *)options->chain_certificate, \
                                              options->chain_certificate_size))) {
                //nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "  . ! mbedtls_x509_crt_parse returned err !!! -0x%x\r\n", -ret);
                printf("  . ! [%s]mbedtls_x509_crt_parse returned err !!! -0x%x\r\n", "chain_certificate", -ret);
                goto ssl_exit;
            }
        }

        if (options && options->ca_certificate) {
            if (0 != mbedtls_x509_crt_parse(&conn->ssl_ctx->ca_cert, (const unsigned char *)options->ca_certificate, \
        ...
        printf("  . 2 Loading the CA root certificate ... ok \r\n");

        if (options && options->private_key) {
            if ((ret = mbedtls_pk_parse_key(&conn->ssl_ctx->pkey, (const unsigned char *)options->private_key, \
                                            options->private_key_size, NULL, 0))) {
                //nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "  . ! mbedtls_pk_parse_key returned err !!! %d\r\n", ret);
                printf("  . ! mbedtls_pk_parse_key returned err !!! %d\r\n", ret);
                goto ssl_exit;
            }
        }

        if ((ret = mbedtls_ssl_config_defaults(&conn->ssl_ctx->conf, MBEDTLS_SSL_IS_CLIENT, MBEDTLS_SSL_TRANSPORT_STREAM, \
                                               MBEDTLS_SSL_PRESET_DEFAULT))) {
            //nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "  . ! mbedtls_ssl_config_defaults returned err !!! %d\r\n", ret);
            printf("  . ! mbedtls_ssl_config_defaults returned err !!! %d\r\n", ret);
            goto ssl_exit;
        }

        printf("  . 3 Setting up the SSL/TLS structure... ok\r\n");

        if (options && (nopoll_true == options->disable_ssl_verify)) {
            mbedtls_ssl_conf_authmode(&conn->ssl_ctx->conf, MBEDTLS_SSL_VERIFY_NONE);

            //该选择验证不过时，继续通信
            //mbedtls_ssl_conf_authmode(&conn->ssl_ctx->conf, MBEDTLS_SSL_VERIFY_OPTIONAL);
        } else {
            mbedtls_ssl_conf_authmode(&conn->ssl_ctx->conf, MBEDTLS_SSL_VERIFY_REQUIRED);

            //该选择验证不过时，继续通信
            //mbedtls_ssl_conf_authmode(&conn->ssl_ctx->conf, MBEDTLS_SSL_VERIFY_OPTIONAL);
        }

        mbedtls_ssl_conf_rng(&conn->ssl_ctx->conf, mbedtls_ctr_drbg_random, &conn->ssl_ctx->ctr_drbg);
        mbedtls_ssl_conf_ca_chain(&conn->ssl_ctx->conf, &conn->ssl_ctx->ca_cert, NULL);

        if ((ret = mbedtls_ssl_conf_own_cert(&conn->ssl_ctx->conf, &conn->ssl_ctx->own_cert, &conn->ssl_ctx->pkey))) {
            //nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "  . ! mbedtls_ssl_conf_own_cert returned err !!! %d\r\n", ret);
            printf("  . ! mbedtls_ssl_conf_own_cert returned err !!! %d\r\n", ret);
            goto ssl_exit;
        }

        if ((ret = mbedtls_ssl_setup(&conn->ssl_ctx->ssl, &conn->ssl_ctx->conf))) {
            //nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "  . ! mbedtls_ssl_setup returned err !!! %d\r\n", ret);
            printf("  . ! mbedtls_ssl_setup returned err !!! %d\r\n", ret);
            goto ssl_exit;
        }

        conn->ssl_ctx->fd = session;
        mbedtls_ssl_set_bio(&conn->ssl_ctx->ssl, &conn->ssl_ctx->fd, nopoll_mbedtls_net_send, nopoll_mbedtls_net_recv, NULL);
        ...
        while (nopoll_true) {
            ...
            ready = select(session + 1, NULL, &wset, NULL, &timeout);
            if (0 == ready) {
                if (iterator > 1000) {
                ...
                nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "select failed, ready %d\n", ready);
                goto ssl_exit;
            } else {
                break;
            }
        ...
        printf("connecting to remote address (host : %s, port : %s)", conn->host, conn->port);

        iterator = 0;
        while ((ssl_error = mbedtls_ssl_handshake(&conn->ssl_ctx->ssl)) != 0) {
            if ((ret = mbedtls_ssl_get_verify_result(&conn->ssl_ctx->ssl)) != 0) {
                char vrfy_buf[512] = {0};
            ...
            if ((ssl_error != MBEDTLS_ERR_SSL_WANT_READ) && (ssl_error != MBEDTLS_ERR_SSL_WANT_WRITE)) {
                printf("  . ! mbedtls_ssl_handshake returned err !!! -0x%x\r\n", -ssl_error);
                goto ssl_exit;
            }
        ...
        printf("  . 4 Performing the SSL/TLS handshake... ok \r\n");

        if (0 != mbedtls_ssl_get_verify_result(&conn->ssl_ctx->ssl)) {
            printf("  . ! The certificate is signed with an unacceptable hash\r\n");
            //goto ssl_exit;
        } else {
            printf("  . Verifying peer X.509 certificate... ok\r\n");
        }
    ...
    //printf("Sending websocket client init: %s", content);
...
ssl_exit:
    if (conn->ssl_ctx) {
        mbedtls_ssl_free(&conn->ssl_ctx->ssl);
        mbedtls_ssl_config_free(&conn->ssl_ctx->conf);
        mbedtls_x509_crt_free(&conn->ssl_ctx->own_cert);
        mbedtls_pk_free(&conn->ssl_ctx->pkey);
        mbedtls_entropy_free(&conn->ssl_ctx->entropy);
        mbedtls_ctr_drbg_free(&conn->ssl_ctx->ctr_drbg);
        nopoll_free(conn->ssl_ctx);
        conn->ssl_ctx = NULL;
    }
...
void nopoll_conn_unref(noPollConn *conn)
{
    ...
    if (conn->ssl_ctx != NULL) {
        ...
        mbedtls_ssl_config_free(&conn->ssl_ctx->conf);
        mbedtls_x509_crt_free(&conn->ssl_ctx->own_cert);
        mbedtls_x509_crt_free(&conn->ssl_ctx->ca_cert);
        mbedtls_pk_free(&conn->ssl_ctx->pkey);
        mbedtls_entropy_free(&conn->ssl_ctx->entropy);
        mbedtls_ctr_drbg_free(&conn->ssl_ctx->ctr_drbg);
        nopoll_free(conn->ssl_ctx);
...
#if defined(NOPOLL_MBEDTLS_ENABLED)
        ssl_error = mbedtls_ssl_handshake(&conn->ssl_ctx->ssl);
        if (0 != ssl_error) {
            printf("  . ! mbedtls_ssl_handshake returned err !!! -0x%x\r\n", -ssl_error);
            switch (ssl_error) {
 ...
 *
 * @param on_close The handler nopoll_conn_acceptto be configured.
 *
...

{
    ...
    if (masked) {
        ...
        mask_value = (unsigned int) rand32();
#endif
        memset(mask, 0, 4);
...
nopoll_bool __nopoll_conn_accept_complete_common(noPollCtx *ctx, noPollConnOpts *options, noPollConn *listener, noPollConn *conn, NOPOLL_SOCKET session, nopoll_bool tls_on)
{
    ...
    if (listener->tls_on || tls_on) {
        ...
        mbedtls_entropy_init(&conn->ssl_ctx->entropy);
        mbedtls_ctr_drbg_init(&conn->ssl_ctx->ctr_drbg);
        mbedtls_ssl_conf_dbg(&conn->ssl_ctx->conf, mbedtls_debug, NULL);

        printf("  . 1 mbedtls init ... ok \r\n");

        if ((ret = mbedtls_x509_crt_parse(&conn->ssl_ctx->own_cert, (const unsigned char *)certificateFile, \
                                          certificateFile_size))) {
            //nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "  . ! mbedtls_x509_crt_parse returned err !!! -0x%x\r\n", -ret);
            printf("  . ! [%s]mbedtls_x509_crt_parse returned err !!! -0x%x\r\n", "certificate", -ret);
            goto ssl_exit;
        }
        printf("the certificate : %s\n", certificateFile);

        if (chainCertificate) {
            if ((ret = mbedtls_x509_crt_parse(&conn->ssl_ctx->own_cert, (const unsigned char *)chainCertificate, chainCertificate_size))) {
                //nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "  . ! mbedtls_x509_crt_parse returned err !!! -0x%x\r\n", -ret);
                printf("  . ! [%s]mbedtls_x509_crt_parse returned err !!! -0x%x\r\n", "certificate", -ret);
                goto ssl_exit;
            }
        ...
        if (options && options->ca_certificate) {
            if ((ret = mbedtls_x509_crt_parse(&conn->ssl_ctx->ca_cert, (const unsigned char *)options->ca_certificate, options->ca_certificate_size))) {
                //nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "  . ! mbedtls_x509_crt_parse returned err !!! -0x%x\r\n", -ret);
                printf("  . ! [%s]mbedtls_x509_crt_parse returned err !!! -0x%x\r\n", "certificate", -ret);
                goto ssl_exit;
            }
        ...
        if ((ret = mbedtls_pk_parse_key(&conn->ssl_ctx->pkey, (const unsigned char *) privateKey, privateKey_size, NULL, 0))) {
            //nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "  . ! mbedtls_pk_parse_key returned err !!! %d\r\n", ret);
            printf("  . ! mbedtls_pk_parse_key returned err !!! %d\r\n", ret);
            goto ssl_exit;
        }
        ...
        if ((ret = mbedtls_ctr_drbg_seed(&conn->ssl_ctx->ctr_drbg, mbedtls_entropy_func, &conn->ssl_ctx->entropy, (const unsigned char *)pers, strlen(pers)))) {
            //nopoll_log(ctx, NOPOLL_LEVEL_CRITICAL, "  . ! mbedtls_ctr_drbg_seed returned err !!! %d\r\n", ret);
            printf("  . ! mbedtls_ctr_drbg_seed returned err !!! %d\r\n", ret);
            goto ssl_exit;
        }

        if ((ret = mbedtls_ssl_config_defaults(&conn->ssl_ctx->conf, MBEDTLS_SSL_IS_SERVER, MBEDTLS_SSL_TRANSPORT_STREAM, MBEDTLS_SSL_PRESET_DEFAULT))) {
        ...
        mbedtls_ssl_session_reset(&conn->ssl_ctx->ssl);
