...
#endif
{
    ...
    if ((listener->certificate == NULL) || (listener->private_key == NULL) || (chain_file && (listener->chain_certificate == NULL))) {
        printf("nopoll_listener_set_certificate : nopoll_new fail\n");
...
#else
    nopoll_return_val_if_fail(ctx, ctx && (session != -1), NULL);
#endif
