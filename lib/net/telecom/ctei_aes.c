...
{
    return ((int)(slen / 16) + 1) * 16;
//return (slen%16 == 0) ? (slen+16) : ((int)(slen/16) + 1)*16;
}

unsigned char *encode(aes_context *ctx, int mode, const unsigned char *key, const unsigned char *input, int slen)
{
    int dlen = encode_length(slen);
    int len = 0;
    unsigned char *buffer;
    unsigned char *output = (unsigned char *)malloc(dlen);
    unsigned char *reg = output;
    do {
        len = (slen < 16) ? slen : 16;
        aes_setkey_enc(ctx, key, KEY_SIZE);
        buffer = aes_crypt_ecb(ctx, AES_ENCRYPT, input, len, &dlen);
        slen -= len;
        input += len;

        memcpy(reg, buffer, dlen);
        free(buffer);
        reg += 16;
    } while (slen > 0);
