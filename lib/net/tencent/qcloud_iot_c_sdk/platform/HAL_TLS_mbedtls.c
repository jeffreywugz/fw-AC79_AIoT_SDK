...
uintptr_t HAL_TLS_Connect(TLSConnectParams *pConnectParams, const char *host, int port)
{
    ...
    Log_i("Before TLS connect, free heap size: %d", xPortGetFreeHeapSize());

    TLSDataParams *pDataParams = (TLSDataParams *)HAL_Malloc(sizeof(TLSDataParams));
    if (pDataParams == NULL) {
        Log_e("TLSDataParams malloc failed!");
        return 0;
    }
...
#else
    mbedtls_ssl_conf_rng(&(pDataParams->ssl_conf), HAL_random, NULL);
#endif
