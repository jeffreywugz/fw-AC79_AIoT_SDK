...
#define DEFAULT_ARREARS_TYPE 	OPEN_AUDIO				//打开音频播放

static void save_arrears_info(int enable)
{
    if (!enable) {
        return;
    }
...
void load_arrears_info(void)
{
    ...
    //读取psk_info的信息
    norflash_read(NULL, data, 1 * 1024, addr);

    if (data[128] == 1) {	//如果enable ==  1
        arrears_type = data[129];
    } else {
...
static void _on_template_downstream_topic_handler(void *pClient, MQTTMessage *message, void *pUserdata)
{
    ...
    if (!strcmp(type_str, CONTROL_CMD)) {
        ...
        if (parse_template_cmd_control(sg_template_cloud_rcv_buf, &control_str)) {
            ...
            cJSON *root = NULL;
            u8 enable = 0;
            root = cJSON_Parse(control_str);
            if (!root) {
                printf("root NULL\n\r");
                goto __exit;
            }
            cJSON *cmd = cJSON_GetObjectItem(root, "_sys_tvs_auth_cmd");
            if (!cmd || !cmd->valuestring) {
                printf("cmd is NULL\n\r");
                goto __exit;
            }
#if ARREARS_ENABLE	//TEST：目前测试只能用网页的那个"tvs授权下行命令"下发 如果有具体的下发内容自行修改到对应的地方
            if (!strcmp(cmd->valuestring, "58")) {
                arrears_type = CLOSE_WITH_PROMPT;
                enable = 1;
                report_dev_sys_info();
            } else if (!strcmp(cmd->valuestring, "59")) {
                arrears_type = CLOSE_WITHOUT_PROMPT;
                enable = 1;
                report_dev_sys_info();
            } else if (!strcmp(cmd->valuestring, "60")) {
                arrears_type = OPEN_AUDIO;
                enable = 1;
                report_dev_sys_info();
            } else if (!strcmp(cmd->valuestring, "sys_info_report")) {
                report_dev_sys_info();
            }
#endif
#if OTA_MQTT_ENABLE
            else if (!strcmp(cmd->valuestring, "sys_ota_report")) {
                report_ota_info();
            }
#endif
#if ARREARS_ENABLE
