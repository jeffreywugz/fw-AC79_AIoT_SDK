                                     ...
                                     uint32_t *serialized_len)
{
    ...
    if (options->device_secret != NULL && options->username != NULL) {
        ...
        extern int8_t utils_hb2hex(uint8_t hb);
        mbedtls_md_hmac(&mbedtls_sha1_info, (const unsigned char *)options->device_secret, options->device_secret_len, (const unsigned char *)options->username, strlen(options->username), (unsigned char *)sign);
        char tmp_buf[64] = {0};
        for (int i = 0; i < 20; ++i) {
            tmp_buf[i * 2]     = utils_hb2hex(sign[i] >> 4);
            tmp_buf[i * 2 + 1] = utils_hb2hex(sign[i]);
        }
        ...
        HAL_Snprintf(options->password, 51, "%s;hmacsha1", tmp_buf);
    }
#endif
