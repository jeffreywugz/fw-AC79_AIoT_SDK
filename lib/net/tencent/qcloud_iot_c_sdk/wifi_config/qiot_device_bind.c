...
bool sg_token_received                  = true;
bool sg_reply_flag						= true;
...
static void _on_message_callback(void *pClient, MQTTMessage *message, void *userData)
{
    ...
    if (!root) {
        ...
        if (code_json) {
            ...
            sg_bind_reply_code = app_data->reply_code;      //yii:返回token验证
            sg_reply_flag = true;		//得到了reply code
...
static int _mqtt_send_token(void)
{
    ...
    sg_token_received 	= false;	//有配网操作，清空标志位
    sg_reply_flag		= false;
