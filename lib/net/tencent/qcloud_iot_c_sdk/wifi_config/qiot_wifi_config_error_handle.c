...
static const char *const g_err_log[] = {
...
int get_and_post_error_log(comm_peer_t *peer)
{
    ...
    do {
        rc = HAL_QueueItemPop(g_err_log_queue, &err_msg, 0);
        if (rc == true) {
...
int save_error_log(void)
{
    ...
    uint32_t i = 0, j = 0;
    do {
        rc = HAL_QueueItemPop(g_err_log_queue, &log_src->err_log[i], 0);
        if (rc == true) {
