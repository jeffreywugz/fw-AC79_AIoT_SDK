...
#ifndef MG_MALLOC
#define MG_MALLOC TVS_MALLOC
#endif

#ifndef MG_CALLOC
#define MG_CALLOC TVS_CALLOC
#endif

#ifndef MG_REALLOC
#define MG_REALLOC TVS_REALLOC
#endif

#ifndef MG_FREE
#define MG_FREE TVS_FREE
#endif
...
MG_INTERNAL int mg_parse_address(const char *str, union socket_address *sa,
                                 int *proto, char *host, size_t host_len, unsigned int force_ip);
...
#define LOGPRINT(fmt, args...)          \
	do {								\
		if (1) 						    \
			TVS_SYS_PRINTF("[mongoose]" fmt, ##args);		    \
	} while (0)
...
#ifndef MBUF_REALLOC
#define MBUF_REALLOC TVS_REALLOC
#endif

#ifndef MBUF_FREE
#define MBUF_FREE TVS_FREE
#endif
...
{
    return mg_ncasecmp(s1, s2, (size_t) 0xffff);
}
...
time_t mg_mgr_poll(struct mg_mgr *m, int timeout_ms)
{
    ...
    for (i = 0; i < m->num_ifaces; i++) {
        now = m->ifaces[i]->vtable->mgpoll(m->ifaces[i], timeout_ms);
    }
    return now;
}
...
int mg_resolve2(const char *host, struct in_addr *ina)
{
...
unsigned int mg_htonl(unsigned int n)
{
    return htonl(n);
}

//添加自定义函数
static int get_host_port(const char *str, char *host, unsigned int *port, int *str_len)
{
    char *pt = NULL;
    int len = 0;

    if (NULL == str || NULL == port || NULL == host) {
        return -1;
    }
    ...
    *port = atoi(++pt);
    ...
    printf("str = %s,   %d  host = %s  port = %d", str, *str_len, host, *port);
    return 2;
}
                                 ...
                                 int *proto, char *host, size_t host_len, unsigned int force_ip)
{
    ...
    *proto = SOCK_STREAM;
    ...
    if (sscanf(str, "%u.%u.%u.%u:%u%n", &a, &b, &c, &d, &port, &len) == 5) {
        ...
        sa->sin.sin_port = htons((uint16_t) port);
#endif
#if (CS_PLATFORM == CS_P_FREERTOS || CS_PLATFORM == CS_P_RTTHREAD)
    } else if (strlen(str) < host_len
               //    && sscanf(str, "%[^ :]:%u%n", host, &port, &len) == 2) {
               && get_host_port(str, host, &port, &len) == 2) {

        sa->sin.sin_port = htons((uint16_t) port);
        if (force_ip != 0) {
            memcpy(&sa->sin.sin_addr, &force_ip, sizeof(struct in_addr));
...
#endif // if CS_PLATFORM == CS_P_FREERTOS
        ...
        union socket_address *sa)
{
    ...
    //TVS_SYS_PRINTF("%p %s://%s:%hu\n", nc, proto == SOCK_DGRAM ? "udp" : "tcp",
    //   inet_ntoa(sa->sin.sin_addr), ntohs(sa->sin.sin_port));
                                     ...
                                     struct mg_connect_opts opts)
{
    ...
    /* printf("addr = %s host = %s force ip  = %d", address, host, mgr->force_ip); */
    if ((rc = mg_parse_address(address, &nc->sa, &proto, host, sizeof(host), mgr->force_ip)) <
        0) {
...
//TVS_SYS_PRINTF("**************parse addr time %f, host %s, addr %s*************\n", mg_time() - time, host, address);
                                  ...
                                  struct mg_bind_opts opts)
{
    ...
    if (mg_parse_address(address, &sa, &proto, host, sizeof(host), mgr->force_ip) <= 0) {
        MG_SET_PTRPTR(opts.error_string, "cannot parse address");
        return NULL;
    }
...
double mg_set_timer(struct mg_connection *c, double timestamp)
{
     ...
     */
    DBG(("%p %p %d -> %lu", c, c->priv_2, (int)(c->flags & MG_F_RESOLVING),
...
{
#if (CS_PLATFORM == CS_P_FREERTOS || CS_PLATFORM == CS_P_RTTHREAD)
    double time = os_wrapper_get_time_ms();
    return time / 1000;
#else
    return cs_time();
#endif
...
static void mg_handle_tcp_read(struct mg_connection *conn)
{
    ...
    memset(buf, 0, MG_TCP_RECV_BUFFER_SIZE);
        ...
        uint32_t stream_id)
{
    ...
    if (stream_id > client->last_stream_id) {
        ...
        LOG(LL_DEBUG, ("new stream 0x%lx, accepting", (long unsigned int)stream_id));
        nc = mg_if_accept_new_conn(client->listener);
        nc->mgr_data = (void *)(uintptr_t) stream_id;
        client->last_stream_id = stream_id;
    } else {
        LOG(LL_DEBUG, ("Ignoring stream 0x%lx (last_stream_id 0x%lx)", (long unsigned int)stream_id,
                       (long unsigned int)client->last_stream_id));
    }
...
int pk_parse_key(mbedtls_pk_context *ctx,
        ...
        const char *ca_cert)
{
    ...
    if (mbedtls_x509_crt_parse(ctx->ca_cert, (const unsigned char *)ca_cert, strlen(ca_cert)) != 0) {
        return MG_SSL_ERROR;
    }
        ...
        const char **err_msg)
{
    ...
    if (pk_parse_key(ctx->key, key, NULL) != 0) {
        MG_SET_PTRPTR(err_msg, "Invalid SSL key");
        return MG_SSL_ERROR;
    }
...
int mg_parse_http(const char *s, int n, struct http_message *hm, int is_req)
{
    ...
    hm->message.p = s;
    hm->body.p = s + len;
    hm->message.len = hm->body.len = (size_t) 0xffff;
    end = s + len;
     ...
     * body is read until socket is closed, i.e. body.len is infinite (0xffff).
     *
     ...
     */
    if (hm->body.len == (size_t) 0xffff && is_req &&
    ...
    if (pd->file.type == DATA_FILE) {
        ...
        if (to_read == 0) {
            /* Rate limiting. send_mbuf is too full, wait until it's drained. */
                   ...
                   (n = fread(pd->file.fp, buf,  1 * to_read)) > 0) {
            mg_send(nc, buf, n);
            pd->file.sent += n;
        } else {
        ...
        size_t n = fwrite(pd->file.fp, io->buf, 1 * to_write);
        if (n > 0) {
            mbuf_remove(io, n);
            pd->file.sent += n;
        }
                                     ...
                                     size_t blen)
{
    size_t c_len = blen;
    struct mg_http_proto_data *pd = mg_http_get_proto_data(nc);
    struct phr_chunked_decoder *decoder = &(pd->chunk.chunked_decoder);
    decoder->consume_trailer = 1;

    phr_decode_chunked(decoder, buf, &c_len);

    hm->body.len = c_len;

    //TVS_SYS_PRINTF("%.*s", hm->body.len, hm->body.p);
    mg_call(nc, nc->handler, MG_EV_HTTP_CHUNK, hm);

    nc->recv_mbuf.len -= blen;

    if (decoder->_state == 4) {
        hm->message.len = nc->recv_mbuf.len;
    }
...
void mg_http_handler(struct mg_connection *nc, int ev, void *ev_data)
{
    ...
    if (ev == MG_EV_RECV) {
        ...
        if ((req_len < 0 ||
            ...
            if (v7 != NULL) {
                /* Lookup JS callback */
                v1 = v7_get(v7, v7_get_global(v7), "Http", 0xffff);
                v2 = v7_get(v7, v1, ev_name, 0xffff);
                ...
                v7_set(v7, req, "method", 0xffff,
                       v7_mk_string(v7, hm->method.p, hm->method.len, 1));
                v7_set(v7, req, "uri", 0xffff, v7_mk_string(v7, hm->uri.p, hm->uri.len, 1));
                v7_set(v7, req, "body", 0xffff,
                       v7_mk_string(v7, hm->body.p, hm->body.len, 1));
                v7_set(v7, req, "headers", 0xffff, headers);
                for (i = 0; hm->header_names[i].len > 0; i++) {
                            ...
                            mg_fu_fname_fn local_name_fn)
{
    ...
    case MG_EV_HTTP_PART_DATA: {
        ...
        if (fwrite(fus->fp, mp->data.p, 1 * mp->data.len) != mp->data.len) {
...
//double time = mg_time();
    ...
    //TVS_SYS_PRINTF("**************connect base time %f*************\n", mg_time() - time);
    return nc;
}
...
static void mg_send_file_data(struct mg_connection *nc, FILE *fp)
{
    ...
    while ((n = fread(fp, buf, 1 * sizeof(buf))) > 0) {
        mg_send(nc, buf, n);
    }
}
...
{
    int status_code = 500;
    if (hm->body.len != (size_t) 0xffff && hm->body.len > 0) {
        status_code = 415;
...
void mg_tun_log_frame(struct mg_tun_frame *frame)
{
    LOG(LL_DEBUG, ("Got TUN frame: type=0x%x, flags=0x%x stream_id=0x%lx, "
                   "len=%zu",
                   frame->type, frame->flags, (long unsigned int)frame->stream_id, frame->body.len));
#if MG_ENABLE_HEXDUMP
    {
        char hex[512];
                                  ...
                                  void *ev_data)
{
    ...
    case MG_EV_WEBSOCKET_FRAME: {
        ...
        if (mg_tun_parse_frame(wm->data, wm->size, &frame) == -1) {
            LOG(LL_ERROR, ("Got invalid tun frame dropping %d", wm->size));
            break;
        }
...
bool pem_to_der(const char *pem_file, const char *der_file)
{
    ...
    while (1) {
        char pem_buf[70];
        char der_buf[48];
        if (!fgets(pem_buf, sizeof(pem_buf), pf)) {
            break;
        }
        if (writing) {
            ...
            int l = 0;
            while (!isspace((unsigned int) pem_buf[l])) {
                l++;
            }
            int der_len = 0;
            ...
            // if (fwrite(der_buf, 1, der_len, df) != der_len)
            if (fwrite(df, der_buf, 1 * der_len) != der_len) {
                break;
            }
