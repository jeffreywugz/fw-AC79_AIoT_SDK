...
/*uchttp只能初始化一次*/
CPU_BOOLEAN HTTPc_Is_InitDone(void)
{
    return HTTPc_InitDone;
}
                         ...
                         HTTPc_ERR         *p_err)
{
    ...
    case HTTPc_PARAM_TYPE_USER_SECURE_DATA:        //增加
        p_conn->UserData = (void *)p_param;
        break;

    case HTTPc_PARAM_TYPE_SECURE_MUTUAL:        //增加,ssl_tsl安全连接参数设置
#ifdef NET_SECURE_MODULE_EN
        p_conn->SockSecureCfg.MutualAuthPtr = (void *)p_param;
        break;
#else
        *p_err = HTTPc_ERR_FEATURE_DIS;
        goto exit;
