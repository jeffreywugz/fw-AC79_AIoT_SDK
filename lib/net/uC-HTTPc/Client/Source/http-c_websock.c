...
static  CPU_BOOLEAN  HTTPcWebSock_IsProtocolError(HTTPc_ERR                err);
...
*
* Description : Process all the states for a HTTP connection that has been upgraded to a WebSocket.
*
* Argument(s) : p_conn  Pointer to current HTTPc Connection (WebSocket Upgraded).
*
* Return(s)   : none.
*
...
*
* Description : Initialize the WebSocket Request object.
*
...
*
* Return(s)   : If the operation is successful, pointer to the WebSocket Request object to initialize.
*               If the operation has failed, DEF_NULL;
*
* Caller(s)   : HTTPcWebSock_Upgrade().
*
* Note(s)     : none.
*********************************************************************************************************
*/
...
HTTPc_WEBSOCK_REQ  *HTTPcWebSock_InitReqObj(HTTPc_ERR  *p_err)
{
    ...
    if (net_err != NET_ERR_NONE) {
        *p_err = HTTPc_ERR_WEBSOCK_BASE64_ENCODE_FAIL;
        goto exit_release;
    }
    ...
    NetSHA1_Reset(&sha1_ctx,
                  &sha1_err);
    if (sha1_err != NET_SHA1_ERR_NONE) {
        *p_err = HTTPc_ERR_WEBSOCK_SHA1_ENCODE_FAIL;
        goto exit_release;
    }
...
*
* Description : Add a message in the TX message queue.
*
* Argument(s) : p_msg   Pointer to the message to queue
*
...
*                                          HTTPcWebSock_TX()
*
* Description : Process the WebSocket states TX.
*
* Argument(s) : p_conn  Pointer to current HTTPc Connection (WebSocket Upgraded).
*
* Return(s)   : none.
*
* Caller(s)   : HTTPcConn_WebSocketProcess().
*
* Note(s)     : none.
*********************************************************************************************************
*/
    ...
    if (p_msg != DEF_NULL) {
        do {
            ...
            case HTTPc_WEBSOCK_TX_STATE_SET_BUF:
                ...
                HTTPcWebSock_SetTxBuf(p_msg, p_conn->TxDataPtr, p_src_buf, len);
            ...
            case HTTPc_WEBSOCK_TX_STATE_SEND_BUF:
                ...
                if ((sock_rdy_wr  == DEF_YES) ||
                    ...
                    (void)HTTPcSock_ConnDataTx(p_conn, &err);
                    switch (err) {
                    case HTTPc_ERR_NONE:
            ...
            case HTTPc_WEBSOCK_TX_STATE_COMPLETE:
                ...
                default:
                    err = HTTPc_ERR_WEBSOCK_INVALID_OPCODE;
                    goto exit_err;
                }
            ...
            default:
                err = HTTPc_ERR_WEBSOCK_INVALID_TX_STATE;
                goto exit_err;
            }
        } while (done == DEF_NO);
    }
...
*                                      HTTPcWebSock_RX()
*
* Description : Process the WebSocket states for RX.
*
* Argument(s) : p_conn  Pointer to current HTTPc Connection (WebSocket Upgraded).
*
* Return(s)   : none.
...
static  void  HTTPcWebSock_RX(HTTPc_CONN  *p_conn)
{
    ...
    do {
        ...
        case HTTPc_WEBSOCK_RX_STATE_HDR:                    /* Process the msg header.                              */
            ...
            rem_rx_len = HTTPcWebSock_GetMsgHdr(p_conn, &err);
            if (err != HTTPc_ERR_NONE) {
                goto exit_err;
            }
                ...
                if (p_ws->Flags.IsCloseStarted == DEF_YES) {
                    ...
                    if (p_ws->Flags.IsTxMsgCtrlUsed == DEF_NO) {
                        ...
                        if (is_std == DEF_YES) {
                            ...
                            err = HTTPc_ERR_WEBSOCK_RX_CLOSE_CODE_INVALID;
                            goto exit_err;
                        }
                    }
                    p_conn->RxBufPtr += p_ws->RxMsgLen;
        ...
        default:
            err = HTTPc_ERR_WEBSOCK_INVALID_RX_STATE;
            goto exit_err;
        }
        /* --------------------- RX DATA ---------------------- */
        ...
        if (result == DEF_YES) {                                /* If more data is needed...                            */
            ...
            if (p_ws->Flags.IsRxDataCached == DEF_NO) {         /* ...and If there is no rx data cached for ctrl msg... */
                ...
                if ((sock_rdy_rd  == DEF_YES) ||                /* ...and if the sock is ready....                      */
                    (sock_rdy_err == DEF_YES)) {
                    /* ...then Rx data.                                     */
                    result = HTTPcSock_ConnDataRx(p_conn, rem_rx_len, &err);
                    switch (err) {
                    case HTTPc_ERR_NONE:
...
*
* Description : Process the header of a WebSocket message.
*
* Argument(s) : p_conn  Pointer to current HTTPc Connection (WebSocket Upgraded).
*
*               p_err   Pointer to variable that will receive the return error code from this function :
*
*                           HTTPc_ERR_NONE
*                           HTTPc_ERR_WEBSOCK_INVALID_RX_LEN
*
* Return(s)   : Remaining number of bytes to parse the hdr.
*
* Caller(s)   : HTTPcWebSock_ProcessRXTX().
*
* Note(s)     : none.
*********************************************************************************************************
*/
        ...
        HTTPc_ERR   *p_err)
{
    ...
    *p_err   = HTTPc_ERR_NONE;
    ...
    if (rsvd_bit == DEF_YES) {
        *p_err = HTTPc_ERR_WEBSOCK_RX_RSVD_BIT_1_SET;
        goto exit;
    }

    rsvd_bit = DEF_BIT_IS_SET(hdr_1, DEF_BIT_05);
    if (rsvd_bit == DEF_YES) {
        *p_err = HTTPc_ERR_WEBSOCK_RX_RSVD_BIT_2_SET;
        goto exit;
    }

    rsvd_bit = DEF_BIT_IS_SET(hdr_1, DEF_BIT_04);
    if (rsvd_bit == DEF_YES) {
        *p_err = HTTPc_ERR_WEBSOCK_RX_RSVD_BIT_3_SET;
        goto exit;
    }
    /* Process MASK bit.                                    */
    ...
    if (payload <= HTTPc_WEBSOCK_SMALL_FRAME_MAX_LEN) {
        ...
        if (payload != 0u) {
            *p_err = HTTPc_ERR_WEBSOCK_INVALID_RX_LEN;
            goto exit;
        }
    ...
    case HTTPc_WEBSOCK_OPCODE_BIN_FRAME:
        ...
        if (p_ws->TotalMsgLen != 0u) {
            *p_err = HTTPc_ERR_WEBSOCK_RX_FRAG_FRAME_SEQUENCE_INVALID;
            goto exit;
        }
    ...
    case HTTPc_WEBSOCK_OPCODE_PONG:
        ...
        if (p_ws->RxMsgLen > HTTPc_WEBSOCK_MAX_CTRL_FRAME_LEN) {
            *p_err = HTTPc_ERR_WEBSOCK_RX_CTRL_FRAME_TOO_LONG;
        }
        /* Control message cannot be fragmented.                */
        if (p_ws->RxMsgFlags.IsFin == DEF_NO) {
            *p_err = HTTPc_ERR_WEBSOCK_RX_CTRL_FRAME_FRAGMENTED;;
        }
        break;

    default:
        *p_err = HTTPc_ERR_WEBSOCK_RX_INVALID_OPCODE;
    }
    /* --------------- PROCESS MASKING KEY ---------------- */
    if (p_ws->RxMsgFlags.IsMasked == DEF_YES) {
...
*
* Description : Get the payload of a WebSocket message.
*
* Argument(s) : p_conn  Pointer to current HTTPc Connection (WebSocket Upgraded).
*
* Return(s)   : none.
*
* Caller(s)   : HTTPcWebSock_ProcessRX().
*
* Note(s)     : none.
*********************************************************************************************************
*/
...
*
* Description : Mask and copy a message into a buffer.
*
* Argument(s) : p_msg       Pointer to the WebSocket message object.
*
*               p_dst_buf   Pointer to the destination buffer.
*
*               p_src_buf   Pointer to the source buffer.
*
*               len         Length of the data to copy.
*
* Return(s)   : none.
*
* Caller(s)   : HTTPcWebSock_ProcessTX().
*
* Note(s)     : none.
*********************************************************************************************************
*/
...
*
* Description : Set the message header into the TX buffer..
*
* Argument(s) : p_conn      Pointer to current HTTPc Connection (WebSocket Upgraded).
*
*               p_tx_msg    Pointer to the WebSocket message object to send.
*
* Return(s)   : None.
*
* Caller(s)   : HTTPcWebSock_ProcessRXTX().
...
*
* Description : Set an internal Control message response using cached Rx Data.
*
* Argument(s) : p_conn  Pointer to current HTTPc Connection (WebSocket Upgraded).
*
*               opcode  Type of control frame to send.
*
* Return(s)   : none.
*
* Caller(s)   : HTTPcWebSock_ProcessRX().
        ...
        CPU_INT32U              msg_len)
{
    ...
    HTTPcWebSock_TxMsgAdd(p_msg_tx, &err);
...
*
* Description : Remove the first message in the TX message queue.
*
* Argument(s) : p_conn  Pointer to current HTTPc Connection (WebSocket Upgraded).
*
* Return(s)   : none.
*
* Caller(s)   : HTTPcWebSock_TX(),
...
*                                     HTTPcWebSock_TxMsgCleanList()
*
* Description : Remove all the pending WebSocket message.
*
* Argument(s) : p_conn  Pointer to current HTTPc Connection (WebSocket Upgraded).
*
* Return(s)   : none.
...
*
* Description : Compute the header length of a WebSocket Msg.
*
* Argument(s) : p_msg   Pointer to a WebSocket Msg obj.
*
* Return(s)   : Length of the header.
*
* Caller(s)   : HTTPcWebSock_TX().
*
* Note(s)     : none.
...
*
* Description : Check if the error code is related to a WebSocket protocol error.
*
* Argument(s) : err     Error code to check.
*
* Return(s)   : DEF_YES, if the error code is a protocol error.
*               DEF_NO,  if the error code is NOT a protocol error.
*
* Caller(s)   : HTTPcWebSock_Process().
*
* Note(s)     : none.
*********************************************************************************************************
*/
...
static  CPU_BOOLEAN  HTTPcWebSock_IsProtocolError(HTTPc_ERR  err)
{
    ...
    switch (err) {
    case HTTPc_ERR_WEBSOCK_RX_RSVD_BIT_1_SET:
    case HTTPc_ERR_WEBSOCK_RX_RSVD_BIT_2_SET:
    case HTTPc_ERR_WEBSOCK_RX_RSVD_BIT_3_SET:
    case HTTPc_ERR_WEBSOCK_RX_CTRL_FRAME_TOO_LONG:
    case HTTPc_ERR_WEBSOCK_RX_CTRL_FRAME_FRAGMENTED:
    case HTTPc_ERR_WEBSOCK_RX_INVALID_OPCODE:
    case HTTPc_ERR_WEBSOCK_RX_FRAG_FRAME_SEQUENCE_INVALID:
    case HTTPc_ERR_WEBSOCK_RX_CLOSE_CODE_INVALID:
        return DEF_YES;
...
*
* Description : Check if the Close code value is known and standarized.
*
* Argument(s) : close_code  Close code value to check.
*
* Return(s)   : DEF_YES, if the close code is standard.
*               DEF_NO,  if the close code is NOT standard.
*
* Caller(s)   : HTTPcWebSock_RX().
*
* Note(s)     : none.
*********************************************************************************************************
*/
