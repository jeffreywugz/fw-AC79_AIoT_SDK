...
u8_t tcp_client_databuf[200];   	//发送数据缓存
u8_t tcp_client_rx_databuf[200];
u8_t tcp_client_sta;				//客户端状态
//[7]:0,无连接;1,已经连接;
//[6]:0,无数据;1,收到客户端数据
//[5]:0,无数据;1,有数据需要发送
...
void tcp_client_demo_appcall(void)
{
    ...
    if (uip_newdata()) {
        if ((tcp_client_sta & (1 << 6)) == 0) { //还未收到数据
            ...
            memset(tcp_client_rx_databuf, 0, sizeof(tcp_client_rx_databuf));
            memcpy(tcp_client_rx_databuf, uip_appdata, uip_len);
...
//初始化TCP客户端  DHCP已经初始化了IP GW mask
void tcp_client_connect(void)
{
    uip_ipaddr_t ipaddr;
    uip_ipaddr(&ipaddr, 192, 168, 31, 228);	//设置IP为192.168.1.50
    uip_connect(&ipaddr, htons(TCP_CLIENT_PORT)); 	//端口为8000
}
//这里我们假定client端的IP地址为:192.168.1.103
//这个IP必须根据client端的IP修改.
//尝试重新连接
void tcp_client_reconnect()
{
    uip_ipaddr_t ipaddr;
    uip_ipaddr(&ipaddr, 192, 168, 31, 228);	//设置IP为192.168.1.50
    uip_connect(&ipaddr, htons(TCP_CLIENT_PORT)); 	//端口为1200
}
//终止连接
void tcp_client_aborted(void)
{
    ...
    DEBUG_PRINTF("tcp_client aborted!\r\n");//打印log
}
//连接超时
void tcp_client_timedout(void)
{
    tcp_client_sta &= ~(1 << 7);	//标志没有连接
    DEBUG_PRINTF("tcp_client timeout!\r\n");//打印log
}
//连接关闭
void tcp_client_closed(void)
{
    tcp_client_sta &= ~(1 << 7);	//标志没有连接
    tcp_client_reconnect();		//尝试重新连接
    DEBUG_PRINTF("tcp_client closed!\r\n");//打印log
}
//连接建立
void tcp_client_connected(void)
{
    ...
    DEBUG_PRINTF("tcp_client connected!\r\n");//打印log
    s->state = STATE_CMD; 		//指令状态
    s->textlen = 0;
    s->textptr = (u8_t *) "Board Connected Successfully!\r\n"; //回应消息
