...
//额外添加 兼容SDK
static int __NetDisconnect(void *net)
{
    if (((MqttNet *)net)->context) {
        NetDisconnect(((MqttNet *)net)->context);
...
int MqttClientNet_Init(MqttNet *net)
{
    ...
    if (net) {
        ...
        net->mqtt_connect = NetConnect;
        net->mqtt_read = NetRead;
        net->mqtt_write = NetWrite;
        net->disconnect = __NetDisconnect;  //原来是NetDisconnect
...
int SN_ClientNet_Init(MqttNet *net, MQTTCtx *mqttCtx)
{
    if (net) {
        ...
        XMEMSET(net, 0, sizeof(MqttNet));
        net->mqtt_connect = SN_NetConnect;
        net->mqtt_read = NetRead;
        net->mqtt_write = NetWrite;
        net->mqtt_peek = NetPeek;
        net->mqtt_disconnect = NetDisconnect;
