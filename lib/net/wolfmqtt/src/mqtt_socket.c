                                ...
                                void *ptr)
{
    ...
    rc = client->net->mqtt_read(client->net->context, (byte *)buf, sz,
                             ...
                             void *ptr)
{
    ...
    rc = client->net->mqtt_write(client->net->context, (byte *)buf, sz,
                                 client->tls.timeout_ms);
...
int MqttSocket_Init(MqttClient *client, MqttNet *net)
{
    ...
    if (client) {
        ...
        if (net && net->mqtt_connect && net->mqtt_read && net->mqtt_write && net->disconnect) {
                              ...
                              int timeout_ms)
{
    ...
    {
        rc = client->net->mqtt_write(client->net->context, buf, buf_len,
                                     timeout_ms);
    }
                     ...
                     int timeout_ms)
{
    ...
    /* Validate arguments */
    if (client == NULL || client->net == NULL || client->net->mqtt_write == NULL ||
        buf == NULL || buf_len <= 0) {
...
#endif /* ENABLE_MQTT_TLS */
    {
        rc = client->net->mqtt_read(client->net->context, buf, buf_len, timeout_ms);
    }
...
int MqttSocket_Read(MqttClient *client, byte *buf, int buf_len, int timeout_ms)
{
    ...
    /* Validate arguments */
    if (client == NULL || client->net == NULL || client->net->mqtt_read == NULL ||
        buf == NULL || buf_len <= 0) {
        return MQTT_CODE_ERROR_BAD_ARG;
    }
                       ...
                       int timeout_ms, int use_tls, MqttTlsCb cb)
{
    ...
    if ((client->flags & MQTT_CLIENT_FLAG_IS_CONNECTED) == 0) {
        ...
        /* Connect to host */
        rc = client->net->mqtt_connect(client->net->context, host, port, timeout_ms);
        if (rc != 0) {
            return rc;
        }
        client->flags |= MQTT_CLIENT_FLAG_IS_CONNECTED;
...
int MqttSocket_Disconnect(MqttClient *client)
{
    ...
    if (client) {
        ...
        /* rc = client->net->disconnect(client->net); */
        /* } */
        client->flags &= ~MQTT_CLIENT_FLAG_IS_CONNECTED;
    }
#ifdef WOLFMQTT_DEBUG_SOCKET
