 ...
 * GNU General Public License for more det internal.c
 *
 * Copyright (C) 2006-2020 wolfSSL Inc.
 *
 * This file is part of wolfSSL.
...
static int InitStreams(WOLFSSL *ssl)
{
    ...
    if (inflateInit(&ssl->d_stream) != Z_OK) {
        return ZLIB_INIT_ERROR;
    }
...
static int ExportCipherSpecState(WOLFSSL *ssl, byte *exp, word32 len, byte ver)
{
    ...
    c16toa(specs->key_size, exp + idx);
    idx += OPAQUE16_LEN;
    c16toa(specs->iv_size, exp + idx);
    idx += OPAQUE16_LEN;
    c16toa(specs->block_size, exp + idx);
    idx += OPAQUE16_LEN;
    c16toa(specs->aead_mac_size, exp + idx);
    idx += OPAQUE16_LEN;
                          ...
                          byte small)
{
    ...
    c32toa(keys->peer_sequence_number_hi, exp + idx);
    idx += OPAQUE32_LEN;
    c32toa(keys->peer_sequence_number_lo, exp + idx);
    idx += OPAQUE32_LEN;
    c32toa(keys->sequence_number_hi, exp + idx);
    idx += OPAQUE32_LEN;
    c32toa(keys->sequence_number_lo, exp + idx);
    idx += OPAQUE32_LEN;

    c16toa(keys->peerSeq[0].nextEpoch, exp + idx);
    idx += OPAQUE16_LEN;
    c16toa(keys->peerSeq[0].nextSeq_hi, exp + idx);
    idx += OPAQUE16_LEN;
    c32toa(keys->peerSeq[0].nextSeq_lo, exp + idx);
    idx += OPAQUE32_LEN;
    c16toa(keys->curEpoch, exp + idx);
    idx += OPAQUE16_LEN;
    c16toa(keys->curSeq_hi, exp + idx);
    idx += OPAQUE16_LEN;
    c32toa(keys->curSeq_lo, exp + idx);
    idx += OPAQUE32_LEN;
    c16toa(keys->peerSeq[0].prevSeq_hi, exp + idx);
    idx += OPAQUE16_LEN;
    c32toa(keys->peerSeq[0].prevSeq_lo, exp + idx);
    idx += OPAQUE32_LEN;
    ...
    c16toa(keys->dtls_sequence_number_hi, exp + idx);
    idx += OPAQUE16_LEN;
    c32toa(keys->dtls_sequence_number_lo, exp + idx);
    idx += OPAQUE32_LEN;
    c16toa(keys->dtls_prev_sequence_number_hi, exp + idx);
    idx += OPAQUE16_LEN;
    c32toa(keys->dtls_prev_sequence_number_lo, exp + idx);
    idx += OPAQUE32_LEN;
    c16toa(keys->dtls_epoch, exp + idx);
    idx += OPAQUE16_LEN;
    c16toa(keys->dtls_handshake_number, exp + idx);
    idx += OPAQUE16_LEN;
    c32toa(keys->encryptSz, exp + idx);
    idx += OPAQUE32_LEN;
    c32toa(keys->padSz, exp + idx);
    idx += OPAQUE32_LEN;
     ...
     * can be larger than DTLS_EXPORT_MIN_KEY_SZ */
    {
        ...
        c16toa(WOLFSSL_DTLS_WINDOW_WORDS, exp + idx);
        idx += OPAQUE16_LEN;
    ...
    if (sz > 0) {
        ...
        XMEMCPY(exp + idx, keys->client_write_MAC_secret, sz);
        idx += sz;
        XMEMCPY(exp + idx, keys->server_write_MAC_secret, sz);
        idx += sz;
#else
        XMEMSET(exp + idx, 0, sz);
        idx += sz;
        XMEMSET(exp + idx, 0, sz);
        idx += sz;
#endif
    ...
    exp[idx++] = sz;
    if (sz > 0) {
        XMEMCPY(exp + idx, keys->client_write_key, sz);
        idx += sz;
        XMEMCPY(exp + idx, keys->server_write_key, sz);
        idx += sz;
    }
    ...
    exp[idx++] = sz;
    if (sz > 0) {
        XMEMCPY(exp + idx, keys->client_write_IV, sz);
        idx += sz;
        XMEMCPY(exp + idx, keys->server_write_IV, sz);
        idx += sz;
    }
    ...
    exp[idx++] = sz;
    if (sz > 0) {
        XMEMCPY(exp + idx, keys->aead_enc_imp_IV, sz);
        idx += sz;
        XMEMCPY(exp + idx, keys->aead_dec_imp_IV, sz);
        idx += sz;
    }
...
static int ImportCipherSpecState(WOLFSSL *ssl, byte *exp, word32 len, byte ver)
{
    ...
    ato16(exp + idx, &specs->key_size);
    idx += OPAQUE16_LEN;
    ato16(exp + idx, &specs->iv_size);
    idx += OPAQUE16_LEN;
    ato16(exp + idx, &specs->block_size);
    idx += OPAQUE16_LEN;
    ato16(exp + idx, &specs->aead_mac_size);
    idx += OPAQUE16_LEN;
...
static int ImportKeyState(WOLFSSL *ssl, byte *exp, word32 len, byte ver)
{
    ...
    ato32(exp + idx, &keys->peer_sequence_number_hi);
    idx += OPAQUE32_LEN;
    ato32(exp + idx, &keys->peer_sequence_number_lo);
    idx += OPAQUE32_LEN;
    ato32(exp + idx, &keys->sequence_number_hi);
    idx += OPAQUE32_LEN;
    ato32(exp + idx, &keys->sequence_number_lo);
    idx += OPAQUE32_LEN;

    ato16(exp + idx, &keys->peerSeq[0].nextEpoch);
    idx += OPAQUE16_LEN;
    ato16(exp + idx, &keys->peerSeq[0].nextSeq_hi);
    idx += OPAQUE16_LEN;
    ato32(exp + idx, &keys->peerSeq[0].nextSeq_lo);
    idx += OPAQUE32_LEN;
    ato16(exp + idx, &keys->curEpoch);
    idx += OPAQUE16_LEN;
    ato16(exp + idx, &keys->curSeq_hi);
    idx += OPAQUE16_LEN;
    ato32(exp + idx, &keys->curSeq_lo);
    idx += OPAQUE32_LEN;
    ato16(exp + idx, &keys->peerSeq[0].prevSeq_hi);
    idx += OPAQUE16_LEN;
    ato32(exp + idx, &keys->peerSeq[0].prevSeq_lo);
    idx += OPAQUE32_LEN;
    ...
    ato16(exp + idx, &keys->dtls_sequence_number_hi);
    idx += OPAQUE16_LEN;
    ato32(exp + idx, &keys->dtls_sequence_number_lo);
    idx += OPAQUE32_LEN;
    ato16(exp + idx, &keys->dtls_prev_sequence_number_hi);
    idx += OPAQUE16_LEN;
    ato32(exp + idx, &keys->dtls_prev_sequence_number_lo);
    idx += OPAQUE32_LEN;
    ato16(exp + idx, &keys->dtls_epoch);
    idx += OPAQUE16_LEN;
    ato16(exp + idx, &keys->dtls_handshake_number);
    idx += OPAQUE16_LEN;
    ato32(exp + idx, &keys->encryptSz);
    idx += OPAQUE32_LEN;
    ato32(exp + idx, &keys->padSz);
    idx += OPAQUE32_LEN;
    ...
    if (sz > 0) {
        XMEMCPY(keys->client_write_MAC_secret, exp + idx, sz);
        idx += sz;
        XMEMCPY(keys->server_write_MAC_secret, exp + idx, sz);
        idx += sz;
    }
#else
    if (sz + idx > len) {
    ...
    if (sz > 0) {
        XMEMCPY(keys->client_write_key, exp + idx, sz);
        idx += sz;
        XMEMCPY(keys->server_write_key, exp + idx, sz);
        idx += sz;
    }
    ...
    if (sz > 0) {
        XMEMCPY(keys->client_write_IV, exp + idx, sz);
        idx += sz;
        XMEMCPY(keys->server_write_IV, exp + idx, sz);
        idx += sz;
    }
    ...
    if (sz > 0) {
        XMEMCPY(keys->aead_enc_imp_IV, exp + idx, sz);
        idx += sz;
        XMEMCPY(keys->aead_dec_imp_IV, exp + idx, sz);
        idx += sz;
    }
...
#ifndef NO_DH
    c16toa(options->minDhKeySz, exp + idx);
    idx += OPAQUE16_LEN;
    c16toa(options->maxDhKeySz, exp + idx);
    idx += OPAQUE16_LEN;
    c16toa(options->dhKeySz, exp + idx);
    idx += OPAQUE16_LEN;
#else
    c16toa(zero, exp + idx);
    idx += OPAQUE16_LEN;
...
#endif
#ifndef NO_RSA
    c16toa((word16)(options->minRsaKeySz), exp + idx);
    idx += OPAQUE16_LEN;
#else
    c16toa(zero, exp + idx);
    idx += OPAQUE16_LEN;
#endif
#ifdef HAVE_ECC
    c16toa((word16)(options->minEccKeySz), exp + idx);
    idx += OPAQUE16_LEN;
#else
    c16toa(zero, exp + idx);
    idx += OPAQUE16_LEN;
...
#ifndef NO_DH
    ato16(exp + idx, &(options->minDhKeySz));
    idx += OPAQUE16_LEN;
    ato16(exp + idx, &(options->maxDhKeySz));
    idx += OPAQUE16_LEN;
    ato16(exp + idx, &(options->dhKeySz));
    idx += OPAQUE16_LEN;
#else
    idx += OPAQUE16_LEN;
    idx += OPAQUE16_LEN;
...
#endif
#ifndef NO_RSA
    ato16(exp + idx, (word16 *) & (options->minRsaKeySz));
    idx += OPAQUE16_LEN;
#else
    idx += OPAQUE16_LEN;
#endif
#ifdef HAVE_ECC
    ato16(exp + idx, (word16 *) & (options->minEccKeySz));
    idx += OPAQUE16_LEN;
#else
    idx += OPAQUE16_LEN;
#endif
...
static int ExportPeerInfo(WOLFSSL *ssl, byte *exp, word32 len, byte ver)
{
    ...
    c16toa((word16)fam, exp + idx);
    idx += DTLS_EXPORT_LEN;
    c16toa((word16)ipSz, exp + idx);
    idx += DTLS_EXPORT_LEN;
    XMEMCPY(exp + idx, ip, ipSz);
    idx += ipSz;
    c16toa(port, exp + idx);
    idx += DTLS_EXPORT_LEN;
...
int wolfSSL_dtls_export_state_internal(WOLFSSL *ssl, byte *buf, word32 sz)
{
    ...
    c16toa((word16)ret, buf + idx - DTLS_EXPORT_LEN);
    idx += ret;
...
int wolfSSL_dtls_export_internal(WOLFSSL *ssl, byte *buf, word32 sz)
{
    ...
    c16toa((word16)DTLS_EXPORT_SPC_SZ, buf + idx);
    idx += DTLS_EXPORT_LEN;
...
int VerifyForTxDtlsMsgDelete(WOLFSSL *ssl, DtlsMsg *item)
{
    ...
    case WOLFSSL_CLIENT_END:
        if (item->type == client_hello &&
            ...
            return 1;
        } /* client can forget first client_hello if received full
    ...
    case WOLFSSL_SERVER_END:
        if (ssl->options.clientState >= CLIENT_FINISHED_COMPLETE &&
            ...
            return 1;
        } /* server can forget everything up to ServerHelloDone if
...
unsigned int LowResTimer(void)
{
    return (unsigned int)(((float)os_time_get()) / configTICK_RATE_HZ);
}
...
#ifdef WOLFSSL_CALLBACKS
    if (ssl->hsInfoOn) {
        AddPacketName(ssl, "Finished");
    }
    if (ssl->toInfoOn) {
        AddLateName("Finished", &ssl->timeoutInfo);
    }
#endif
                                ...
                                word32 *inOutIdx, word32 size)
{
    ...
    if (ssl->toInfoOn) {
        AddLateName("HelloVerifyRequest", &ssl->timeoutInfo);
    }
#endif
                  ...
                  word32 helloSz)
{
    ...
    if (ssl->toInfoOn) {
        AddLateName("ServerHello", &ssl->timeoutInfo);
    }
#endif
