                           ...
                           word32 inSz)
{
    ...
    if (GetInt(&key->dP, input, inOutIdx, inSz) < 0 ||
        ...
        GetInt(&key->u,  input, inOutIdx, inSz) < 0) {
        return ASN_RSA_KEY_E;
    }
#else
                ...
                struct tm *certTime, int *idx)
{
    ...
    if (format == ASN_UTC_TIME) {
        ...
        if (GetTime(&certTime->tm_year, date, idx) != 0) {
            return 0;
        }
    ...
    certTime->tm_year -= 1900;
    if (GetTime(&certTime->tm_mon, date, idx) != 0) {
        return 0;
    }
    certTime->tm_mon  -= 1;
    if (GetTime(&certTime->tm_mday, date, idx) != 0) {
        return 0;
    }
    if (GetTime(&certTime->tm_hour, date, idx) != 0) {
        return 0;
    }
    if (GetTime(&certTime->tm_min, date, idx) != 0) {
        return 0;
    }
    if (GetTime(&certTime->tm_sec, date, idx) != 0) {
        return 0;
    }
...
int GetAsnTimeString(void *currTime, byte *buf, word32 len)
{
    ...
    if (ts->tm_year >= 50 && ts->tm_year < 150) {
        ...
        *data_ptr = (byte) ASN_UTC_TIME;
        data_ptr++;
        /* -1 below excludes null terminator */
        *data_ptr = (byte) ASN_UTC_TIME_SIZE - 1;
        data_ptr++;
        ...
        *data_ptr = (byte) ASN_GENERALIZED_TIME;
        data_ptr++;
        /* -1 below excludes null terminator */
        *data_ptr = (byte) ASN_GENERALIZED_TIME_SIZE - 1;
        data_ptr++;
...
int AllocDer(DerBuffer **pDer, word32 length, int type, void *heap)
{
    ...
    if (pDer) {
        ...
        switch (type) {
        case CA_TYPE:
            dynType = DYNAMIC_TYPE_CA;
            break;
        case CERT_TYPE:
            dynType = DYNAMIC_TYPE_CERT;
            break;
        case CRL_TYPE:
            dynType = DYNAMIC_TYPE_CRL;
            break;
        case DSA_TYPE:
            dynType = DYNAMIC_TYPE_DSA;
            break;
        case ECC_TYPE:
            dynType = DYNAMIC_TYPE_ECC;
            break;
        case RSA_TYPE:
            dynType = DYNAMIC_TYPE_RSA;
            break;
        default:
            dynType = DYNAMIC_TYPE_KEY;
            break;
        }
...
int wc_PemGetHeaderFooter(int type, const char **header, const char **footer)
{
    ...
    case DSA_PARAM_TYPE:
        if (header) {
            *header = BEGIN_DSA_PARAM;
        }
        if (footer) {
            *footer = END_DSA_PARAM;
        }
        ret = 0;
        break;
#endif
