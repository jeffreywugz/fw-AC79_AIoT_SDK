                    ...
                    const byte *salt, word32 saltSz)
{
    ...
    if (!r) {
        r = SrpHashUpdate(&hash, (byte *) N, nSz);
    }
    ...
    if (!r) {
        r = SrpHashUpdate(&hash, (byte *) g, gSz);
    }
    if (!r) {
        r = SrpHashFinal(&hash, srp->k);
    }
    ...
    if (!r) {
        r = SrpHashUpdate(&hash, srp->user, srp->userSz);
    }
    if (!r) {
        r = SrpHashFinal(&hash, digest2);
    }
    ...
    if (!r) {
        r = SrpHashUpdate(&srp->client_proof, digest1, j);
    }
    if (!r) {
        r = SrpHashUpdate(&srp->client_proof, digest2, j);
    }
    if (!r) {
        r = SrpHashUpdate(&srp->client_proof, salt, saltSz);
    }
...
int wc_SrpSetPassword(Srp *srp, const byte *password, word32 size)
{
    ...
    if (!r) {
        r = SrpHashUpdate(&hash, (const byte *) ":", 1);
    }
    if (!r) {
        r = SrpHashUpdate(&hash, password, size);
    }
    if (!r) {
    ...
    if (!r) {
        r = SrpHashUpdate(&hash, srp->salt, srp->saltSz);
    }
    if (!r) {
        r = SrpHashUpdate(&hash, digest, digestSz);
    }
    if (!r) {
        r = SrpHashFinal(&hash, digest);
    }
    ...
    /* Set x (private key) */
    if (!r) {
        r = mp_read_unsigned_bin(&srp->auth, digest, digestSz);
    }
...
int wc_SrpGetVerifier(Srp *srp, byte *verifier, word32 *size)
{
    ...
    if (!r) {
        r = *size < (word32)mp_unsigned_bin_size(&v) ? BUFFER_E : MP_OKAY;
    }
    if (!r) {
        r = mp_to_unsigned_bin(&v, verifier);
    }
    if (!r) {
        *size = mp_unsigned_bin_size(&v);
    }
...
int wc_SrpSetPrivate(Srp *srp, const byte *priv, word32 size)
{
    ...
    if (!r) {
        r = mp_mod(&p, &srp->N, &srp->priv);
    }
    if (!r) {
        r = mp_iszero(&srp->priv) == MP_YES ? SRP_BAD_KEY_E : 0;
    }
...
int wc_SrpGetPublic(Srp *srp, byte *pub, word32 *size)
{
    ...
    if (srp->side == SRP_CLIENT_SIDE) {
        if (!r) {
            r = mp_exptmod(&srp->g, &srp->priv, &srp->N, &pubkey);
        }
        ...
        if (mp_init_multi(&i, &j, 0, 0, 0, 0) == MP_OKAY) {
            ...
            if (!r) {
                r = mp_iszero(&i) == MP_YES ? SRP_BAD_KEY_E : 0;
            }
            if (!r) {
                r = mp_exptmod(&srp->g, &srp->priv, &srp->N, &pubkey);
            }
            if (!r) {
                r = mp_mulmod(&i, &srp->auth, &srp->N, &j);
            }
            if (!r) {
                r = mp_add(&j, &pubkey, &i);
            }
            if (!r) {
                r = mp_mod(&i, &srp->N, &pubkey);
            }
    ...
    if (!r) {
        r = mp_to_unsigned_bin(&pubkey, pub);
    }
    if (!r) {
        *size = mp_unsigned_bin_size(&pubkey);
    }
    mp_clear(&pubkey);
...
static int wc_SrpSetKey(Srp *srp, byte *secret, word32 size)
{
    ...
    for (i = j = 0; j < srp->keySz; i++) {
        ...
        r = SrpHashInit(&hash, srp->type);
        if (!r) {
            r = SrpHashUpdate(&hash, secret, size);
        }
        if (!r) {
            r = SrpHashUpdate(&hash, counter, 4);
        }
                     ...
                     byte *serverPubKey, word32 serverPubKeySz)
{
    ...
    if (!r) {
        r = SrpHashUpdate(&hash, clientPubKey, clientPubKeySz);
    }
    ...
    if (!r) {
        r = SrpHashUpdate(&hash, serverPubKey, serverPubKeySz);
    }
    ...
    if (!r) {
        r = mp_read_unsigned_bin(&u, digest, SrpHashSize(srp->type));
    }
    ...
    if (!r && srp->side == SRP_CLIENT_SIDE) {
        ...
        if (!r) {
            r = mp_iszero(&temp1) == MP_YES ? SRP_BAD_KEY_E : 0;
        }
        if (!r) {
            r = mp_exptmod(&srp->g, &srp->auth, &srp->N, &temp2);
        }
        if (!r) {
            r = mp_mulmod(&temp1, &temp2, &srp->N, &s);
        }
        if (!r) {
            r = mp_read_unsigned_bin(&temp2, serverPubKey, serverPubKeySz);
        }
        if (!r) {
            r = mp_iszero(&temp2) == MP_YES ? SRP_BAD_KEY_E : 0;
        }
        if (!r) {
            r = mp_cmp(&temp2, &srp->N) != MP_LT ? SRP_BAD_KEY_E : 0;
        }
        if (!r) {
            r = mp_sub(&temp2, &s, &temp1);
        }

        /* temp2 = a + u * x */
        if (!r) {
            r = mp_mulmod(&u, &srp->auth, &srp->N, &s);
        }
        if (!r) {
            r = mp_add(&srp->priv, &s, &temp2);
        }

        /* secret = temp1 ^ temp2 % N */
        if (!r) {
            r = mp_exptmod(&temp1, &temp2, &srp->N, &s);
        }
        ...
        if (!r) {
            r = mp_read_unsigned_bin(&s, clientPubKey, clientPubKeySz);
        }
        if (!r) {
            r = mp_iszero(&s) == MP_YES ? SRP_BAD_KEY_E : 0;
        }
        if (!r) {
            r = mp_cmp(&s, &srp->N) != MP_LT ? SRP_BAD_KEY_E : 0;
        }
        if (!r) {
            r = mp_mulmod(&s, &temp1, &srp->N, &temp2);
        }

        /* rejects A * v ^ u % N >= 1, A * v ^ u % N == -1 % N */
        if (!r) {
            r = mp_read_unsigned_bin(&temp1, (const byte *)"\001", 1);
        }
        if (!r) {
            r = mp_cmp(&temp2, &temp1) != MP_GT ? SRP_BAD_KEY_E : 0;
        }
        if (!r) {
            r = mp_sub(&srp->N, &temp1, &s);
        }
        if (!r) {
            r = mp_cmp(&temp2, &s) == MP_EQ ? SRP_BAD_KEY_E : 0;
        }

        /* secret = temp2 * b % N */
        if (!r) {
            r = mp_exptmod(&temp2, &srp->priv, &srp->N, &s);
        }
    }
    ...
    if (!r) {
        r = mp_to_unsigned_bin(&s, secret);
    }
    if (!r) {
        r = srp->keyGenFunc_cb(srp, secret, mp_unsigned_bin_size(&s));
    }

    /* updating client proof = H( H(N) ^ H(g) | H(user) | salt | A | B | K) */

    if (!r) {
        r = SrpHashUpdate(&srp->client_proof, clientPubKey, clientPubKeySz);
    }
    if (!r) {
        r = SrpHashUpdate(&srp->client_proof, serverPubKey, serverPubKeySz);
    }
    if (!r) {
        r = SrpHashUpdate(&srp->client_proof, srp->key,     srp->keySz);
    }
...
int wc_SrpGetProof(Srp *srp, byte *proof, word32 *size)
{
    ...
    *size = SrpHashSize(srp->type);
    ...
    if (srp->side == SRP_CLIENT_SIDE) {
        ...
        if (!r) {
            r = SrpHashUpdate(&srp->server_proof, proof, *size);
        }
        if (!r) {
            r = SrpHashUpdate(&srp->server_proof, srp->key, srp->keySz);
        }
    }
