...
#error Both LITTLE_ENDIAN_ORDER and BIG_ENDIAN_ORDER defined.
#endif
    {
...
#ifdef BIG_ENDIAN_ORDER
        {
            ...
            case 0:
                do {
                    pd[idx + 0] = *b++; // fallthrough
                case 3:
                    pd[idx + 1] = *b++; // fallthrough
                case 2:
                    pd[idx + 2] = *b++; // fallthrough
                case 1:
                    pd[idx + 3] = *b++; // fallthrough
                    idx -= 4;
                } while ((c -= 4) > 0);
            }
        }
...
int fp_randprime(fp_int *N, int len, WC_RNG *rng, void *heap)
{
    ...
    do {
        ...
        mp_prime_is_prime_ex(N, 8, &isPrime, rng);//FIX_ME
        int puts(const char *out);
        puts("FIX_ME Miller-Rabin, just wait 3min... \r\n");
    } while (isPrime == FP_NO);
