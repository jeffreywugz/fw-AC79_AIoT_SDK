...
#pragma bss_seg(".ble_mesh_access_bss")
#pragma data_seg(".ble_mesh_access_data")
#pragma const_seg(".ble_mesh_access_const")
#pragma code_seg(".ble_mesh_access_code")
#endif
...
#if defined(CONFIG_BT_MESH_HEALTH_SRV)
...
static void publish_sent(int err, void *user_data)
{
    ...
    BT_DBG("mod->pub->count %d", mod->pub->count);
    if (mod->pub->count) {
...
int publish_retransmit(struct bt_mesh_model *mod)
{
...
static bool model_has_key(struct bt_mesh_model *mod, u16_t key)
{
    ...
    for (i = 0; i < ARRAY_SIZE(mod->keys); i++) {
        if (mod->keys[i] == key || key == BT_MESH_KEY_MY_DEV) {
            return true;
        }
    }
...
void bt_mesh_model_recv(struct bt_mesh_net_rx *rx, struct net_buf_simple *buf)
{
    ...
    BT_DBG("\n<Configuration messagees OpCode> 0x%08x", opcode);
...
u8 bt_mesh_each_elem_sig_model_numbers_get(u8 index)
{
    return dev_comp->elem[index].model_count;
}

u8 bt_mesh_each_elem_vendor_model_numbers_get(u8 index)
{
    return dev_comp->elem[index].vnd_model_count;
}
