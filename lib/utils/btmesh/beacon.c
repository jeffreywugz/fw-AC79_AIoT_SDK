...
#pragma bss_seg(".ble_mesh_beacon_bss")
#pragma data_seg(".ble_mesh_beacon_data")
#pragma const_seg(".ble_mesh_beacon_const")
#pragma code_seg(".ble_mesh_beacon_code")
#endif
...
#define UNPROVISIONED_INTERVAL     K_MSEC(config_bt_mesh_unprov_beacon_interval)
/* #define PROVISIONED_INTERVAL       K_SECONDS(10) */
#define PROVISIONED_INTERVAL       K_SECONDS(config_bt_mesh_secure_beacon_interval)
...
static int secure_beacon_send(void)
{
    ...
    for (i = 0; i < ARRAY_SIZE(bt_mesh.sub); i++) {
        ...
        BT_INFO("now=%d, %d, %d, %d", now, sub->beacon_sent, time_diff, BEACON_THRESHOLD(sub));
...
static int unprovisioned_beacon_send(void)
{
    ...
    if (oob_info & BT_MESH_PROV_OOB_URI) {
...
void bt_mesh_beacon_recv(struct net_buf_simple *buf)
{
    ...
    case BEACON_TYPE_UNPROVISIONED:
        printf("\nGet Provision Beacon\n");
        if (CONFIG_BT_MESH_PROVISIONER == 1) {
            link_start(buf);
        }
        break;
