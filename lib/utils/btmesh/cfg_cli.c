...
#pragma bss_seg(".ble_mesh_cfgcli_bss")
#pragma data_seg(".ble_mesh_cfgcli_data")
#pragma const_seg(".ble_mesh_cfgcli_const")
#pragma code_seg(".ble_mesh_cfgcli_code")
#endif
                           ...
                           struct net_buf_simple *buf)
{
    ...
    printf("cli->op_pending = 0x%x, OP_APP_KEY_STATUS = 0x%x\n", cli->op_pending, OP_APP_KEY_STATUS);
    ...
    param = cli->op_param;
    printf("param->net_idx = 0x%x, net_idx = %x, param->app_idx = 0x%x, app_idx = 0x%x\n", param->net_idx, net_idx, param->app_idx, app_idx);
                            ...
                            u8_t *status)
{
    ...
    static struct app_key_param param;
    param.status = status;
    param.net_idx = key_net_idx;
    param.app_idx = key_app_idx;

    printf("param.net_idx = 0x%x, param.app_idx = 0x%x\n", param.net_idx, param.app_idx);

    int err;
    err = cli_prepare(&param, OP_APP_KEY_STATUS);
    printf("cli->op_pending = 0x%x\n", cli->op_pending);

    struct app_key_param *test_param;
    test_param = cli->op_param;
    printf("cli->param->net_idx = 0x%x\n", test_param->net_idx);
    if (err) {
        return err;
    }
                        ...
                        u8_t *status)
{
    ...
    static struct mod_app_param param;
    param.status = status;
    param.elem_addr = elem_addr;
                   ...
                   u16_t sub_addr, u16_t mod_id, u16_t cid, u8_t *status)
{
    ...
    static struct mod_sub_param param;
    param.status = status;
    param.elem_addr = elem_addr;
    param.expect_sub = &sub_addr;
    param.mod_id = mod_id;
    param.cid = cid;
...
void cfg_cli_machine(void (*state_machine)(void))
{
    cli->cfg_next = state_machine;
}
