...
#pragma bss_seg(".ble_mesh_friend_bss")
#pragma data_seg(".ble_mesh_friend_data")
#pragma const_seg(".ble_mesh_friend_const")
#pragma code_seg(".ble_mesh_friend_code")
#endif
...
#define FRIEND_REQ_IO_0()           CUR_DEBUG_IO_0(A, 0)
#define FRIEND_REQ_IO_1()           CUR_DEBUG_IO_1(A, 0)
#define FRIEND_POLL_IO_0()          CUR_DEBUG_IO_0(A, 0)
#define FRIEND_POLL_IO_1()          CUR_DEBUG_IO_1(A, 0)
...
static void discard_buffer(void)
{
    ...
    for (i = 1; i < CONFIG_BT_MESH_FRIEND_LPN_COUNT; i++) {
...
static struct net_buf *friend_buf_alloc(u16_t src)
{
    ...
    BT_DBG("allocated buf addr 0x%x", buf);
...
static void friend_clear(struct bt_mesh_friend *frnd)
{
    ...
    frnd->queue_size = 0;
    frnd->pending_req = 0;
    (void)memset(frnd->sub_list, 0, FRIEND_SUB_LIST_SIZE * sizeof(u16_t));
}

void bt_mesh_friend_clear_net_idx(u16_t net_idx)
{
    BT_MESH_FEATURES_IS_SUPPORT_OPTIMIZE(BT_MESH_FEAT_FRIEND);
...
static void friend_sub_add(struct bt_mesh_friend *frnd, u16_t addr)
{
    ...
    for (i = 0; i < FRIEND_SUB_LIST_SIZE; i++) {
...
{
    BT_INFO("\n--rx TRANS_CTL_OP_FRIEND_SUB_ADD\n");
...
int bt_mesh_friend_sub_rem(struct bt_mesh_net_rx *rx,
                           struct net_buf_simple *buf)
{
    BT_INFO("\n--rx TRANS_CTL_OP_FRIEND_SUB_REM\n");
...
int bt_mesh_friend_clear_cfm(struct bt_mesh_net_rx *rx,
                             struct net_buf_simple *buf)
{
    BT_INFO("\n--rx TRANS_CTL_OP_FRIEND_CLEAR_CFM\n");
...
static void enqueue_offer(struct bt_mesh_friend *frnd, s8_t rssi)
{
    ...
    off->recv_win = CONFIG_BT_MESH_FRIEND_RECV_WIN,
         off->queue_size = CONFIG_BT_MESH_FRIEND_QUEUE_SIZE,
              off->sub_list_size = FRIEND_SUB_LIST_SIZE,
                   off->rssi = rssi,
...
static void friend_timeout(struct k_work *work)
{
    ...
    BT_DBG("lpn 0x%04x send_last %u last 0x%x", frnd->lpn,
           frnd->send_last, frnd->last);

    if (frnd->send_last && frnd->last) {
        BT_DBG("Sending frnd->last 0x%x", frnd->last);
        frnd->send_last = 0;
        goto send_last;
    }
    ...
    BT_DBG("Sending buf 0x%x from Friend Queue of LPN 0x%04x, queue_size %u",
           frnd->last, frnd->lpn, frnd->queue_size);
...
{
    BT_MESH_FEATURES_IS_SUPPORT_OPTIMIZE(BT_MESH_FEAT_FRIEND) 0;
                                 ...
                                 u16_t src)
{
    ...
    BT_DBG("SeqAuth 0x%x src 0x%04x", *seq_auth, src);
...
extern int bt_mesh_friend_cred_size_need(void);
extern void bt_mesh_friend_cred_malloc(void *frnd_cred);
...
void bt_mesh_friend_buf_alloc(void)
{
    ...
    u32 frnd_p, sub_list_p, seg_p, frnd_cred_p;

    buf_size = sizeof(struct net_buf) * FRIEND_BUF_COUNT;
    BT_DBG("net_buf size=0x%x", buf_size);
    net_buf_data_p = buf_size;
    buf_size += ALIGN_4BYTE(FRIEND_BUF_COUNT * BT_MESH_ADV_DATA_SIZE);
    BT_DBG("net_buf_data size=0x%x", ALIGN_4BYTE(FRIEND_BUF_COUNT * BT_MESH_ADV_DATA_SIZE));
    adv_pool_p = buf_size;
    buf_size += (sizeof(struct friend_adv) * FRIEND_BUF_COUNT);
    BT_DBG("adv_pool size=0x%x", sizeof(struct friend_adv) * FRIEND_BUF_COUNT);
    frnd_p = buf_size;
    buf_size += (sizeof(struct bt_mesh_friend) * CONFIG_BT_MESH_FRIEND_LPN_COUNT);
    BT_DBG("frnd size=0x%x", sizeof(struct bt_mesh_friend) * CONFIG_BT_MESH_FRIEND_LPN_COUNT);
    sub_list_p = buf_size;
    buf_size += ALIGN_4BYTE(FRIEND_SUB_LIST_SIZE * sizeof(u16_t));
    BT_DBG("sub_list size=0x%x", ALIGN_4BYTE(FRIEND_SUB_LIST_SIZE * sizeof(u16_t)));
    seg_p = buf_size;
    buf_size += (sizeof(struct bt_mesh_friend_seg) * FRIEND_SEG_RX);
    BT_DBG("seg size=0x%x", sizeof(struct bt_mesh_friend_seg) * FRIEND_SEG_RX);
    frnd_cred_p = buf_size;
    buf_size += bt_mesh_friend_cred_size_need();
    BT_DBG("frnd_cred size=0x%x", bt_mesh_friend_cred_size_need());
    ...
    BT_DBG("frnd addr=0x%x", frnd_p);
    BT_DBG("sub_list addr=0x%x", sub_list_p);
    BT_DBG("seg addr=0x%x", seg_p);
    BT_DBG("frnd_cred addr=0x%x", frnd_cred_p);
    BT_DBG("*fixed->data_pool=0x%x", *((u32 *)net_buf_fixed_friend_buf_pool.data_pool));
    BT_DBG("net_buf_adv_buf_pool user_data addr=0x%x\r\n", net_buf_friend_buf_pool->user_data);
}

void bt_mesh_friend_buf_free(void)
{
    free(NET_BUF_FREE(friend_buf_pool));
}

#endif /* NET_BUF_USE_MALLOC */

#endif /* defined(CONFIG_BT_MESH_FRIEND) */
