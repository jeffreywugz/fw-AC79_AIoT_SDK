...
#pragma bss_seg(".ble_mesh_lpn_bss")
#pragma data_seg(".ble_mesh_lpn_data")
#pragma const_seg(".ble_mesh_lpn_const")
#pragma code_seg(".ble_mesh_lpn_code")
#endif
...
#define LPN_REQ_IO_0()              CUR_DEBUG_IO_0(A, 0)
#define LPN_REQ_IO_1()              CUR_DEBUG_IO_1(A, 0)
#define LPN_POLL_IO_0()             CUR_DEBUG_IO_0(A, 0)
#define LPN_POLL_IO_1()             CUR_DEBUG_IO_1(A, 0)
...
static inline void group_zero(atomic_tt *target)
{
#if CONFIG_BT_MESH_LPN_GROUPS > 32
    int i;

    for (i = 0; i < ARRAY_SIZE(bt_mesh.lpn.added); i++) {
        atomic_sett(&target[i], 0);
    }
#else
    atomic_sett(target, 0);
#endif
}

static inline void group_set(atomic_tt *target, atomic_tt *source)
{
#if CONFIG_BT_MESH_LPN_GROUPS > 32
    int i;
...
static inline void group_clear(atomic_tt *target, atomic_tt *source)
{
#if CONFIG_BT_MESH_LPN_GROUPS > 32
    int i;
...
{
    BT_MESH_FEATURES_IS_SUPPORT_OPTIMIZE(BT_MESH_FEAT_LOW_POWER);
...
static inline int group_popcount(atomic_tt *target)
{
#if CONFIG_BT_MESH_LPN_GROUPS > 32
    int i, count = 0;
...
static void lpn_timeout(struct k_work *work)
{
    ...
    case BT_MESH_LPN_WAIT_OFFER:
        ...
        break;
    case BT_MESH_LPN_ESTABLISHED:
        BT_DBG("BT_MESH_LPN_ESTABLISHED");
        //< 3.6.6.4.2 Low Power messaging
        //It is recommended to resend this message 3 times, which assures a good balance
    ...
    case BT_MESH_LPN_RECV_DELAY:
        ...
        break;
    case BT_MESH_LPN_WAIT_UPDATE:
        BT_DBG("BT_MESH_LPN_WAIT_UPDATE");
        LPN_POLL_IO_0();
        update_timeout(lpn);
        break;
    default:
...
int bt_mesh_lpn_friend_sub_cfm(struct bt_mesh_net_rx *rx,
                               struct net_buf_simple *buf)
{
    BT_INFO("\n--rx TRANS_CTL_OP_FRIEND_SUB_CFM\n");
...
int bt_mesh_lpn_init(void)
{
    ...
    if (config_bt_mesh_lpn_powerup_add_sub_list) {
        bt_mesh_model_foreach(lpn_sub_add, NULL);
    }
