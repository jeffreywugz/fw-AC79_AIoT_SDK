...
#pragma bss_seg(".ble_mesh_net_bss")
#pragma data_seg(".ble_mesh_net_data")
#pragma const_seg(".ble_mesh_net_const")
#pragma code_seg(".ble_mesh_net_code")
#endif
...
#define FRIEND_CRED_COUNT_LPN   CONFIG_BT_MESH_SUBNET_COUNT
#else
#define FRIEND_CRED_COUNT       0
#define FRIEND_CRED_COUNT_LPN   0
#endif

#if NET_BUF_USE_MALLOC
static struct friend_cred *friend_cred;
#else
static struct friend_cred friend_cred[FRIEND_CRED_COUNT];
#endif /* NET_BUF_USE_MALLOC */

static struct friend_cred friend_cred_lpn[FRIEND_CRED_COUNT_LPN];
...
int friend_cred_set(struct friend_cred *cred, u8_t idx, const u8_t net_key[16])
{
...
#if defined(CONFIG_BT_MESH_LOW_POWER)
    if (BT_MESH_FEATURES_IS_SUPPORT(BT_MESH_FEAT_LOW_POWER)) {
    ...
    lpn_addr = cred->addr;
    frnd_addr = bt_mesh_primary_addr();
#endif /* defined(CONFIG_BT_MESH_LOW_POWER) */
...
void friend_cred_refresh(u16_t net_idx)
{
    ...
    for (i = 0; i < friend_cred_count; i++) {
        ...
        if (BT_MESH_FEATURES_IS_SUPPORT(BT_MESH_FEAT_FRIEND)) {
            cred = &friend_cred[i];
        } else {
            cred = &friend_cred_lpn[i];
        }
                                       ...
                                       u16_t lpn_counter, u16_t frnd_counter)
{
    ...
    for (cred = NULL, i = 0; i < friend_cred_count; i++) {
        if (BT_MESH_FEATURES_IS_SUPPORT(BT_MESH_FEAT_FRIEND)) {
            ...
            if ((friend_cred_lpn[i].addr == BT_MESH_ADDR_UNASSIGNED) ||
                (friend_cred_lpn[i].addr == addr &&
                 friend_cred_lpn[i].net_idx == sub->net_idx)) {
                cred = &friend_cred_lpn[i];
                break;
            }
        }
                       ...
                       u32_t iv_index)
{
    ...
    BT_DBG("Net key: %s", bt_hex(key, 16));
...
static void bt_mesh_net_local(struct net_buf *buf)
{
    BT_INFO("--func=%s", __FUNCTION__);
...
#if (defined(CONFIG_BT_MESH_LOW_POWER) || \
     defined(CONFIG_BT_MESH_FRIEND))
        if (BT_MESH_FEATURES_IS_SUPPORT(BT_MESH_FEAT_LOW_POWER | BT_MESH_FEAT_FRIEND)) {
...
static void relay_sent(u16_t *delay, u16_t *duration, void *cb_data)
{
    BT_DBG("internal relay_sent");
}
                              ...
                              struct bt_mesh_net_rx *rx)
{
    ...
    if (relay_to_adv(rx->net_if)) {
        bt_mesh_adv_send(buf, &relay_sent_cb, NULL);
    }
                      ...
                      enum bt_mesh_net_if net_if)
{
    ...
    BT_MESH_FEATURES_IS_SUPPORT_OPTIMIZE(BT_MESH_FEAT_RELAY);
...
#endif /* defined(CONFIG_BT_MESH_RELAY) */
}

static void ivu_refresh(void)
{
...
void bt_mesh_friend_cred_malloc(void *frnd_cred)
{
    friend_cred = (struct friend_cred *)frnd_cred;
}

int bt_mesh_friend_cred_size_need(void)
{
    return (sizeof(struct friend_cred) * FRIEND_CRED_COUNT);
}
