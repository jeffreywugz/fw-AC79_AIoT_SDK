...
#define RETRANSMIT_TIMEOUT   K_MSEC(config_bt_mesh_prov_retransmit_timeout)
#define BUF_TIMEOUT          K_MSEC(400)
/* #define CLOSING_TIMEOUT      K_SECONDS(3) */
#define CLOSING_TIMEOUT      K_SECONDS(config_bt_mesh_prov_link_close_timeout)
/* #define TRANSACTION_TIMEOUT  K_SECONDS(30) */
#define TRANSACTION_TIMEOUT  K_SECONDS(config_bt_mesh_prov_transaction_timeout)
/* #define PROTOCOL_TIMEOUT     K_SECONDS(60) */
#define PROTOCOL_TIMEOUT     K_SECONDS(config_bt_mesh_prov_protocol_timeout)
...
#if 0
    BT_DBG("Send Capabilities");
    BT_DBG("Elements: %u", buf.data[1]);
    BT_DBG("Algorithms:        %u", buf.data[2]);
    BT_DBG("Public Key Type:   0x%02x", buf.data[4]);
    BT_DBG("Static OOB Type:   0x%02x", buf.data[5]);
    BT_DBG("Output OOB Size:   %u", buf.data[6]);
    BT_DBG("Output OOB Action: 0x%04x", buf.data[7]);
    BT_DBG("Input OOB Size:    %u", buf.data[9]);
    BT_DBG("Input OOB Action:  0x%04x", buf.data[10]);
#endif /*  */
...
static void prov_data(const u8_t *data)
{
    ...
    BT_DBG("Dev key: %s", bt_hex(dev_key, 16));
     ...
     */
    if (IS_ENABLED(CONFIG_BT_MESH_GATT_PROXY) &&
        BT_MESH_FEATURES_IS_SUPPORT(BT_MESH_FEAT_PROXY) && identity_enable) {
...
extern int bt_mesh_pb_adv_open(const u8_t uuid[16], u16_t net_idx, u16_t addr, u8_t attention_duration);

void link_start(struct net_buf_simple *buf)
{
    printf("Link start");
    u8_t uuid[16];
    int i;
    ...
    struct bt_mesh_cdb_subnet *sub = bt_mesh_cdb_subnet_get(0);
    if (sub == NULL) {
        bt_mesh_cdb_subnet_alloc(0);
    }
    bt_mesh_pb_adv_open(uuid, 0, BT_MESH_ADDR_UNASSIGNED, 0);
}
...
static const char *const prov_handlers_name_table[] = {
    STR(prov_invite),
    STR(prov_capabilities),
    STR(prov_start),
    STR(prov_pub_key),
    STR(prov_input_complete),
    STR(prov_confirm),
    STR(prov_random),
    STR(prov_data),
    STR(prov_complete),
...
static const char *const gen_prov_name_table[] = {
    STR(gen_prov_start),
    STR(gen_prov_ack),
    STR(gen_prov_cont),
    STR(gen_prov_ctl),
...
static void gen_prov_recv(struct prov_rx *rx, struct net_buf_simple *buf)
{
    ...
    BT_DBG("\ngen_prov_recv RX \"%s\"\n", gen_prov_name_table[GPCF(rx->gpc)]);
