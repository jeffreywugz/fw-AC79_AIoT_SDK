...
#pragma bss_seg(".ble_mesh_transport_bss")
#pragma data_seg(".ble_mesh_transport_data")
#pragma const_seg(".ble_mesh_transport_const")
#pragma code_seg(".ble_mesh_transport_code")
#endif
                      ...
                      const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    if (tx->ctx->app_idx == BT_MESH_KEY_DEV || tx->ctx->app_idx == BT_MESH_KEY_MY_DEV) {
                    ...
                    const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    if (net_tx->ctx->app_idx == BT_MESH_KEY_DEV || net_tx->ctx->app_idx == BT_MESH_KEY_MY_DEV) {
        seg_hdr = SEG_HDR(0, 0);
    } else {
...
int bt_mesh_get_if_app_key_add(void)
{
    u8 cmp_key[16] = {0};

    for (int i = 0; i < ARRAY_SIZE(bt_mesh.app_keys); i++) {
        struct bt_mesh_app_key *key = &bt_mesh.app_keys[i];
        if (memcmp(key->keys[0].val, cmp_key, sizeof(cmp_key))) {
            return true;
        }
    }
                       ...
                       const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    if (tx->ctx->app_idx == BT_MESH_KEY_DEV) {
        ...
        struct bt_mesh_cdb_node *node = bt_mesh_cdb_node_get(tx->ctx->addr);
        if (!node) {
            log_info("TRANSPORT SEND, no find node for addr 0x%x\n", tx->ctx->addr);
            key = bt_mesh.dev_key;
            tx->aid = 0;
        } else {
            key = node->dev_key;
        ...
        key = bt_mesh.dev_key;
#endif
    } else if (tx->ctx->app_idx == BT_MESH_KEY_MY_DEV) {
        key = bt_mesh.dev_key;
    } else {
    ...
    err = bt_mesh_app_encrypt(key, tx->ctx->app_idx == BT_MESH_KEY_DEV || tx->ctx->app_idx == BT_MESH_KEY_MY_DEV,
...
#if (CONFIG_BT_MESH_PROVISIONER)
        struct bt_mesh_cdb_node *node = bt_mesh_cdb_node_get(rx->ctx.addr);
        u8_t *decrypt_key;
        if (!node) {
            log_info("sdu recv, no find node for addr 0x%x\n", rx->ctx.addr);
            decrypt_key = bt_mesh.dev_key;
        } else {
            decrypt_key = node->dev_key;
        }
#else
        decrypt_key = bt_mesh.dev_key;
#endif

        err = bt_mesh_app_decrypt(decrypt_key, true, aszmic, buf,
                                  &sdu, ad, rx->ctx.addr,
                                  rx->ctx.recv_dst, seq,
                    ...
                    struct net_buf_simple *buf, u64_t *seq_auth)
{
    ...
    BT_DBG("\n<Transport layer Control messages OpCode> 0x%02x len %u\n", ctl_op, buf->len);

    switch (ctl_op) {
    case TRANS_CTL_OP_ACK:
        BT_DBG("\nLower Transport layer Control message: ACK\n");
                     ...
                     const struct bt_mesh_send_cb *cb, void *cb_data)
{
    ...
    if (IS_ENABLED(CONFIG_BT_MESH_FRIEND) && BT_MESH_FEATURES_IS_SUPPORT(BT_MESH_FEAT_FRIEND)) {
        if (bt_mesh_friend_enqueue_tx(tx, BT_MESH_FRIEND_PDU_SINGLE,
                                      seq_auth, &buf->b) &&
            BT_MESH_ADDR_IS_UNICAST(tx->ctx->addr)) {
            /* PDUs for a specific Friend should only go
